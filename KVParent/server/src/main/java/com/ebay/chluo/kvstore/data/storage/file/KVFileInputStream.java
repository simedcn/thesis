...
public class KVFileInputStream extends FilterInputStream implements KVInputStream {
	...
	public KVFileInputStream(FileInputStream in, int blockSize, int startBlock, int offset)
			throws IOException {
		super(in);
		if (offset >= blockSize) {
	...
	public void readFully(byte[] b, int off, int len) throws IOException {
		...
		if (blockStream.available() >= len) {
			...
			for (;;) {
				...
				readNextBlock();
				toRead = blockStream.available() - (len - read) >= 0 ? blockStream.available()
						: (len - read);
			}
		}
	}
	 ...
	 * n<= {@link KVFileInputStream#getBlockPos()}. if n>0, n should not exceed
	 * the file size limit.
	 */
	@Override
