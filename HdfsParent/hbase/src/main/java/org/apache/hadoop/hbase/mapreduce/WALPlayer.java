...
public class WALPlayer extends Configured implements Tool {
   ...
   static class HLogKeyValueMapper extends Mapper<HLogKey, WALEdit, ImmutableBytesWritable, KeyValue> {
      ...
      @Override
      public void map(HLogKey key, WALEdit value, Context context) throws IOException {
         try {
            // skip all other tables
            if (Bytes.equals(table, key.getTablename())) {
               for (KeyValue kv : value.getKeyValues()) {
                  if (HLog.isMetaFamily(kv.getFamily()))
                     continue;
      ...
      public void map(HLogKey key, WALEdit value, Context context) throws IOException {
         try {
            if (tables.isEmpty() || tables.containsKey(key.getTablename())) {
               byte[] targetTable = tables.isEmpty() ? key.getTablename() : tables.get(key.getTablename());
   ...
   void setupTime(Configuration conf, String option) throws IOException {
      ...
      try {
         ...
         try {
            ...
            throw new IOException(option + " must be specified either in the form 2001-02-20T16:35:06.99 "
