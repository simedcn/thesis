...
public class ParseFilter {
   ...
   static {
      ...
      filterHashMap.put("SingleColumnValueFilter", ParseConstants.FILTER_PACKAGE + "." + "SingleColumnValueFilter");
   ...
   public byte[] extractFilterSimpleExpression(byte[] filterStringAsByteArray, int filterExpressionStartOffset)
      ...
      for (int i = filterExpressionStartOffset; i < filterStringAsByteArray.length; i++) {
         ...
         if (filterStringAsByteArray[i] == ParseConstants.RPAREN && (quoteCount % 2) == 0) {
            ...
            Bytes.putBytes(filterSimpleExpression, 0, filterStringAsByteArray, filterExpressionStartOffset, i
                  - filterExpressionStartOffset + 1);
   ...
   public static ArrayList<byte[]> getFilterArguments(byte[] filterStringAsByteArray) {
      int argumentListStartIndex = KeyValue.getDelimiter(filterStringAsByteArray, 0, filterStringAsByteArray.length,
         ...
         if (filterStringAsByteArray[i] == ParseConstants.SINGLE_QUOTE) {
            ...
            for (int j = argumentStartIndex + 1; j < filterStringAsByteArray.length; j++) {
               if (filterStringAsByteArray[j] == ParseConstants.SINGLE_QUOTE) {
                  if (isQuoteUnescaped(filterStringAsByteArray, j)) {
                     argumentEndIndex = j;
                     i = j + 1;
                     byte[] filterArgument = createUnescapdArgument(filterStringAsByteArray, argumentStartIndex,
                           argumentEndIndex);
            ...
            for (int j = argumentStartIndex; j < filterStringAsByteArray.length; j++) {
               if (filterStringAsByteArray[j] == ParseConstants.WHITESPACE
                  ...
                  Bytes.putBytes(filterArgument, 0, filterStringAsByteArray, argumentStartIndex, argumentEndIndex
    ...
    */
   public static boolean checkForOr(byte[] filterStringAsByteArray, int indexOfOr) throws CharacterCodingException,
         ArrayIndexOutOfBoundsException {

      try {
         if (filterStringAsByteArray[indexOfOr] == ParseConstants.O
               && filterStringAsByteArray[indexOfOr + 1] == ParseConstants.R
               && (filterStringAsByteArray[indexOfOr - 1] == ParseConstants.WHITESPACE || filterStringAsByteArray[indexOfOr - 1] == ParseConstants.RPAREN)
               && (filterStringAsByteArray[indexOfOr + 2] == ParseConstants.WHITESPACE || filterStringAsByteArray[indexOfOr + 2] == ParseConstants.LPAREN)) {
            return true;
         } else {
            return false;
         }
   ...
   public static boolean checkForAnd(byte[] filterStringAsByteArray, int indexOfAnd) throws CharacterCodingException {
      ...
      try {
         if (filterStringAsByteArray[indexOfAnd] == ParseConstants.A
               ...
               && (filterStringAsByteArray[indexOfAnd - 1] == ParseConstants.WHITESPACE || filterStringAsByteArray[indexOfAnd - 1] == ParseConstants.RPAREN)
               && (filterStringAsByteArray[indexOfAnd + 3] == ParseConstants.WHITESPACE || filterStringAsByteArray[indexOfAnd + 3] == ParseConstants.LPAREN)) {
            return true;
         } else {
            return false;
         }
      ...
      try {
         if (filterStringAsByteArray[indexOfSkip] == ParseConstants.S
               ...
               && (indexOfSkip == 0 || filterStringAsByteArray[indexOfSkip - 1] == ParseConstants.WHITESPACE
                     || filterStringAsByteArray[indexOfSkip - 1] == ParseConstants.RPAREN || filterStringAsByteArray[indexOfSkip - 1] == ParseConstants.LPAREN)
               && (filterStringAsByteArray[indexOfSkip + 4] == ParseConstants.WHITESPACE || filterStringAsByteArray[indexOfSkip + 4] == ParseConstants.LPAREN)) {
            return true;
         } else {
            return false;
         }
      ...
      try {
         if (filterStringAsByteArray[indexOfWhile] == ParseConstants.W
               ...
               && (indexOfWhile == 0 || filterStringAsByteArray[indexOfWhile - 1] == ParseConstants.WHITESPACE
                     || filterStringAsByteArray[indexOfWhile - 1] == ParseConstants.RPAREN || filterStringAsByteArray[indexOfWhile - 1] == ParseConstants.LPAREN)
               && (filterStringAsByteArray[indexOfWhile + 5] == ParseConstants.WHITESPACE || filterStringAsByteArray[indexOfWhile + 5] == ParseConstants.LPAREN)) {
            return true;
         } else {
            return false;
         }
   ...
   public static byte[] removeQuotesFromByteArray(byte[] quotedByteArray) {
      if (quotedByteArray == null || quotedByteArray.length < 2 || quotedByteArray[0] != ParseConstants.SINGLE_QUOTE
