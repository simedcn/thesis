...
public class ExecRPCInvoker implements InvocationHandler {
   ...
   public ExecRPCInvoker(Configuration conf, HConnection connection, Class<? extends CoprocessorProtocol> protocol,
         byte[] table, byte[] row) {
      this.conf = conf;
   ...
   @Override
   public Object invoke(Object instance, final Method method, final Object[] args) throws Throwable {
      ...
      if (row != null) {
         ...
         ServerCallable<ExecResult> callable = new ServerCallable<ExecResult>(connection, table, row) {
            public ExecResult call() throws Exception {
               return server.execCoprocessor(location.getRegionInfo().getRegionName(), exec);
            }
         };
