...
public class FSTableDescriptors implements TableDescriptors {
   ...
   public HTableDescriptor get(final String tablename) throws IOException {
      ...
      if (tdmt == null) {
         LOG.warn("The following folder is in HBase's root directory and " + "doesn't contain a table descriptor, "
   ...
   public void add(HTableDescriptor htd) throws IOException {
      ...
      if (!this.fsreadonly)
         updateHTableDescriptor(this.fs, this.rootdir, htd);
   ...
   private static FileStatus getTableInfoPath(final FileSystem fs, final Path rootdir, final String tableName)
         throws IOException {
    ...
    */
   private static final Pattern SUFFIX = Pattern.compile(TABLEINFO_NAME + "(\\.([0-9]{" + WIDTH_OF_SEQUENCE_ID
         + "}))?$");
   ...
   static Path getTableInfoFileName(final Path tabledir, final int sequenceid) {
      return new Path(tabledir, TABLEINFO_NAME + "." + formatTableInfoSequenceId(sequenceid));
   }
    ...
    * @throws IOException
    */
   static long getTableInfoModtime(final FileSystem fs, final Path rootdir, final String tableName) throws IOException {
      FileStatus status = getTableInfoPath(fs, rootdir, tableName);
    ...
    * @throws IOException
    */
   public static HTableDescriptor getTableDescriptor(FileSystem fs, Path hbaseRootDir, byte[] tableName)
         throws IOException {
      HTableDescriptor htd = null;
      try {
         TableDescriptorModtime tdmt = getTableDescriptorModtime(fs, hbaseRootDir, Bytes.toString(tableName));
         htd = tdmt == null ? null : tdmt.getTableDescriptor();
      } catch (NullPointerException e) {
         LOG.debug("Exception during readTableDecriptor. Current table name = " + Bytes.toString(tableName), e);
      }
      return htd;
   }
   ...
   static TableDescriptorModtime getTableDescriptorModtime(FileSystem fs, Path hbaseRootDir, String tableName)
         throws NullPointerException, IOException {
   ...
   static TableDescriptorModtime getTableDescriptorModtime(FileSystem fs, Path tableDir) throws NullPointerException,
         IOException {
      if (tableDir == null)
   ...
   public static HTableDescriptor getTableDescriptor(FileSystem fs, Path tableDir) throws IOException,
    ...
    * @throws IOException 
    */
   private static Path writeTableDescriptor(final FileSystem fs, final HTableDescriptor hTableDescriptor,
         final Path tableDir, final FileStatus status) throws IOException {
    ...
    * @param conf
    */
   public static boolean createTableDescriptor(final HTableDescriptor htableDescriptor, Configuration conf)
         throws IOException {
    ...
    */
   static boolean createTableDescriptor(final HTableDescriptor htableDescriptor, final Configuration conf,
    ...
    * @param rootdir
    */
   public static boolean createTableDescriptor(FileSystem fs, Path rootdir, HTableDescriptor htableDescriptor)
         throws IOException {
