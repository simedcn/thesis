...
public class HRegionServer implements HRegionInterface, HBaseRPCErrorHandler, Runnable, RegionServerServices {
   ...
   public HRegionServer(Configuration conf) throws IOException, InterruptedException {
      ...
      this.rpcServer = HBaseRPC.getServer(this,
            new Class<?>[] { HRegionInterface.class, HBaseRPCErrorHandler.class, OnlineRegions.class },
            initialIsa.getHostName(), // BindAddress is IP we got for this server.
            initialIsa.getPort(), conf.getInt("hbase.regionserver.handler.count", 10),
            conf.getInt("hbase.regionserver.metahandler.count", 10), conf.getBoolean("hbase.rpc.verbose", false), conf,
      ...
      ZKUtil.loginClient(this.conf, "hbase.zookeeper.client.keytab.file", "hbase.zookeeper.client.kerberos.principal",
            this.isa.getHostName());

      // login the server principal (if using secure Hadoop)
      User.login(this.conf, "hbase.regionserver.keytab.file", "hbase.regionserver.kerberos.principal",
            this.isa.getHostName());
   ...
   private void initializeZooKeeper() throws IOException, InterruptedException {
      ...
      this.catalogTracker = new CatalogTracker(this.zooKeeper, this.conf, this, this.conf.getInt(
            "hbase.regionserver.catalog.timeout", Integer.MAX_VALUE));
   ...
   HServerLoad buildServerLoad() {
      ...
      return new HServerLoad(requestCount.get(), (int) metrics.getRequests(), (int) (memory.getUsed() / 1024 / 1024),
            (int) (memory.getMax() / 1024 / 1024), regionLoads, this.hlog.getCoprocessorHost().getCoprocessors());
   }
   ...
   protected void handleReportForDutyResponse(final MapWritable c) throws IOException {
      try {
         ...
         startServiceThreads();
         LOG.info("Serving as " + this.serverNameFromMasterPOV + ", RPC listening on " + this.isa + ", sessionid=0x"
   ...
   private HServerLoad.RegionLoad createRegionLoad(final HRegion r) {
      ...
      return new HServerLoad.RegionLoad(name, stores, storefiles, storeUncompressedSizeMB, storefileSizeMB,
            memstoreSizeMB, storefileIndexSizeMB, rootIndexSizeKB, totalStaticIndexSizeKB, totalStaticBloomSizeKB,
            (int) r.readRequestsCount.get(), (int) r.writeRequestsCount.get(), totalCompactingKVs, currentCompactedKVs);
   }
   ...
   private IOException convertThrowableToIOE(final Throwable t, final String msg) {
      return (t instanceof IOException ? (IOException) t : msg == null || msg.length() == 0 ? new IOException(t)
   ...
   private static class CompactionChecker extends Chore {
      ...
      CompactionChecker(final HRegionServer h, final int sleepTime, final Stoppable stopper) {
          ...
          */
         this.majorCompactPriority = this.instance.conf.getInt(
               "hbase.regionserver.compactionChecker.majorCompactPriority", DEFAULT_PRIORITY);
      }
            ...
            for (Store s : r.getStores().values()) {
               try {
                  if (s.needsCompaction()) {
                     ...
                     this.instance.compactSplitThread.requestCompaction(r, s, getName() + " requests compaction");
                     ...
                     if (majorCompactPriority == DEFAULT_PRIORITY || majorCompactPriority > r.getCompactPriority()) {
                        ...
                        this.instance.compactSplitThread.requestCompaction(r, s, getName()
                              + " requests major compaction; use configured priority", this.majorCompactPriority);
                     }
                  }
               } catch (IOException e) {
   ...
   private HLog setupWALAndReplication() throws IOException {
      ...
      Path logdir = new Path(rootDir, HLog.getHLogDirectoryName(this.serverNameFromMasterPOV.toString()));
      if (LOG.isDebugEnabled())
   ...
   protected void metrics() {
      ...
      this.metrics.updatesBlockedSecondsHighWater
            .update(updatesBlockedMsHigherWater > 0 ? updatesBlockedMsHigherWater / 1000 : 0);
   ...
   private boolean isHealthy() {
      ...
      if (!(leases.isAlive() && cacheFlusher.isAlive() && hlogRoller.isAlive() && this.compactionChecker.isAlive())) {
   ...
   public void postOpenDeployTasks(final HRegion r, final CatalogTracker ct, final boolean daughter)
      ...
      checkOpen();
      LOG.info("Post open deploy tasks for region=" + r.getRegionNameAsString() + ", daughter=" + daughter);
      ...
      LOG.info("Done with post open deploy task for region=" + r.getRegionNameAsString() + ", daughter=" + daughter);
   ...
   private ServerName getMaster() {
      ...
      while (keepLooping() && master == null) {
         ...
         try {
            ...
            master = (HMasterRegionInterface) HBaseRPC.waitForProxy(HMasterRegionInterface.class,
                  HMasterRegionInterface.VERSION, masterIsa, this.conf, -1, this.rpcTimeout, this.rpcTimeout);
         } catch (IOException e) {
   ...
   private MapWritable reportForDuty() throws IOException {
      ...
      try {
         this.requestCount.set(0);
         LOG.info("Telling master at " + masterServerName + " that we are up " + "with port=" + this.isa.getPort()
   ...
   public HRegionInfo getRegionInfo(final byte[] regionName) throws NotServingRegionException, IOException {
      checkOpen();
   ...
   private boolean checkAndMutate(final byte[] regionName, final byte[] row, final byte[] family,
         final byte[] qualifier, final CompareOp compareOp, final WritableByteArrayComparable comparator,
    ...
    */
   public boolean checkAndPut(final byte[] regionName, final byte[] row, final byte[] family, final byte[] qualifier,
      ...
      if (region.getCoprocessorHost() != null) {
         Boolean result = region.getCoprocessorHost().preCheckAndPut(row, family, qualifier, CompareOp.EQUAL,
               comparator, put);
         if (result != null) {
            return result.booleanValue();
         }
      }
      boolean result = checkAndMutate(regionName, row, family, qualifier, CompareOp.EQUAL, comparator, put, lock);
      if (region.getCoprocessorHost() != null) {
         result = region.getCoprocessorHost().postCheckAndPut(row, family, qualifier, CompareOp.EQUAL, comparator, put,
               result);
      }
      return result;
   }
    ...
    */
   public boolean checkAndPut(final byte[] regionName, final byte[] row, final byte[] family, final byte[] qualifier,
         final CompareOp compareOp, final WritableByteArrayComparable comparator, final Put put) throws IOException {
      checkOpen();
      if (regionName == null) {
         throw new IOException("Invalid arguments to checkAndPut " + "regionName is null");
      }
      ...
      boolean result = checkAndMutate(regionName, row, family, qualifier, compareOp, comparator, put, lock);
      if (region.getCoprocessorHost() != null) {
         result = region.getCoprocessorHost().postCheckAndPut(row, family, qualifier, compareOp, comparator, put,
               result);
      }
      return result;
   }
   ...
   public boolean checkAndDelete(final byte[] regionName, final byte[] row, final byte[] family,
      ...
      WritableByteArrayComparable comparator = new BinaryComparator(value);
      if (region.getCoprocessorHost() != null) {
         Boolean result = region.getCoprocessorHost().preCheckAndDelete(row, family, qualifier, CompareOp.EQUAL,
               comparator, delete);
         if (result != null) {
            return result.booleanValue();
         }
      ...
      if (region.getCoprocessorHost() != null) {
         result = region.getCoprocessorHost().postCheckAndDelete(row, family, qualifier, CompareOp.EQUAL, comparator,
               delete, result);
      }
      return result;
   }
   ...
   public boolean checkAndDelete(final byte[] regionName, final byte[] row, final byte[] family,
      ...
      Integer lock = getLockFromId(delete.getLockId());
      if (region.getCoprocessorHost() != null) {
         Boolean result = region.getCoprocessorHost().preCheckAndDelete(row, family, qualifier, compareOp, comparator,
               delete);
         if (result != null) {
            return result.booleanValue();
         }
      }
      boolean result = checkAndMutate(regionName, row, family, qualifier, compareOp, comparator, delete, lock);
      if (region.getCoprocessorHost() != null) {
         result = region.getCoprocessorHost().postCheckAndDelete(row, family, qualifier, compareOp, comparator, delete,
               result);
      }
      return result;
   }
   ...
   private void checkIfRegionInTransition(HRegionInfo region, String currentAction)
      ...
      if (this.regionsInTransitionInRS.containsKey(encodedName)) {
         ...
         throw new RegionAlreadyInTransitionException("Received:" + currentAction + " for the region:"
               + region.getRegionNameAsString() + " ,which we are already trying to " + (openAction ? OPEN : CLOSE)
               + ".");
      }
   }
   ...
   @Override
   @QosPriority(priority = HConstants.HIGH_QOS)
   public boolean closeRegion(final HRegionInfo region, final int versionOfClosingNode) throws IOException {
   ...
   @QosPriority(priority = HConstants.HIGH_QOS)
   protected boolean closeRegion(HRegionInfo region, final boolean zk, final int versionOfClosingNode)
         throws IOException {
      checkOpen();
      ...
      LOG.info("Received close region: " + region.getRegionNameAsString() + ". Version of ZK closing node:"
   ...
   */
    ...
    * @return True if closed a region.
    */
   protected boolean closeRegion(byte[] encodedRegionName, final boolean abort, final boolean zk) throws IOException {
      ...
      LOG.error("The specified region name" + encodedRegionNameStr + " does not exist to close the region.");
      return false;
   }
   ...
   @Override
   @QosPriority(priority = HConstants.HIGH_QOS)
   public void compactRegion(HRegionInfo regionInfo, boolean major, byte[] family) throws NotServingRegionException,
         IOException {
      checkOpen();
      HRegion region = getRegion(regionInfo.getRegionName());
      Store store = null;
      if (family != null) {
         ...
         if (store == null) {
            throw new IOException("column family " + Bytes.toString(family) + " does not exist in region "
   ...
   public long incrementColumnValue(byte[] regionName, byte[] row, byte[] family, byte[] qualifier, long amount,
      ...
      try {
         HRegion region = getRegion(regionName);
         if (region.getCoprocessorHost() != null) {
            Long amountVal = region.getCoprocessorHost().preIncrementColumnValue(row, family, qualifier, amount,
                  writeToWAL);
            if (amountVal != null) {
         ...
         if (region.getCoprocessorHost() != null) {
            retval = region.getCoprocessorHost().postIncrementColumnValue(row, family, qualifier, amount, writeToWAL,
                  retval);
         }
         return retval;
      } catch (IOException e) {
   ...
   public ServerName getServerName() {
      ...
      return this.serverNameFromMasterPOV == null ? new ServerName(this.isa.getHostName(), this.isa.getPort(),
            this.startcode) : this.serverNameFromMasterPOV;
   }
    ...
    */
   static private void createNewReplicationInstance(Configuration conf, HRegionServer server, FileSystem fs,
      ...
      if (sourceClassname.equals(sinkClassname)) {
         server.replicationSourceHandler = (ReplicationSourceService) newReplicationInstance(sourceClassname, conf,
               server, fs, logDir, oldLogDir);
         server.replicationSinkHandler = (ReplicationSinkService) server.replicationSourceHandler;
      } else {
         server.replicationSourceHandler = (ReplicationSourceService) newReplicationInstance(sourceClassname, conf,
               server, fs, logDir, oldLogDir);
         server.replicationSinkHandler = (ReplicationSinkService) newReplicationInstance(sinkClassname, conf, server,
               fs, logDir, oldLogDir);
      }
   }

   static private ReplicationService newReplicationInstance(String classname, Configuration conf, HRegionServer server,
    ...
    * @throws IOException
    */
   public static Thread startRegionServer(final HRegionServer hrs, final String name) throws IOException {
