...
public class IncreasingToUpperBoundRegionSplitPolicy extends ConstantSizeRegionSplitPolicy {
   ...
   protected boolean shouldSplit() {
      ...
      for (Store store : region.getStores().values()) {
         ...
         if (size > sizeToCheck) {
            LOG.debug("ShouldSplit because " + store.getColumnFamilyName() + " size=" + size + ", sizeToCheck="
                  + sizeToCheck + ", regionsWithCommonTable=" + tableRegionsCount);
            foundABigStore = true;
            break;
         }
      }
   ...
   long getSizeToCheck(final int tableRegionsCount) {
      return tableRegionsCount == 0 ? getDesiredMaxFileSize() : Math.min(getDesiredMaxFileSize(), this.flushSize
