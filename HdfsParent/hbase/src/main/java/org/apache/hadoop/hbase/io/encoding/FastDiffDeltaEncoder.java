...
public class FastDiffDeltaEncoder extends BufferedDataBlockEncoder {
   ...
   private void compressSingleKeyValue(FastDiffCompressionState previousState, FastDiffCompressionState currentState,
      ...
      if (previousState.isFirst()) {
         ...
         int commonPrefix = ByteBufferUtils.findCommonPrefix(in, in.position(), previousState.prevOffset
               + KeyValue.ROW_OFFSET, Math.min(keyLength, previousState.keyLength) - KeyValue.TIMESTAMP_TYPE_SIZE);
         ...
         if (valueLength == previousState.valueLength) {
            ...
            if (ByteBufferUtils.arePartsEqual(in, previousValueOffset, previousState.valueLength, valueOffset,
                  valueLength)) {
         ...
         if (commonPrefix < currentState.rowLength + KeyValue.ROW_LENGTH_SIZE) {
            ...
            ByteBufferUtils.moveBufferToStream(out, in, currentState.rowLength + KeyValue.ROW_LENGTH_SIZE
                  - commonPrefix);
   ...
   private int findCommonTimestampPrefix(FastDiffCompressionState left, FastDiffCompressionState right) {
      ...
      while (prefixTimestamp < (KeyValue.TIMESTAMP_SIZE - 1)
            && left.timestamp[prefixTimestamp] == right.timestamp[prefixTimestamp]) {
         prefixTimestamp++;
      }
      return prefixTimestamp; // has to be at most 7 bytes
   }

   private void uncompressSingleKeyValue(DataInputStream source, ByteBuffer out, FastDiffCompressionState state)
      ...
      if (!state.isFirst()) {
         ...
         if (commonLength < state.rowLength + KeyValue.ROW_LENGTH_SIZE) {
            ...
            ByteBufferUtils.copyFromBufferToBuffer(out, out, state.prevOffset + KeyValue.ROW_OFFSET
                  + KeyValue.ROW_LENGTH_SIZE + state.rowLength, state.familyLength + KeyValue.FAMILY_LENGTH_SIZE);
         ...
         if ((flag & FLAG_SAME_TYPE) != 0) {
            out.put(state.type);
            if ((flag & FLAG_SAME_VALUE) != 0) {
               ByteBufferUtils.copyFromBufferToBuffer(out, out, state.prevOffset + KeyValue.ROW_OFFSET + prevKeyLength,
                     state.valueLength);
            } else {
   ...
   protected static class FastDiffSeekerState extends SeekerState {
      ...
      protected void copyFromNext(SeekerState that) {
         ...
         System.arraycopy(other.prevTimestampAndType, 0, prevTimestampAndType, 0, KeyValue.TIMESTAMP_TYPE_SIZE);
   ...
   public EncodedSeeker createSeeker(RawComparator<byte[]> comparator, final boolean includesMemstoreTS) {
      return new BufferedEncodedSeeker<FastDiffSeekerState>(comparator) {
         private void decode(boolean isFirst) {
            ...
            if ((flag & FLAG_SAME_KEY_LENGTH) == 0) {
               if (!isFirst) {
                  System.arraycopy(current.keyBuffer, current.keyLength - current.prevTimestampAndType.length,
                        current.prevTimestampAndType, 0, current.prevTimestampAndType.length);
               }
            ...
            if (isFirst) {
               // copy everything
               currentBuffer.get(current.keyBuffer, current.lastCommonPrefix, current.keyLength
               ...
               currentBuffer.get(current.keyBuffer, current.lastCommonPrefix, Bytes.SIZEOF_SHORT
                     - current.lastCommonPrefix);
               current.rowLengthWithSize = Bytes.toShort(current.keyBuffer, 0) + Bytes.SIZEOF_SHORT;

               // move the column family
               System.arraycopy(current.keyBuffer, oldRowLengthWithSize, current.keyBuffer, current.rowLengthWithSize,
               ...
               // copy the qualifier
               currentBuffer.get(current.keyBuffer, current.rowLengthWithSize + current.familyLengthWithSize,
               ...
               currentBuffer.get(current.keyBuffer, current.lastCommonPrefix, current.rowLengthWithSize
                     - current.lastCommonPrefix);
               ...
               currentBuffer.get(current.keyBuffer, current.lastCommonPrefix, current.keyLength
                     - current.prevTimestampAndType.length - current.lastCommonPrefix);
            }
