...
public abstract class HBaseServer implements RpcServer {
   ...
   protected static final ThreadLocal<RpcServer> SERVER = new ThreadLocal<RpcServer>();
   ...
   static Class<? extends VersionedProtocol> getProtocolClass(String protocolName, Configuration conf)
         throws ClassNotFoundException {
      Class<? extends VersionedProtocol> protocol = PROTOCOL_CACHE.get(protocolName);

      if (protocol == null) {
         protocol = (Class<? extends VersionedProtocol>) conf.getClassByName(protocolName);
   ...
   protected final List<Connection> connectionList = Collections.synchronizedList(new LinkedList<Connection>());
   ...
   protected class Call implements RpcCallContext {
      ...
      public synchronized void endDelayThrowing(Throwable t) throws IOException {
         this.setResponse(null, Status.ERROR, t.getClass().toString(), StringUtils.stringifyException(t));
         this.delayResponse = false;
      ...
      public void throwExceptionIfCallerDisconnected() throws CallerDisconnectedException {
         if (!connection.channel.isOpen()) {
            ...
            throw new CallerDisconnectedException("Aborting call " + this + " after " + afterTime + " ms, since "
                  + "caller disconnected");
         }
      }
   ...
   private class Listener extends Thread {
      ...
      public Listener() throws IOException {
         ...
         readPool = Executors.newFixedThreadPool(readThreads,
               new ThreadFactoryBuilder().setNameFormat("IPC Reader %d on port " + port).setDaemon(true).build());
            ...
            while (running) {
               ...
               try {
                  ...
                  if (running) { // unexpected -- log it
                     LOG.info(getName() + " unexpectedly interrupted: " + StringUtils.stringifyException(e));
                  }
         ...
         try {
            ...
            synchronized (responseQueue) {
               ...
               if (!call.response.hasRemaining()) {
                  ...
                  if (LOG.isDebugEnabled()) {
                     LOG.debug(getName() + ": responding to #" + call.id + " from " + call.connection + " Wrote "
                           + numBytes + " bytes.");
                  }
               } else {
                  if (inHandler) {
   ...
   protected class Connection {
      ...
      private void setupBadVersionResponse(int clientVersion) throws IOException {
         ...
         if (clientVersion >= 3) {
            ...
            setupResponse(buffer, fakeCall, Status.FATAL, null, VersionMismatch.class.getName(), errMsg);
      ...
      protected void processData(byte[] buf) throws IOException, InterruptedException {
         ...
         if ((callSize + callQueueSize.get()) > maxQueueSize) {
            ...
            setupResponse(responseBuffer, callTooBig, Status.FATAL, null, IOException.class.getName(),
         ...
         try {
            ...
            setupResponse(responseBuffer, readParamsFailedCall, Status.FATAL, null, t.getClass().getName(),
         ...
         while (running) {
            try {
               ...
               try {
                  ...
                  // make the call
                  value = call(call.connection.protocol, call.param, call.timestamp, status);
               } catch (Throwable e) {
   ...
   protected HBaseServer(String bindAddress, int port, Class<? extends Writable> paramClass, int handlerCount,
         int priorityHandlerCount, Configuration conf, String serverName, int highPriorityLevel) throws IOException {
      ...
      if (oldMaxQueueSize == null) {
         this.maxQueueLength = this.conf.getInt("ipc.server.max.callqueue.length", handlerCount
      ...
      this.responseQueuesSizeThrottler = new SizeBasedThrottler(conf.getLong(RESPONSE_QUEUES_MAX_SIZE,
            DEFAULT_RESPONSE_QUEUES_MAX_SIZE));
    ...
    * @throws IOException
    */
   private void setupResponse(ByteArrayOutputStream response, Call call, Status status, Writable rv, String errorClass,
