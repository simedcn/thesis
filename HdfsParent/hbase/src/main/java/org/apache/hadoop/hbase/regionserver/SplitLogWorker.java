...
public class SplitLogWorker extends ZooKeeperListener implements Runnable {
   ...
   public SplitLogWorker(ZooKeeperWatcher watcher, final Configuration conf, final String serverName) {
      this(watcher, conf, serverName, new TaskExecutor() {
         ...
         public Status exec(String filename, CancelableProgressable p) {
            ...
            try {
               if (HLogSplitter.splitLogFile(rootdir, fs.getFileStatus(new Path(filename)), fs, conf, p) == false) {
   ...
   public void run() {
      try {
         ...
         while (res == -1) {
            ...
            if (res == -1) {
               try {
                  LOG.info(watcher.splitLogZNode + " znode does not exist," + " waiting for master to create one");
                  Thread.sleep(1000);
   ...
   private void taskLoop() {
      while (true) {
         ...
         if (paths == null) {
            LOG.warn("Could not get tasks, did someone remove " + this.watcher.splitLogZNode
   ...
   private void grabTask(String path) {
      ...
      try {
         ...
         if (t > 0) {
            LOG.info("worker " + serverName + " done with task " + path + " in " + (System.currentTimeMillis() - t)
                  + "ms");
         }
         synchronized (grabTaskLock) {
   ...
   private boolean attemptToOwnTask(boolean isFirstTime) {
      try {
         Stat stat = this.watcher.getRecoverableZooKeeper().setData(currentTask, TaskState.TASK_OWNED.get(serverName),
               currentVersion);
         if (stat == null) {
         ...
         LOG.warn("Interrupted while trying to assert ownership of " + currentTask + " "
   ...
   void getDataSetWatchAsync() {
      this.watcher.getRecoverableZooKeeper().getZooKeeper()
            .getData(currentTask, this.watcher, new GetDataAsyncCallback(), null);
      ...
      synchronized (grabTaskLock) {
         if (workerInGrabTask) {
            ...
            if (taskpath != null && taskpath.equals(path)) {
               ...
               if (!TaskState.TASK_OWNED.equals(data, serverName) && !TaskState.TASK_DONE.equals(data, serverName)
                     && !TaskState.TASK_ERR.equals(data, serverName)
                     && !TaskState.TASK_RESIGNED.equals(data, serverName)) {
                  LOG.info("task " + taskpath + " preempted from " + serverName + ", current task state and owner="
                        + new String(data));
                  stopTask();
               }
            }
   ...
   private List<String> getTaskList() {
      ...
      while (!exitWorker) {
         ...
         try {
            LOG.debug("Retry listChildren of znode " + this.watcher.splitLogZNode + " after sleep for " + sleepTime
                  + "ms!");
