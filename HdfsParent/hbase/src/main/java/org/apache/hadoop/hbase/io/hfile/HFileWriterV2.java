...
public class HFileWriterV2 extends AbstractHFileWriter {
   ...
   static class WriterFactoryV2 extends HFile.WriterFactory {
      ...
      public Writer createWriter(FileSystem fs, Path path, FSDataOutputStream ostream, int blockSize,
            Compression.Algorithm compress, HFileDataBlockEncoder blockEncoder, final KeyComparator comparator,
            final ChecksumType checksumType, final int bytesPerChecksum) throws IOException {
         return new HFileWriterV2(conf, cacheConf, fs, path, ostream, blockSize, compress, blockEncoder, comparator,
   ...
   public HFileWriterV2(Configuration conf, CacheConfig cacheConf, FileSystem fs, Path path,
      ...
      super(cacheConf, ostream == null ? createOutputStream(conf, fs, path) : ostream, path, blockSize, compressAlgo,
   ...
   private void finishInit(final Configuration conf) {
      ...
      fsBlockWriter = new HFileBlock.Writer(compressAlgo, blockEncoder, includeMemstoreTS, checksumType,
            bytesPerChecksum);

      // Data block index writer
      boolean cacheIndexesOnWrite = cacheConf.shouldCacheIndexesOnWrite();
      dataBlockIndexWriter = new HFileBlockIndex.BlockIndexWriter(fsBlockWriter,
            cacheIndexesOnWrite ? cacheConf.getBlockCache() : null, cacheIndexesOnWrite ? name : null);
   ...
   private void doCacheOnWrite(long offset) {
      ...
      HFileBlock cacheFormatBlock = blockEncoder.diskToCacheFormat(fsBlockWriter.getBlockForCaching(), isCompaction);
      passSchemaMetricsTo(cacheFormatBlock);
      cacheConf.getBlockCache().cacheBlock(
            new BlockCacheKey(name, offset, blockEncoder.getEncodingInCache(), cacheFormatBlock.getBlockType()),
            cacheFormatBlock);
   }
   ...
   public void append(final KeyValue kv) throws IOException {
      append(kv.getMemstoreTS(), kv.getBuffer(), kv.getKeyOffset(), kv.getKeyLength(), kv.getBuffer(),
