...
public class HFilePrettyPrinter {
   ...
   public boolean parseOptions(String args[]) throws ParseException, IOException {
      ...
      if (cmd.hasOption("r")) {
         ...
         List<Path> regionFiles = HFile.getStoreFiles(FileSystem.get(conf), regionDir);
         if (verbose)
   ...
   private void processFile(Path file) throws IOException {
      ...
      if (verbose || printKey || checkRow || checkFamily || printStats) {
         ...
         if (this.isSeekToRow) {
            ...
            shouldScanKeysValues = (scanner.seekTo(KeyValue.createFirstOnRow(this.row).getKey()) != -1);
         } else {
   ...
   private void scanKeysValues(Path file, KeyValueStatsCollector fileStats, HFileScanner scanner, byte[] row)
         throws IOException {
      KeyValue pkv = null;
      do {
   ...
   private void printMeta(HFile.Reader reader, Map<byte[], byte[]> fileInfo) throws IOException {
      ...
      for (Map.Entry<byte[], byte[]> e : fileInfo.entrySet()) {
         ...
         if (Bytes.compareTo(e.getKey(), Bytes.toBytes("MAX_SEQ_ID_KEY")) == 0) {
            ...
            System.out
                  .println(timeRangeTracker.getMinimumTimestamp() + "...." + timeRangeTracker.getMaximumTimestamp());
   ...
   private static class LongStats {
      ...
      public String toString() {
         return "count: " + count + "\tmin: " + min + "\tmax: " + max + "\tmean: " + ((double) sum / count);
      }
   }
   ...
   private static class KeyValueStatsCollector {
      ...
      public void collect(KeyValue kv) {
         ...
         if (prevKV != null && KeyValue.COMPARATOR.compareRows(prevKV, kv) != 0) {
            // new row
            collectRow();
         }
