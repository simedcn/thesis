...
public class HFileBlockIndex {
   ...
   public static class BlockIndexReader implements HeapSize {
      ...
      public HFileBlock seekToDataBlock(final byte[] key, int keyOffset, int keyLength, HFileBlock currentBlock,
         ...
         BlockWithScanInfo blockWithScanInfo = loadDataBlockWithScanInfo(key, keyOffset, keyLength, currentBlock,
       ...
       * @throws IOException
       */
      public BlockWithScanInfo loadDataBlockWithScanInfo(final byte[] key, int keyOffset, int keyLength,
            HFileBlock currentBlock, boolean cacheBlocks, boolean pread, boolean isCompaction) throws IOException {
         ...
         while (true) {
            ...
            if (block == null) {
               throw new IOException("Failed to read block at offset " + currentOffset + ", onDiskSize="
                     + currentOnDiskSize);
            }
      ...
      public byte[] midkey() throws IOException {
         ...
         if (midLeafBlockOffset >= 0) {
            ...
            HFileBlock midLeafBlock = cachingBlockReader.readBlock(midLeafBlockOffset, midLeafBlockOnDiskSize, true,
                  true, false, BlockType.LEAF_INDEX);
            ...
            int keyOffset = b.arrayOffset() + Bytes.SIZEOF_INT * (numDataBlocks + 2) + keyRelOffset
       ...
       * @throws IOException
       */
      static int binarySearchNonRootIndex(byte[] key, int keyOffset, int keyLength, ByteBuffer nonRootIndex,
         ...
         while (low <= high) {
            ...
            // gives us the size of the whole (offset, onDiskSize, key) tuple. We
            // then need to subtract the overhead of offset and onDiskSize.
            int midLength = nonRootIndex.getInt(Bytes.SIZEOF_INT * (mid + 2)) - midKeyRelOffset
                  - SECONDARY_INDEX_ENTRY_OVERHEAD;

            // we have to compare in this order, because the comparator order
            // has special logic when the 'left side' is a special key.
            int cmp = comparator.compare(key, keyOffset, keyLength, nonRootIndex.array(), nonRootIndex.arrayOffset()
                  + midKeyOffset, midLength);
       ...
       *
       */
      static int locateNonRootIndexEntry(ByteBuffer nonRootBlock, byte[] key, int keyOffset, int keyLength,
            RawComparator<byte[]> comparator) {
         int entryIndex = binarySearchNonRootIndex(key, keyOffset, keyLength, nonRootBlock, comparator);
   ...
   public static class BlockIndexWriter extends SchemaConfigured implements InlineBlockWriter {
       ...
       */
      public BlockIndexWriter(HFileBlock.Writer blockWriter, BlockCache blockCache, String nameForCaching) {
      ...
      public long writeIndexBlocks(FSDataOutputStream out) throws IOException {
         ...
         if (LOG.isTraceEnabled()) {
            LOG.trace("Wrote a " + numLevels + "-level index with root level at pos " + rootLevelIndexPos + ", "
                  + rootChunk.getNumEntries() + " root-level entries, " + totalNumEntries + " total entries, "
      ...
      public void writeSingleLevelIndex(DataOutput out, String description) throws IOException {
         ...
         if (LOG.isTraceEnabled()) {
            LOG.trace("Wrote a single-level " + description + " index with " + rootChunk.getNumEntries() + " entries, "
                  + rootChunk.getRootSize() + " bytes");
         }
       ...
       * @throws IOException
       */
      private BlockIndexChunk writeIntermediateLevel(FSDataOutputStream out, BlockIndexChunk currentLevel)
            throws IOException {
         ...
         for (int i = 0; i < currentLevel.getNumEntries(); ++i) {
            curChunk
                  .add(currentLevel.getBlockKey(i), currentLevel.getBlockOffset(i), currentLevel.getOnDiskDataSize(i));
      ...
      private void writeIntermediateBlock(FSDataOutputStream out, BlockIndexChunk parent, BlockIndexChunk curChunk)
            throws IOException {
         ...
         if (blockCache != null) {
            ...
            blockCache.cacheBlock(new BlockCacheKey(nameForCaching, beginOffset, DataBlockEncoding.NONE,
      ...
      private void expectNumLevels(int expectedNumLevels) {
         if (numLevels != expectedNumLevels) {
            throw new IllegalStateException("Number of block index levels is " + numLevels + "but is expected to be "
                  + expectedNumLevels);
         }
      }
      ...
      public void blockWritten(long offset, int onDiskSize, int uncompressedSize) {
         ...
         if (firstKey == null) {
            throw new IllegalStateException("Trying to add second-level index " + "entry with offset=" + offset
                  + " and onDiskSize=" + onDiskSize + "but the first key was not set in writeInlineBlock");
         }
      ...
      public void ensureSingleLevel() throws IOException {
         if (numLevels > 1) {
            throw new IOException("Wrote a " + numLevels + "-level index with " + rootChunk.getNumEntries()
   ...
   static class BlockIndexChunk {
      ...
      void writeNonRoot(DataOutput out) throws IOException {
         ...
         if (secondaryIndexOffsetMarks.size() != blockKeys.size()) {
            throw new IOException("Corrupted block index chunk writer: " + blockKeys.size() + " entries but "
