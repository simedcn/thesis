...
public class Result implements Writable, WritableWithSize {
   ...
   protected int binarySearch(final KeyValue[] kvs, final byte[] family, final byte[] qualifier) {
   ...
   public NavigableMap<byte[], NavigableMap<byte[], NavigableMap<Long, byte[]>>> getMap() {
      ...
      for (KeyValue kv : this.kvs) {
         ...
         if (columnMap == null) {
            columnMap = new TreeMap<byte[], NavigableMap<Long, byte[]>>(Bytes.BYTES_COMPARATOR);
   ...
   public NavigableMap<byte[], NavigableMap<byte[], byte[]>> getNoVersionMap() {
      ...
      NavigableMap<byte[], NavigableMap<byte[], byte[]>> returnMap = new TreeMap<byte[], NavigableMap<byte[], byte[]>>(
   ...
   public static void writeArray(final DataOutput out, Result[] results) throws IOException {
   ...
   public static Result[] readArray(final DataInput in) throws IOException {
   ...
   public static void compareResults(Result res1, Result res2) throws Exception {
      ...
      if (res1.size() != res2.size()) {
         throw new Exception("This row doesn't have the same number of KVs: " + res1.toString() + " compared to "
               + res2.toString());
      }
