...
public class ServerName implements Comparable<ServerName> {
   ...
   public static Pattern SERVERNAME_PATTERN = Pattern.compile("[^" + SERVERNAME_SEPARATOR + "]+" + SERVERNAME_SEPARATOR
   ...
   public ServerName(final String serverName) {
      this(parseHostname(serverName), parsePort(serverName), parseStartcode(serverName));
   }
   ...
   public static String getServerName(final String hostAndPort, final long startcode) {
      ...
      return getServerName(hostAndPort.substring(0, index), Integer.parseInt(hostAndPort.substring(index + 1)),
            startcode);
   }
   ...
   public static ServerName parseServerName(final String str) {
      return SERVERNAME_PATTERN.matcher(str).matches() ? new ServerName(str) : new ServerName(str, NON_STARTCODE);
   }
}
