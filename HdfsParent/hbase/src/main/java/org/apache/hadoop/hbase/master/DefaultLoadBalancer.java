   ...
   */
   ...
   public List<RegionPlan> balanceCluster(Map<ServerName, List<HRegionInfo>> clusterState) {
      ...
      if (serversByLoad.lastKey().getLoad() <= ceiling && serversByLoad.firstKey().getLoad() >= floor) {
         ...
         LOG.info("Skipping load balancing because balanced cluster; " + "servers=" + numServers + " " + "regions="
               + numRegions + " average=" + average + " " + "mostloaded=" + serversByLoad.lastKey().getLoad()
   ...
   private List<ServerName> getTopBlockLocations(FileSystem fs, HRegionInfo region) {
      ...
      try {
         ...
         if (tableDescriptor != null) {
            HDFSBlocksDistribution blocksDistribution = HRegion.computeHDFSBlocksDistribution(config, tableDescriptor,
         ...
         LOG.debug(
               "IOException during HDFSBlocksDistribution computation. for " + "region = " + region.getEncodedName(),
               ioe);
      }
