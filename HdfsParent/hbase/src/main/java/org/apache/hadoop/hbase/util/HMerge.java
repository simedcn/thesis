...
class HMerge {
    ...
    * @throws IOException
    */
   public static void merge(Configuration conf, FileSystem fs, final byte[] tableName) throws IOException {
    ...
    * @throws IOException
    */
   public static void merge(Configuration conf, FileSystem fs, final byte[] tableName, final boolean testMasterRunning)
         throws IOException {
   ...
   private static abstract class Merger {
      ...
      protected boolean merge(final HRegionInfo[] info) throws IOException {
         ...
         for (int i = 0; i < info.length - 1; i++) {
            ...
            LOG.info("not merging regions " + Bytes.toStringBinary(currentRegion.getRegionName()) + " and "
      ...
      protected abstract void updateMeta(final byte[] oldRegion1, final byte[] oldRegion2, HRegion newRegion)
            throws IOException;
   ...
   private static class OnlineMerger extends Merger {
      ...
      @Override
      protected void updateMeta(final byte[] oldRegion1, final byte[] oldRegion2, HRegion newRegion) throws IOException {
   ...
   private static class OfflineMerger extends Merger {
      ...
      OfflineMerger(Configuration conf, FileSystem fs) throws IOException {
         ...
         Path rootTableDir = HTableDescriptor.getTableDir(fs.makeQualified(new Path(conf.get(HConstants.HBASE_DIR))),
