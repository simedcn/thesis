...
class GetClosestRowBeforeTracker {
    ...
    */
   GetClosestRowBeforeTracker(final KVComparator c, final KeyValue kv, final long ttl, final boolean metaregion) {
      super();
      ...
      int l = -1;
      if (metaregion) {
         l = KeyValue.getDelimiter(kv.getBuffer(), rowoffset, kv.getRowLength(), HRegionInfo.DELIMITER)
               - this.rowoffset;
      }
   ...
   boolean isBetterCandidate(final KeyValue contender) {
      return this.candidate == null
            || (this.kvcomparator.compareRows(this.candidate, contender) < 0 && this.kvcomparator.compareRows(
                  contender, this.targetkey) <= 0);
   }
   ...
   public boolean isDeleted(final KeyValue kv, final NavigableSet<KeyValue> ds) {
      ...
      for (KeyValue d : ds) {
         ...
         // Check column
         int ret = Bytes.compareTo(kv.getBuffer(), kv.getQualifierOffset(), kv.getQualifierLength(), d.getBuffer(),
               d.getQualifierOffset(), d.getQualifierLength());
         if (ret <= -1) {
   ...
   boolean isTargetTable(final KeyValue kv) {
      ...
      return Bytes.compareTo(this.targetkey.getBuffer(), this.rowoffset, this.tablenamePlusDelimiterLength,
