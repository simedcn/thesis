...
public class SchemaResource extends ResourceBase {
   ...
   private Response replace(final byte[] name, final TableSchemaModel model, final UriInfo uriInfo,
      ...
      try {
         ...
         if (admin.tableExists(name)) {
            ...
            try {
               ...
               return Response.status(Response.Status.NOT_MODIFIED).type(MIMETYPE_TEXT).entity("Not modified" + CRLF)
                     .build();
            }
   ...
   private Response update(final byte[] name, final TableSchemaModel model, final UriInfo uriInfo,
         final HBaseAdmin admin) {
      if (servlet.isReadOnly()) {
         return Response.status(Response.Status.FORBIDDEN).type(MIMETYPE_TEXT).entity("Forbidden" + CRLF).build();
      }
