...
public class ScanDeleteTracker implements DeleteTracker {
   ...
   public void add(byte[] buffer, int qualifierOffset, int qualifierLength, long timestamp, byte type) {
      if (!hasFamilyStamp || timestamp > familyStamp) {
         ...
         if (deleteBuffer != null && type < deleteType) {
            ...
            if (Bytes.equals(deleteBuffer, deleteOffset, deleteLength, buffer, qualifierOffset, qualifierLength)) {
               return;
            }
         }
    ...
    */
   @Override
   public DeleteResult isDeleted(byte[] buffer, int qualifierOffset, int qualifierLength, long timestamp) {
      if (hasFamilyStamp && timestamp <= familyStamp) {
         return DeleteResult.FAMILY_DELETED;
      }

      if (deleteBuffer != null) {
         int ret = Bytes.compareTo(deleteBuffer, deleteOffset, deleteLength, buffer, qualifierOffset, qualifierLength);
         ...
         if (ret == 0) {
            ...
            throw new IllegalStateException("isDelete failed: deleteBuffer="
                  ...
                  + Bytes.toStringBinary(buffer, qualifierOffset, qualifierLength) + ", timestamp=" + timestamp
                  + ", comparison result: " + ret);
         }
      }
