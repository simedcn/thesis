...
public class HBaseFsck extends Configured implements Tool {
   ...
   public void offlineHdfsIntegrityRepair() throws IOException, InterruptedException {
      ...
      if (shouldCheckHdfs()
            && (shouldFixHdfsOrphans() || shouldFixHdfsHoles() || shouldFixHdfsOverlaps() || shouldFixTableOrphans())) {
   ...
   private void adoptHdfsOrphan(HbckInfo hi) throws IOException {
      ...
      if (dirs == null) {
         LOG.warn("Attempt to adopt ophan hdfs region skipped becuase no files present in " + p
      ...
      for (FileStatus cf : dirs) {
         ...
         if (cfName.startsWith(".") || cfName.equals("splitlog"))
            continue;
   ...
   private SortedMap<String, TableInfo> loadHdfsRegionInfos() throws IOException, InterruptedException {
      ...
      for (HbckInfo hbi : hbckInfos) {
         ...
         if (modTInfo == null) {
            ...
            try {
               HTableDescriptor htd = FSTableDescriptors.getTableDescriptor(hbaseRoot.getFileSystem(getConf()),
                     hbaseRoot, tableName);
               ...
               if (!orphanTableDirs.containsKey(tableName)) {
                  ...
                  errors.reportError(ERROR_CODE.NO_TABLEINFO_FILE, "Unable to read .tableinfo from " + hbaseRoot + "/"
                        + tableName);
   ...
   void sidelineTable(FileSystem fs, byte[] table, Path hbaseDir, Path backupHbaseDir) throws IOException {
      ...
      if (fs.exists(tableDir)) {
         ...
         if (!success) {
            throw new IOException("Failed to move  " + tableName + " from " + tableDir.getName() + " to "
                  + backupTableDir.getName());
         }
      } else {
      ...
      try {
         ...
         try {
            ...
            LOG.fatal("... failed to sideline root and meta and failed to restore "
                  + "prevoius state.  Currently in inconsistent state.  To restore " + "try to rename -ROOT- in "
                  + backupDir.getName() + " to " + hbaseDir.getName() + ".", ioe);
         }
   ...
   public void loadHdfsRegionDirs() throws IOException, InterruptedException {
      ...
      if (!foundVersionFile) {
         ...
         if (shouldFixVersionFile()) {
            ...
            setShouldRerun();
            FSUtils.setVersion(fs, rootDir, getConf().getInt(HConstants.THREAD_WAKE_FREQUENCY, 10 * 1000), getConf()
   ...
   private ServerName getRootRegionServerName() throws IOException, InterruptedException {
      RootRegionTracker rootRegionTracker = new RootRegionTracker(this.connection.getZooKeeperWatcher(),
            new Abortable() {
               @Override
   ...
   private void offline(byte[] regionName) throws IOException {
      ...
      if (!rsSupportsOffline) {
         LOG.warn("Using unassign region " + regionString + " instead of using offline method, you should"
   ...
   private void closeRegion(HbckInfo hi) throws IOException, InterruptedException {
      ...
      if (value == null || startcodeBytes == null) {
         errors.reportError("Unable to close region " + hi.getRegionNameAsString() + " because meta does not "
      ...
      if (hri == null) {
         LOG.warn("Unable to close region " + hi.getRegionNameAsString() + " because META had invalid or missing "
               + HConstants.CATALOG_FAMILY_STR + ":" + Bytes.toString(HConstants.REGIONINFO_QUALIFIER)
               + " qualifier value.");
         return;
      }
    ...
    */
   private void checkRegionConsistency(final String key, final HbckInfo hbi) throws IOException, KeeperException,
      ...
      // ========== Cases where the region is not in META =============
         ...
         errors.reportError(ERROR_CODE.NOT_IN_META_HDFS, "Region " + descriptiveName + ", key=" + key
               + ", not on HDFS or in META but " + "deployed on " + Joiner.on(", ").join(hbi.deployedOn));
         if (shouldFixAssignments()) {
            undeployRegions(hbi);
         }
         ...
         errors.reportError(ERROR_CODE.NOT_IN_META, "Region " + descriptiveName + " not in META, but deployed on "
         ...
         errors.reportError(ERROR_CODE.LINGERING_SPLIT_PARENT, "Region " + descriptiveName
         ...
         errors.reportError(ERROR_CODE.NOT_IN_HDFS, "Region " + descriptiveName + " found in META, but not in HDFS, "
         ...
         errors.reportError(ERROR_CODE.SHOULD_NOT_BE_DEPLOYED,
               "Region " + descriptiveName + " should not be deployed according " + "to META, but is deployed on "
                     + Joiner.on(", ").join(hbi.deployedOn));
         if (shouldFixAssignments()) {
         ...
         errors.reportError(ERROR_CODE.MULTI_DEPLOYED, "Region " + descriptiveName
               ...
               + " but is multiply assigned to region servers " + Joiner.on(", ").join(hbi.deployedOn));
         // If we are trying to fix the errors
         if (shouldFixAssignments()) {
         ...
         errors.reportError(ERROR_CODE.SERVER_DOES_NOT_MATCH_META, "Region " + descriptiveName
               + " listed in META on region server " + hbi.metaEntry.regionServer + " but found on region server "
         ...
         errors.reportError(ERROR_CODE.UNKNOWN, "Region " + descriptiveName + " is in an unforeseen state:"
               + " inMeta=" + inMeta + " inHdfs=" + inHdfs + " isDeployed=" + isDeployed + " isMultiplyDeployed="
   ...
   public class TableInfo {
      ...
      public void addRegionInfo(HbckInfo hir) {
         ...
         if (Bytes.compareTo(hir.getStartKey(), hir.getEndKey()) > 0) {
            errors.reportError(ERROR_CODE.REGION_CYCLE, String.format("The endkey for this region comes before the "
                  + "startkey, startkey=%s, endkey=%s", Bytes.toStringBinary(hir.getStartKey()),
      ...
      private class IntegrityFixSuggester extends TableIntegrityErrorHandlerImpl {
         ...
         public void handleRegionStartKeyNotEmpty(HbckInfo hi) throws IOException {
            errors.reportError(ERROR_CODE.FIRST_REGION_STARTKEY_NOT_EMPTY,
                  "First region should start with an empty key.  You need to "
                        + " create a new region and regioninfo in HDFS to plug the hole.", getTableInfo(), hi);
         }
         ...
         public void handleDegenerateRegion(HbckInfo hi) throws IOException {
            errors.reportError(ERROR_CODE.DEGENERATE_REGION, "Region has the same start and end key.", getTableInfo(),
                  hi);
         }
         ...
         public void handleDuplicateStartKeys(HbckInfo r1, HbckInfo r2) throws IOException {
            ...
            errors.reportError(ERROR_CODE.DUPE_STARTKEYS,
                  "Multiple regions have the same startkey: " + Bytes.toStringBinary(key), getTableInfo(), r1);
            errors.reportError(ERROR_CODE.DUPE_STARTKEYS,
                  "Multiple regions have the same startkey: " + Bytes.toStringBinary(key), getTableInfo(), r2);
         }

         @Override
         public void handleOverlapInRegionChain(HbckInfo hi1, HbckInfo hi2) throws IOException {
            errors.reportError(ERROR_CODE.OVERLAP_IN_REGION_CHAIN, "There is an overlap in the region chain.",
      ...
      private class HDFSIntegrityFixer extends IntegrityFixSuggester {
         ...
         HDFSIntegrityFixer(TableInfo ti, ErrorReporter errors, Configuration conf, boolean fixHoles,
               boolean fixOverlaps) {
            super(ti, errors);
            this.conf = conf;
         ...
         public void handleRegionStartKeyNotEmpty(HbckInfo next) throws IOException {
            errors.reportError(ERROR_CODE.FIRST_REGION_STARTKEY_NOT_EMPTY,
                  "First region should start with an empty key.  Creating a new "
                        + "region and regioninfo in HDFS to plug the hole.", getTableInfo(), next);
            HTableDescriptor htd = getTableInfo().getHTD();
            // from special EMPTY_START_ROW to next region's startKey
            HRegionInfo newRegion = new HRegionInfo(htd.getName(), HConstants.EMPTY_START_ROW, next.getStartKey());
         ...
         public void handleHoleInRegionChain(byte[] holeStartKey, byte[] holeStopKey) throws IOException {
            errors.reportError(
                  ERROR_CODE.HOLE_IN_REGION_CHAIN,
                  "There is a hole in the region chain between " + Bytes.toStringBinary(holeStartKey) + " and "
                        + Bytes.toStringBinary(holeStopKey) + ".  Creating a new regioninfo and region "
                        + "dir in hdfs to plug the hole.");
            HTableDescriptor htd = getTableInfo().getHTD();
         ...
         public void handleOverlapGroup(Collection<HbckInfo> overlap) throws IOException {
            ...
            if (overlap.size() > maxMerge) {
               LOG.warn("Overlap group has " + overlap.size() + " overlapping " + "regions which is greater than "
                     + maxMerge + ", the max number of regions to merge");
         ...
         void mergeOverlaps(Collection<HbckInfo> overlap) throws IOException {
            ...
            for (HbckInfo hi : overlap) {
               ...
               try {
                  ...
                  LOG.warn("Was unable to close region " + hi + ".  Just continuing... ", ioe);
               } catch (InterruptedException e) {
                  LOG.warn("Was unable to close region " + hi + ".  Just continuing... ", e);
               }
               ...
               try {
                  ...
                  LOG.warn("Unable to offline region from master: " + hi + ".  Just continuing... ", ioe);
               }
            }
            ...
            HRegion region = HBaseFsckRepair.createHDFSRegionDir(conf, newRegion, htd);
            LOG.info("Created new empty container region: " + newRegion + " to contain regions: "
         ...
         void sidelineBigOverlaps(Collection<HbckInfo> bigOverlap) throws IOException {
            ...
            for (HbckInfo regionToSideline : regionsToSideline) {
               ...
               if (sidelineRegionDir != null) {
                  ...
                  LOG.info("After sidelined big overlapped region: " + regionToSideline.getRegionNameAsString()
      ...
      public boolean checkRegionChain(TableIntegrityErrorHandler handler) throws IOException {
         ...
         if (details) {
            ...
            System.out.println("There are " + overlapGroups.keySet().size() + " overlap groups with "
   ...
   public void dumpSidelinedRegions(Map<Path, HbckInfo> regions) {
      for (Map.Entry<Path, HbckInfo> entry : regions.entrySet()) {
         ...
         System.out.println("This sidelined region dir should be bulk loaded: " + path.toString());
         System.out.println("Bulk load command looks like: "
               + "hbase org.apache.hadoop.hbase.mapreduce.LoadIncrementalHFiles " + path.toUri().getPath() + " "
               + tableName);
      }
   }
   ...
   boolean checkMetaRegion() throws IOException, KeeperException, InterruptedException {
      ...
      if (metaRegions.size() != 1) {
         HRegionLocation rootLocation = connection.locateRegion(HConstants.ROOT_TABLE_NAME, HConstants.EMPTY_START_ROW);
         HbckInfo root = regionInfoMap.get(rootLocation.getRegionInfo().getEncodedName());
   ...
   boolean loadMetaEntries() throws IOException {
      ...
      MetaScanner.metaScan(getConf(), visitor, null, null, Integer.MAX_VALUE, HConstants.ROOT_TABLE_NAME);
   ...
   private static ErrorReporter getErrorReporter(final Configuration conf) throws ClassNotFoundException {
      Class<? extends ErrorReporter> reporter = conf.getClass("hbasefsck.errorreporter", PrintingErrorReporter.class,
            ErrorReporter.class);
   ...
   static class PrintingErrorReporter implements ErrorReporter {
      ...
      public synchronized void reportError(ERROR_CODE errorCode, String message, TableInfo table, HbckInfo info) {
         errorTables.add(table);
         String reference = "(region " + info.getRegionNameAsString() + ")";
         reportError(errorCode, reference + " " + message);
      }

      public synchronized void reportError(ERROR_CODE errorCode, String message, TableInfo table, HbckInfo info1,
            HbckInfo info2) {
         errorTables.add(table);
   ...
   static class WorkItemRegion implements Callable<Void> {
      ...
      public synchronized Void call() throws IOException {
         errors.progress();
         try {
            HRegionInterface server = connection.getHRegionConnection(rsinfo.getHostname(), rsinfo.getPort());
      ...
      private List<HRegionInfo> filterRegions(List<HRegionInfo> regions) {
         ...
         for (HRegionInfo hri : regions) {
            if (hri.isMetaTable() || (!hbck.checkMetaOnly && hbck.isTableIncluded(hri.getTableNameAsString()))) {
               ret.add(hri);
            }
         }
         return ret;
         ...
         if (hbi.getHdfsHRI() == null) {
            try {
               ...
               String msg = "Orphan region in HDFS: Unable to load .regioninfo from table "
                     + Bytes.toString(hbi.getTableName()) + " in hdfs dir " + hbi.getHdfsRegionDir()
   ...
   protected HBaseFsck printUsageAndExit() {
      ...
      System.err.println("   -maxMerge <n>     When fixing region overlaps, allow at most <n> regions to merge. (n="
      ...
      System.err
            .println("   -maxOverlapsToSideline <n>  When fixing region overlaps, allow at most <n> regions to sideline per group. (n="
