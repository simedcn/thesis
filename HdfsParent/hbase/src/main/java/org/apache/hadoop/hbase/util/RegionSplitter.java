...
public class RegionSplitter {
   ...
   static void createPresplitTable(String tableName, SplitAlgorithm splitAlgo, String[] columnFamilies,
   ...
   static void rollingSplit(String tableName, SplitAlgorithm splitAlgo, Configuration conf) throws IOException,
            ...
            for (String rsLoc : serversLeft) {
               ...
               while (!regionList.isEmpty()) {
                  ...
                  if (sk.length != 0) {
                     ...
                     Preconditions.checkArgument(Bytes.equals(start, sk), splitAlgo.rowToStr(start) + " != "
                           + splitAlgo.rowToStr(sk));
                  }
   ...
   static LinkedList<Pair<byte[], byte[]>> getSplits(HTable table, SplitAlgorithm splitAlgo) throws IOException {
      ...
      if (!fs.exists(splitFile)) {
         ...
         LOG.debug("Table " + Bytes.toString(table.getTableName()) + " has " + rows.size()
   ...
   public static class HexStringSplit implements SplitAlgorithm {
      ...
      public String toString() {
         return this.getClass().getSimpleName() + " [" + rowToStr(firstRow()) + "," + rowToStr(lastRow()) + "]";
      }
   }
