...
public abstract class TableEventHandler extends EventHandler {
   ...
   public TableEventHandler(EventType eventType, byte[] tableName, Server server, MasterServices masterServices)
         throws IOException {
   ...
   public void process() {
      try {
         ...
         if (eventType.isOnlineSchemaChangeSupported()
               && this.masterServices.getAssignmentManager().getZKTable().isEnabledTable(Bytes.toString(tableName))) {
    ...
    * @throws IOException
    */
   HTableDescriptor getTableDescriptor() throws FileNotFoundException, IOException {
   ...
   byte[] hasColumnFamily(final HTableDescriptor htd, final byte[] cf) throws InvalidFamilyOperationException {
      if (!htd.hasFamily(cf)) {
   ...
   protected abstract void handleTableOperation(List<HRegionInfo> regions) throws IOException, KeeperException;
}
