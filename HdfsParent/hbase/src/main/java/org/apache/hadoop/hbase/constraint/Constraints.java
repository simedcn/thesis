...
public final class Constraints {
   ...
   private static final Pattern CONSTRAINT_HTD_ATTR_KEY_PATTERN = Pattern.compile(CONSTRAINT_HTD_KEY_PREFIX,
         Pattern.LITERAL);
    ...
    */
   public static void add(HTableDescriptor desc, Pair<Class<? extends Constraint>, Configuration>... constraints)
         throws IOException {
      enable(desc);
      long priority = getNextPriority(desc);
    ...
    *           wrong and that all constraints need to be enforced.
    */
   public static void add(HTableDescriptor desc, Class<? extends Constraint> constraint, Configuration conf)
         throws IOException {
      enable(desc);
      long priority = getNextPriority(desc);
    ...
    */
   private static void addConstraint(HTableDescriptor desc, Class<? extends Constraint> clazz, Configuration conf,
         long priority) throws IOException {
      writeConstraint(desc, serializeConstraintClass(clazz), configure(conf, true, priority));
   }
    ...
    * disabled.
    */
   private static void changeConstraintEnabled(HTableDescriptor desc, Class<? extends Constraint> clazz, boolean enabled)
         throws IOException {
   ...
   private static final Comparator<Constraint> constraintComparator = new Comparator<Constraint>() {
      ...
      public int compare(Constraint c1, Constraint c2) {
         ...
         return Long.valueOf(c1.getConf().getLong(PRIORITY_KEY, DEFAULT_PRIORITY)).compareTo(
