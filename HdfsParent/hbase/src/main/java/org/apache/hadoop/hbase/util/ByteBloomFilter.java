...
public class ByteBloomFilter implements BloomFilter, BloomFilterWriter {
   ...
   private static final byte[] bitvals = { (byte) 0x01, (byte) 0x02, (byte) 0x04, (byte) 0x08, (byte) 0x10,
   ...
   public static int computeFoldableByteSize(long bitSize, int foldFactor) {
      ...
      if (byteSizeLong > Integer.MAX_VALUE) {
         throw new IllegalArgumentException("byteSize=" + byteSizeLong + " too " + "large for bitSize=" + bitSize
   ...
   public boolean contains(byte[] buf, int offset, int length, ByteBuffer theBloom) {
      ...
      if (theBloom.limit() != byteSize) {
         throw new IllegalArgumentException("Bloom does not match expected size:" + " theBloom.limit()="
               + theBloom.limit() + ", byteSize=" + byteSize);
      }

      return contains(buf, offset, length, theBloom.array(), theBloom.arrayOffset(), (int) byteSize, hash, hashCount);
   }

   public static boolean contains(byte[] buf, int offset, int length, byte[] bloomArray, int bloomOffset,
         int bloomSize, Hash hash, int hashCount) {
