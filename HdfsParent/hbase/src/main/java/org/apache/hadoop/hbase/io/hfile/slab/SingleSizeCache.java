...
public class SingleSizeCache implements BlockCache, HeapSize {
   ...
   public final static long CACHE_FIXED_OVERHEAD = ClassSize.align((2 * Bytes.SIZEOF_INT) + (5 * ClassSize.REFERENCE)
         + +ClassSize.OBJECT);
   ...
   public SingleSizeCache(int blockSize, int numBlocks, SlabItemActionWatcher master) {
      ...
      backingMap = CacheBuilder.newBuilder().maximumSize(numBlocks - 1).removalListener(listener)
            .<BlockCacheKey, CacheablePair> build().asMap();
   ...
   public void logStats() {
      ...
      LOG.info("For Slab of size " + this.blockSize + ": " + this.getOccupiedSize() / this.blockSize
            + " occupied, out of a capacity of " + this.numBlocks + " blocks. HeapSize is "
            + StringUtils.humanReadableInt(this.heapSize()) + " bytes." + ", " + "churnTime="
