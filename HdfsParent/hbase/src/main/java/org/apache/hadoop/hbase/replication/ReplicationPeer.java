...
public class ReplicationPeer implements Abortable {
   ...
   public void startStateTracker(ZooKeeperWatcher zookeeper, String peerStateNode) throws KeeperException {
      if (ZKUtil.checkExists(zookeeper, peerStateNode) == -1) {
         ...
         ZKUtil.createNodeIfNotExistsAndWatch(zookeeper, peerStateNode, Bytes.toBytes(PeerState.ENABLED.name()));
      }
   ...
   private void readPeerStateZnode() {
      ...
      this.peerEnabled.set(PeerState.ENABLED.equals(PeerState.valueOf(currentState)));
   }
   ...
   public void abort(String why, Throwable e) {
      LOG.fatal("The ReplicationPeer coresponding to peer " + clusterKey + " was aborted for the following reason(s):"
            + why, e);
   }
   ...
   public void reloadZkWatcher() throws IOException {
      if (zkw != null)
         zkw.close();
      zkw = new ZooKeeperWatcher(conf, "connection to cluster: " + id, this);
   }
