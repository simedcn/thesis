...
public class RegionServerDynamicMetrics implements Updater {
   ...
   private RegionServerDynamicMetrics(HRegionServer regionServer) {
      ...
      this.metricsRecord = MetricsUtil.createRecord(this.context, "RegionServerDynamicStatistics");
      context.registerUpdater(this);
      this.rsDynamicStatistics = new RegionServerDynamicStatistics(this.registry);
      this.regionServer = regionServer;
      try {
         updateMbeanInfoIfMetricsListChanged = this.rsDynamicStatistics.getClass().getSuperclass()
               .getDeclaredMethod("updateMbeanInfoIfMetricsListChanged", new Class[] {});
   ...
   public static RegionServerDynamicMetrics newInstance(HRegionServer regionServer) {
      RegionServerDynamicMetrics metrics = new RegionServerDynamicMetrics(regionServer);
      return metrics;
   }
   ...
   public void doUpdates(MetricsContext context) {
      ...
      for (Entry<String, Pair<AtomicLong, AtomicInteger>> entry : RegionMetricsStorage.getTimeVaryingMetrics()
            .entrySet()) {
      ...
      if (needsUpdateMessage) {
         try {
            if (updateMbeanInfoIfMetricsListChanged != null) {
               updateMbeanInfoIfMetricsListChanged.invoke(this.rsDynamicStatistics, new Object[] {});
            }
         } catch (Exception e) {
            LOG.error(e);
         }
