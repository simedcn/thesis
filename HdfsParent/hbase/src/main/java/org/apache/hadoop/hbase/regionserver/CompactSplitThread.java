...
public class CompactSplitThread implements CompactionRequestor {
   ...
   CompactSplitThread(HRegionServer server) {
      ...
      this.largeCompactions = new ThreadPoolExecutor(largeThreads, largeThreads, 60, TimeUnit.SECONDS,
      ...
      this.smallCompactions = new ThreadPoolExecutor(smallThreads, smallThreads, 60, TimeUnit.SECONDS,
            new PriorityBlockingQueue<Runnable>(), new ThreadFactory() {
               @Override
               public Thread newThread(Runnable r) {
                  Thread t = new Thread(r);
   ...
   public String toString() {
      return "compaction_queue=(" + largeCompactions.getQueue().size() + ":" + smallCompactions.getQueue().size() + ")"
   ...
   public synchronized void requestSplit(final HRegion r, byte[] midKey) {
      if (midKey == null) {
         LOG.debug("Region " + r.getRegionNameAsString() + " not splittable because midkey=null");
         return;
      }
      try {
   ...
   public synchronized void requestCompaction(final HRegion r, final String why) throws IOException {
      for (Store s : r.getStores().values()) {
         requestCompaction(r, s, why, Store.NO_PRIORITY);
      }
   }

   public synchronized void requestCompaction(final HRegion r, final Store s, final String why) throws IOException {
      requestCompaction(r, s, why, Store.NO_PRIORITY);
   }

   public synchronized void requestCompaction(final HRegion r, final String why, int p) throws IOException {
      for (Store s : r.getStores().values()) {
    ...
    */
   public synchronized void requestCompaction(final HRegion r, final Store s, final String why, int priority)
         throws IOException {
      ...
      if (cr != null) {
         ...
         if (LOG.isDebugEnabled()) {
            ...
            LOG.debug(type + "Compaction requested: " + cr + (why != null && !why.isEmpty() ? "; Because: " + why : "")
                  + "; " + this);
         }
      } else {
         if (LOG.isDebugEnabled()) {
