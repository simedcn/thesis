...
public class HFileDataBlockEncoderImpl implements HFileDataBlockEncoder {
   ...
   public HFileDataBlockEncoderImpl(DataBlockEncoding onDisk, DataBlockEncoding inCache) {
      ...
      Preconditions.checkArgument(onDisk == DataBlockEncoding.NONE || onDisk == inCache, "on-disk encoding (" + onDisk
            + ") must be " + "either the same as in-cache encoding (" + inCache + ") or " + DataBlockEncoding.NONE);
   }

   public static HFileDataBlockEncoder createFromFileInfo(FileInfo fileInfo, DataBlockEncoding preferredEncodingInCache)
         throws IOException {
   ...
   public HFileBlock diskToCacheFormat(HFileBlock block, boolean isCompaction) {
      ...
      if (block.getBlockType() == BlockType.ENCODED_DATA) {
         ...
         // have to do this.
         throw new AssertionError("Expected on-disk data block encoding " + onDisk + ", got "
               + block.getDataBlockEncoding());
      }
      return block;
   }
   ...
   public Pair<ByteBuffer, BlockType> beforeWriteToDisk(ByteBuffer in, boolean includesMemstoreTS, byte[] dummyHeader) {
      ...
      ByteBuffer encodedBuffer = encodeBufferToHFileBlockBuffer(in, onDisk, includesMemstoreTS, dummyHeader);
   ...
   private HFileBlock encodeDataBlock(HFileBlock block, DataBlockEncoding algo, boolean includesMemstoreTS) {
      ByteBuffer compressedBuffer = encodeBufferToHFileBlockBuffer(block.getBufferWithoutHeader(), algo,
            includesMemstoreTS, block.getDummyHeaderForVersion());
      int sizeWithoutHeader = compressedBuffer.limit() - block.headerSize();
      HFileBlock encodedBlock = new HFileBlock(BlockType.ENCODED_DATA, block.getOnDiskSizeWithoutHeader(),
            sizeWithoutHeader, block.getPrevBlockOffset(), compressedBuffer, HFileBlock.FILL_HEADER, block.getOffset(),
            includesMemstoreTS, block.getMinorVersion(), block.getBytesPerChecksum(), block.getChecksumType(),
   ...
   public String toString() {
      return getClass().getSimpleName() + "(onDisk=" + onDisk + ", inCache=" + inCache + ")";
   }
