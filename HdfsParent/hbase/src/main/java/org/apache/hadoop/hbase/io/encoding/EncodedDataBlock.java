...
public class EncodedDataBlock {
   ...
   public void addKv(KeyValue kv) {
      ...
      uncompressedOutputStream.write(kv.getBuffer(), kv.getOffset(), kv.getLength());
   }
   ...
   public Iterator<KeyValue> getIterator() {
      ...
      final ByteArrayInputStream bais = new ByteArrayInputStream(getCompressedData());
      ...
      return new Iterator<KeyValue>() {
         ...
         public KeyValue next() {
            if (decompressedData == null) {
               try {
                  decompressedData = dataBlockEncoder.uncompressKeyValues(dis, includesMemstoreTS);
               } catch (IOException e) {
   ...
   public static int checkCompressedSize(Compressor compressor, byte[] buffer, int offset, int length) {
      ...
      while (!compressor.finished()) {
         try {
            ...
            throw new RuntimeException("For some reason compressor couldn't read data. "
                  + "It is likely a problem with " + compressor.getClass().getName(), e);
         }
      }
      return currentPos;
   }
   ...
   public int checkCompressedSize(Compressor compressor) {
      ...
      return checkCompressedSize(compressor, compressedBytes, 0, compressedBytes.length);
   }
   ...
   private ByteBuffer getUncompressedBuffer() {
      if (uncompressedBuffer == null || uncompressedBuffer.limit() < uncompressedOutputStream.size()) {
         uncompressedBuffer = ByteBuffer.wrap(uncompressedOutputStream.toByteArray());
      }
   ...
   public byte[] doCompressData() {
      ...
      try {
         ...
         throw new RuntimeException(String.format("Bug in decoding part of algorithm %s. "
