...
public class HFileArchiver {
    ...
    * @throws IOException
    */
   public static void archiveRegion(FileSystem fs, HRegionInfo info) throws IOException {
   ...
   public static boolean archiveRegion(FileSystem fs, Path rootdir, Path tableDir, Path regionDir) throws IOException {
      ...
      if (tableDir == null || regionDir == null) {
         LOG.error("No archive directory could be found because tabledir (" + tableDir + ") or regiondir (" + regionDir
               + "was null. Deleting files instead.");
    ...
    */
   public static void archiveFamily(FileSystem fs, Configuration conf, HRegionInfo parent, Path tableDir, byte[] family)
         throws IOException {
      ...
      if (storeFiles == null) {
         LOG.debug("No store files to dispose for region=" + parent.getRegionNameAsString() + ", family="
    ...
    * @throws IOException if the files could not be correctly disposed.
    */
   public static void archiveStoreFiles(FileSystem fs, HRegion parent, Configuration conf, byte[] family,
      ...
      if (parent == null || family == null)
         throw new IOException("Need to have a parent region and a family to archive from.");
    ...
    */
   private static boolean resolveAndArchive(FileSystem fs, Path baseArchiveDir, Collection<File> toArchive)
         throws IOException {
    ...
    */
   private static List<File> resolveAndArchive(FileSystem fs, Path baseArchiveDir, Collection<File> toArchive,
    ...
    */
   private static boolean resolveAndArchiveFile(Path archiveDir, File currentFile, String archiveStartTime)
         throws IOException {
