...
public class ZooKeeperScanPolicyObserver extends BaseRegionObserver {
   ...
   public void start(CoprocessorEnvironment e) throws IOException {
      ...
      if (!re.getSharedData().containsKey(zkkey)) {
         ...
         re.getSharedData().putIfAbsent(zkkey,
               new ZKWatcher(re.getRegionServerServices().getZooKeeper().getRecoverableZooKeeper().getZooKeeper()));
      }
   }
   ...
   @Override
   public InternalScanner preFlushScannerOpen(final ObserverContext<RegionCoprocessorEnvironment> c, Store store,
   ...
   @Override
   public InternalScanner preCompactScannerOpen(final ObserverContext<RegionCoprocessorEnvironment> c, Store store,
      ...
      Scan scan = new Scan();
      scan.setMaxVersions(scanInfo.getMaxVersions());
      return new StoreScanner(store, scanInfo, scan, scanners, scanType, store.getHRegion().getSmallestReadPoint(),
            earliestPutTs);
   }
