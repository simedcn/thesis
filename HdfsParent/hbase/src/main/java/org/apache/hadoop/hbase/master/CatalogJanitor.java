...
class CatalogJanitor extends Chore {
   ...
   CatalogJanitor(final Server server, final MasterServices services) {
      super(server.getServerName() + "-CatalogJanitor", server.getConfiguration().getInt(
            "hbase.catalogjanitor.interval", 300000), server);
      this.server = server;
   ...
   int scan() throws IOException {
      ...
      for (Map.Entry<HRegionInfo, Result> e : splitParents.entrySet()) {
         if (!parentNotCleaned.contains(e.getKey().getEncodedName()) && cleanParent(e.getKey(), e.getValue())) {
            ...
            parentNotCleaned.add(getDaughterRegionInfo(e.getValue(), HConstants.SPLITA_QUALIFIER).getEncodedName());
            parentNotCleaned.add(getDaughterRegionInfo(e.getValue(), HConstants.SPLITB_QUALIFIER).getEncodedName());
         }
      }
      if (cleaned != 0) {
    ...
    * @throws IOException
    */
   static HRegionInfo getHRegionInfo(final Result result) throws IOException {
    ...
    * @throws IOException
    */
   private HRegionInfo getDaughterRegionInfo(final Result result, final byte[] which) throws IOException {
    ...
    * @throws IOException
    */
   private void removeDaughtersFromParent(final HRegionInfo parent) throws IOException {
    ...
    * @throws IOException
    */
   Pair<Boolean, Boolean> checkDaughterInFs(final HRegionInfo parent, final HRegionInfo split, final byte[] qualifier)
         throws IOException {
