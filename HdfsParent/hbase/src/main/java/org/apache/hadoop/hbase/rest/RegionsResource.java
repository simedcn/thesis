...
public class RegionsResource extends ResourceBase {
   ...
   public Response get(final @Context UriInfo uriInfo) {
      ...
      try {
         ...
         Map<HRegionInfo, ServerName> regions = MetaScanner.allTableRegions(servlet.getConfiguration(),
               Bytes.toBytes(tableName), false);
         for (Map.Entry<HRegionInfo, ServerName> e : regions.entrySet()) {
            HRegionInfo hri = e.getKey();
            ServerName addr = e.getValue();
            model.add(new TableRegionModel(tableName, hri.getRegionId(), hri.getStartKey(), hri.getEndKey(), addr
                  .getHostAndPort()));
         }
