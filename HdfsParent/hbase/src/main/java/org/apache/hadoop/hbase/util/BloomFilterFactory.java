...
public final class BloomFilterFactory {
    ...
    */
   public static BloomFilter createFromMeta(DataInput meta, HFile.Reader reader) throws IllegalArgumentException,
         IOException {
   ...
   */
    ...
    */
   public static BloomFilterWriter createGeneralBloomAtWrite(Configuration conf, CacheConfig cacheConf,
    ...
    *         or when failed to create one.
    */
   public static BloomFilterWriter createDeleteBloomAtWrite(Configuration conf, CacheConfig cacheConf, int maxKeys,
         HFile.Writer writer) {
      ...
      if (HFile.getFormatVersion(conf) > HFile.MIN_FORMAT_VERSION) {
         ...
         // In case of compound Bloom filters we ignore the maxKeys hint.
         CompoundBloomFilterWriter bloomWriter = new CompoundBloomFilterWriter(getBloomBlockSize(conf), err,
               Hash.getHashType(conf), maxFold, cacheConf.shouldCacheBloomsOnWrite(), Bytes.BYTES_RAWCOMPARATOR);
         writer.addInlineBlockWriter(bloomWriter);
         return bloomWriter;
      } else {
