...
public class ClassSize {
   ...
   static {
      ...
      CONCURRENT_HASHMAP = align((2 * Bytes.SIZEOF_INT) + ARRAY + (6 * REFERENCE) + OBJECT);
      ...
      if (debug) {
         if (LOG.isDebugEnabled()) {
            ...
            LOG.debug("Primitives=" + coeff[0] + ", arrays=" + coeff[1] + ", references(includes " + nrOfRefsPerObj
                  + " for object overhead)=" + coeff[2] + ", refSize " + REFERENCE + ", size=" + size
                  + ", prealign_size=" + prealign_size);
         }
      }
      return size;
   }
