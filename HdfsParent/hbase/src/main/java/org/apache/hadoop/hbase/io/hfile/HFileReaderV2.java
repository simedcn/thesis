...
public class HFileReaderV2 extends AbstractHFileReader {
    ...
    */
   public HFileReaderV2(Path path, FixedFileTrailer trailer, final FSDataInputStream fsdis,
         final FSDataInputStream fsdisNoFsChecksum, final long size, final boolean closeIStream,
         final CacheConfig cacheConf, DataBlockEncoding preferredEncodingInCache, final HFileSystem hfs)
         throws IOException {
      ...
      HFileBlock.FSReaderV2 fsBlockReaderV2 = new HFileBlock.FSReaderV2(fsdis, fsdisNoFsChecksum, compressAlgo,
            fileSize, trailer.getMinorVersion(), hfs, path);
      ...
      HFileBlock.BlockIterator blockIter = fsBlockReaderV2.blockRange(trailer.getLoadOnOpenDataOffset(), fileSize
   ...
   public HFileScanner getScanner(boolean cacheBlocks, final boolean pread, final boolean isCompaction) {
      ...
      if (dataBlockEncoder.useEncodedScanner(isCompaction)) {
         return new EncodedScannerV2(this, cacheBlocks, pread, isCompaction, includesMemstoreTS);
      }
   ...
   public ByteBuffer getMetaBlock(String metaBlockName, boolean cacheBlock) throws IOException {
      ...
      synchronized (metaBlockIndexReader.getRootBlockKey(block)) {
         ...
         BlockCacheKey cacheKey = new BlockCacheKey(name, metaBlockOffset, DataBlockEncoding.NONE, BlockType.META);

         cacheBlock &= cacheConf.shouldCacheDataOnRead();
         if (cacheConf.isBlockCacheEnabled()) {
            HFileBlock cachedBlock = (HFileBlock) cacheConf.getBlockCache().getBlock(cacheKey, cacheBlock, false);
            if (cachedBlock != null) {
   ...
   private void validateBlockType(HFileBlock block, BlockType expectedBlockType) throws IOException {
      ...
      if (actualBlockType == BlockType.ENCODED_DATA && expectedBlockType == BlockType.DATA) {
         // We consider DATA to match ENCODED_DATA for the purpose of this
         // verification.
         return;
      }
      if (actualBlockType != expectedBlockType) {
         throw new IOException("Expected block type " + expectedBlockType + ", " + "but got " + actualBlockType + ": "
               + block);
      }
   }
   ...
   protected abstract static class AbstractScannerV2 extends AbstractHFileReader.Scanner {
      ...
      public AbstractScannerV2(HFileReaderV2 r, boolean cacheBlocks, final boolean pread, final boolean isCompaction) {
      ...
      protected int seekTo(byte[] key, int offset, int length, boolean rewind) throws IOException {
         ...
         BlockWithScanInfo blockWithScanInfo = indexReader.loadDataBlockWithScanInfo(key, offset, length, block,
               cacheBlocks, pread, isCompaction);
         if (blockWithScanInfo == null || blockWithScanInfo.getHFileBlock() == null) {
            // This happens if the key e.g. falls before the beginning of the file.
            return -1;
         }
         return loadBlockAndSeekToKey(blockWithScanInfo.getHFileBlock(), blockWithScanInfo.getNextIndexedKey(), rewind,
               key, offset, length, false);
      }

      protected abstract ByteBuffer getFirstKeyInBlock(HFileBlock curBlock);

      protected abstract int loadBlockAndSeekToKey(HFileBlock seekToBlock, byte[] nextIndexedKey, boolean rewind,
            byte[] key, int offset, int length, boolean seekBefore) throws IOException;
      ...
      public int reseekTo(byte[] key, int offset, int length) throws IOException {
         ...
         if (isSeeked()) {
            ...
            compared = reader.getComparator().compare(key, offset, length, bb.array(), bb.arrayOffset(), bb.limit());
            if (compared < 1) {
               ...
               if (this.nextIndexedKey != null
                     && (this.nextIndexedKey == HConstants.NO_NEXT_INDEXED_KEY || reader.getComparator().compare(key,
                           offset, length, nextIndexedKey, 0, nextIndexedKey.length) < 0)) {
                  ...
                  return loadBlockAndSeekToKey(this.block, this.nextIndexedKey, false, key, offset, length, false);
               }
            }
         }
      ...
      public boolean seekBefore(byte[] key, int offset, int length) throws IOException {
         HFileBlock seekToBlock = reader.getDataBlockIndexReader().seekToDataBlock(key, offset, length, block,
               cacheBlocks, pread, isCompaction);
         if (seekToBlock == null) {
            return false;
         }
         ...
         if (reader.getComparator().compare(firstKey.array(), firstKey.arrayOffset(), firstKey.limit(), key, offset,
               length) == 0) {
            ...
            // figure out the size.
            seekToBlock = reader.readBlock(previousBlockOffset, seekToBlock.getOffset() - previousBlockOffset,
                  cacheBlocks, pread, isCompaction, BlockType.DATA);
      ...
      protected HFileBlock readNextDataBlock() throws IOException {
         ...
         do {
            ...
            curBlock = reader.readBlock(curBlock.getOffset() + curBlock.getOnDiskSizeWithHeader(),
                  curBlock.getNextBlockOnDiskSizeWithHeader(), cacheBlocks, pread, isCompaction, null);
         } while (!(curBlock.getBlockType().equals(BlockType.DATA) || curBlock.getBlockType().equals(
   ...
   protected static class ScannerV2 extends AbstractScannerV2 {
      ...
      public KeyValue getKeyValue() {
         ...
         KeyValue ret = new KeyValue(blockBuffer.array(), blockBuffer.arrayOffset() + blockBuffer.position(),
               KEY_VALUE_LEN_SIZE + currKeyLen + currValueLen, currKeyLen);
      ...
      public ByteBuffer getKey() {
         assertSeeked();
         return ByteBuffer.wrap(blockBuffer.array(),
               blockBuffer.arrayOffset() + blockBuffer.position() + KEY_VALUE_LEN_SIZE, currKeyLen).slice();
      }
      ...
      public ByteBuffer getValue() {
         assertSeeked();
         return ByteBuffer.wrap(blockBuffer.array(),
               blockBuffer.arrayOffset() + blockBuffer.position() + KEY_VALUE_LEN_SIZE + currKeyLen, currValueLen)
               .slice();
      }
      ...
      public boolean next() throws IOException {
         ...
         try {
            blockBuffer.position(blockBuffer.position() + KEY_VALUE_LEN_SIZE + currKeyLen + currValueLen
                  + currMemstoreTSLen);
         } catch (IllegalArgumentException e) {
            LOG.error("Current pos = " + blockBuffer.position() + "; currKeyLen = " + currKeyLen + "; currValLen = "
                  + currValueLen + "; block limit = " + blockBuffer.limit() + "; HFile name = " + reader.getName()
      ...
      @Override
      protected int loadBlockAndSeekToKey(HFileBlock seekToBlock, byte[] nextIndexedKey, boolean rewind, byte[] key,
            int offset, int length, boolean seekBefore) throws IOException {
      ...
      private void updateCurrBlock(HFileBlock newBlock) {
         ...
         if (block.getBlockType() != BlockType.DATA) {
            throw new IllegalStateException("ScannerV2 works only on data " + "blocks, got " + block.getBlockType()
                  + "; " + "fileName=" + reader.name + ", " + "dataBlockEncoder=" + reader.dataBlockEncoder + ", "
      ...
      private final void readKeyValueLen() {
         ...
         if (this.reader.shouldIncludeMemstoreTS()) {
            try {
               int memstoreTSOffset = blockBuffer.arrayOffset() + blockBuffer.position() + KEY_VALUE_LEN_SIZE
         ...
         if (currKeyLen < 0 || currValueLen < 0 || currKeyLen > blockBuffer.limit()
            ...
            throw new IllegalStateException("Invalid currKeyLen " + currKeyLen + " or currValueLen " + currValueLen
                  + ". Block offset: " + block.getOffset() + ", block length: " + blockBuffer.limit() + ", position: "
               ...
               if (seekBefore) {
                  if (lastKeyValueSize < 0) {
                     throw new IllegalStateException("blockSeek with seekBefore "
                           + "at the first key of the block: key=" + Bytes.toStringBinary(key) + ", blockOffset="
                           + block.getOffset() + ", onDiskSize=" + block.getOnDiskSizeWithHeader());
                  }
      ...
      public String getValueString() {
         return Bytes.toString(blockBuffer.array(), blockBuffer.arrayOffset() + blockBuffer.position()
   ...
   protected static class EncodedScannerV2 extends AbstractScannerV2 {
      ...
      public EncodedScannerV2(HFileReaderV2 reader, boolean cacheBlocks, boolean pread, boolean isCompaction,
      ...
      private void setDataBlockEncoder(DataBlockEncoder dataBlockEncoder) {
         ...
         seeker = dataBlockEncoder.createSeeker(reader.getComparator(), includesMemstoreTS);
      }
      ...
      public String getKeyString() {
         ...
         return Bytes.toStringBinary(keyBuffer.array(), keyBuffer.arrayOffset(), keyBuffer.limit());
      }

      @Override
      public String getValueString() {
         ByteBuffer valueBuffer = getValue();
         return Bytes.toStringBinary(valueBuffer.array(), valueBuffer.arrayOffset(), valueBuffer.limit());
      }
   ...
   private void validateMinorVersion(Path path, int minorVersion) {
      if (minorVersion < MIN_MINOR_VERSION || minorVersion > MAX_MINOR_VERSION) {
         String msg = "Minor version for path " + path + " is expected to be between " + MIN_MINOR_VERSION + " and "
               + MAX_MINOR_VERSION + " but is found to be " + minorVersion;
         LOG.error(msg);
