   ...
   public static enum Algorithm {
      ...
      SNAPPY("snappy") {
         ...
         CompressionCodec getCodec(Configuration conf) {
            if (snappyCodec == null) {
               try {
                  Class<?> externalCodec = getClassLoaderForCodec().loadClass(
       ...
       * buffering streams.
       */
      CompressionOutputStream createPlainCompressionStream(OutputStream downStream, Compressor compressor)
            throws IOException {
         CompressionCodec codec = getCodec(conf);
