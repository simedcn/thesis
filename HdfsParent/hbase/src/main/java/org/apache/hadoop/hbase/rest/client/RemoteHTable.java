...
public class RemoteHTable implements HTableInterface {
   ...
   protected String buildRowSpec(final byte[] row, final Map familyMap, final long startTime, final long endTime,
   ...
   protected CellSetModel buildModelFromPut(Put put) {
      ...
      for (List<KeyValue> kvs : put.getFamilyMap().values()) {
         for (KeyValue kv : kvs) {
            row.addCell(new CellModel(kv.getFamily(), kv.getQualifier(), ts != HConstants.LATEST_TIMESTAMP ? ts : kv
                  .getTimestamp(), kv.getValue()));
         }
      }
   ...
   public void delete(Delete delete) throws IOException {
      String spec = buildRowSpec(delete.getRow(), delete.getFamilyMap(), delete.getTimeStamp(), delete.getTimeStamp(),
            1);
      for (int i = 0; i < maxRetries; i++) {
