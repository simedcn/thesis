...
public abstract class CoprocessorHost<E extends CoprocessorEnvironment> {
    ...
    * (@see HBASE-7205)
    */
   static ConcurrentMap<Path, ClassLoader> classLoadersCache = new MapMaker().concurrencyLevel(3).weakValues()
         .makeMap();
   ...
   public E load(Path path, String className, int priority, Configuration conf) throws IOException {
      ...
      if (implClass == null) {
         ...
         Path dst = new Path(System.getProperty("java.io.tmpdir") + java.io.File.separator + "." + pathPrefix + "."
         ...
         while (entries.hasMoreElements()) {
            ...
            if (entry.getName().matches("/lib/[^/]+\\.jar")) {
               File file = new File(System.getProperty("java.io.tmpdir") + java.io.File.separator + "." + pathPrefix
    ...
    */
   public abstract E createEnvironment(Class<?> implClass, Coprocessor instance, int priority, int sequence,
         Configuration conf);
   ...
   protected void abortServer(final String service, final Server server, final CoprocessorEnvironment environment,
         final Throwable e) {
      String coprocessorName = (environment.getInstance()).toString();
      server.abort("Aborting service: " + service + " running on : " + server.getServerName()
            + " because coprocessor: " + coprocessorName + " threw an exception.", e);
   }

   protected void abortServer(final CoprocessorEnvironment environment, final Throwable e) {
      String coprocessorName = (environment.getInstance()).toString();
      LOG.error("The coprocessor: " + coprocessorName + " threw an unexpected " + "exception: " + e
            + ", but there's no specific implementation of " + " abortServer() for this coprocessor's environment.");
   }
