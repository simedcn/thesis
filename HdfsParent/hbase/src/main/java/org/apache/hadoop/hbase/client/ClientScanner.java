...
public class ClientScanner extends AbstractClientScanner {
    ...
    * @throws IOException
    */
   public ClientScanner(final Configuration conf, final Scan scan, final byte[] tableName) throws IOException {
    ...
    * @throws IOException
    */
   public ClientScanner(final Configuration conf, final Scan scan, final byte[] tableName, HConnection connection)
         throws IOException {
      ...
      this.scannerTimeout = (int) conf.getLong(HConstants.HBASE_REGIONSERVER_LEASE_PERIOD_KEY,
   ...
   protected ScannerCallable getScannerCallable(byte[] localStartKey, int nbRows) {
      ...
      ScannerCallable s = new ScannerCallable(getConnection(), getTableName(), scan, this.scanMetrics);
      s.setCaching(nbRows);
      return s;
   }
               ...
               if (e instanceof UnknownScannerException) {
                  ...
                  if (timeout < System.currentTimeMillis()) {
                     ...
                     ScannerTimeoutException ex = new ScannerTimeoutException(elapsed
                           + "ms passed since the last invocation, " + "timeout is currently set to " + scannerTimeout);
                     ex.initCause(e);
                     throw ex;
                  }
               } else {
