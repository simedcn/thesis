...
public class ReplicationSink {
   ...
   public ReplicationSink(Configuration conf, Stoppable stopper) throws IOException {
      ...
      this.sharedThreadPool = new ThreadPoolExecutor(1, conf.getInt("hbase.htable.threads.max", Integer.MAX_VALUE),
            conf.getLong("hbase.htable.threads.keepalivetime", 60), TimeUnit.SECONDS, new SynchronousQueue<Runnable>(),
            Threads.newDaemonThreadFactory("hbase-repl"));
