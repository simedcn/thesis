...
public class SequenceFileLogWriter implements HLog.Writer {
    ...
    */
   private static Metadata createMetadata(final Configuration conf, final boolean compress) {
   ...
   public void init(FileSystem fs, Path path, Configuration conf) throws IOException {
      ...
      try {
         ...
         this.writer = (SequenceFile.Writer) SequenceFile.class
               .getMethod(
                     "createWriter",
                     new Class[] { FileSystem.class, Configuration.class, Path.class, Class.class, Class.class,
                           Integer.TYPE, Short.TYPE, Long.TYPE, Boolean.TYPE, CompressionType.class,
                           CompressionCodec.class, Metadata.class }).invoke(
                     null,
                     new Object[] {
                           fs,
                           conf,
                           ...
                           Long.valueOf(conf.getLong("hbase.regionserver.hlog.blocksize", fs.getDefaultBlockSize())),
      ...
      if (this.writer == null) {
         ...
         this.writer = SequenceFile.createWriter(fs, conf, path, HLog.getKeyClass(conf), WALEdit.class, fs.getConf()
               .getInt("io.file.buffer.size", 4096), (short) conf.getInt("hbase.regionserver.hlog.replication",
               fs.getDefaultReplication()),
