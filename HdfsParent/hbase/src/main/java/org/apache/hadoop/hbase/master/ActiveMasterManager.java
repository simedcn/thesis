...
class ActiveMasterManager extends ZooKeeperListener {
   ...
   boolean blockUntilBecomingActiveMaster(MonitoredTask startupStatus, ClusterStatusTracker clusterStatusTracker) {
      while (true) {
         ...
         try {
            ...
            if (ZKUtil.createEphemeralNodeAndWatch(this.watcher, this.watcher.masterAddressZNode,
   ...
   public void stop() {
      try {
         ...
         byte[] bytes = ZKUtil.getDataAndWatch(watcher, watcher.masterAddressZNode);
