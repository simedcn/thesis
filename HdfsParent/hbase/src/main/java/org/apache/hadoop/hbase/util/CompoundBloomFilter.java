...
public class CompoundBloomFilter extends CompoundBloomFilterBase implements BloomFilter {
   ...
   public boolean contains(byte[] key, int keyOffset, int keyLength, ByteBuffer bloom) {
      ...
      if (block < 0) {
         ...
         try {
            ...
            bloomBlock = reader.readBlock(index.getRootBlockOffset(block), index.getRootBlockDataSize(block), true,
                  true, false, BlockType.BLOOM_CHUNK);
         } catch (IOException ex) {
            // The Bloom filter is broken, turn it off.
            throw new IllegalArgumentException("Failed to load Bloom block for key "
                  + Bytes.toStringBinary(key, keyOffset, keyLength), ex);
         }

         ByteBuffer bloomBuf = bloomBlock.getBufferReadOnly();
         result = ByteBloomFilter.contains(key, keyOffset, keyLength, bloomBuf.array(), bloomBuf.arrayOffset()
   ...
   public String toString() {
      ...
      sb.append(ByteBloomFilter.STATS_RECORD_SEP + "Number of chunks: " + numChunks);
      sb.append(ByteBloomFilter.STATS_RECORD_SEP + "Comparator: " + comparator.getClass().getSimpleName());
      return sb.toString();
   }
