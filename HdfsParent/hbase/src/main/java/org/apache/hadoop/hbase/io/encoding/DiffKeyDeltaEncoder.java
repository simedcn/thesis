...
public class DiffKeyDeltaEncoder extends BufferedDataBlockEncoder {
   ...
   private void compressSingleKeyValue(DiffCompressionState previousState, DiffCompressionState currentState,
      ...
      if (previousState.isFirst()) {
         ...
         commonPrefix = ByteBufferUtils.findCommonPrefix(in, in.position(), previousState.prevOffset
               + KeyValue.ROW_OFFSET, keyLength - KeyValue.TIMESTAMP_TYPE_SIZE);
   ...
   private void uncompressSingleKeyValue(DataInputStream source, ByteBuffer buffer, DiffCompressionState state)
      ...
      if (state.isFirst() || commonPrefix < state.rowLength + KeyValue.ROW_LENGTH_SIZE) {
         ...
         if (commonPrefix < KeyValue.ROW_LENGTH_SIZE) {
            ...
            ByteBufferUtils.copyFromStreamToBuffer(buffer, source, KeyValue.ROW_LENGTH_SIZE - commonPrefix);
         ...
         private void decode(boolean isFirst) {
            ...
            if (current.lastCommonPrefix < Bytes.SIZEOF_SHORT) {
               ...
               currentBuffer.get(current.keyBuffer, current.lastCommonPrefix, current.keyLength
                     - TIMESTAMP_WITH_TYPE_LENGTH - current.lastCommonPrefix);
            }
