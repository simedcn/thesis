...
public class AssignmentManager extends ZooKeeperListener {
   ...
   private List<EventType> ignoreStatesRSOffline = Arrays.asList(new EventType[] { EventType.RS_ZK_REGION_FAILED_OPEN,
    ...
    * @throws IOException 
    */
   public AssignmentManager(Server master, ServerManager serverManager, CatalogTracker catalogTracker,
         final LoadBalancer balancer, final ExecutorService service) throws KeeperException, IOException {
      ...
      this.timeoutMonitor = new TimeoutMonitor(conf.getInt("hbase.master.assignment.timeoutmonitor.period", 10000),
            master, serverManager, conf.getInt("hbase.master.assignment.timeoutmonitor.timeout", 1800000));
      this.timerUpdater = new TimerUpdater(conf.getInt("hbase.master.assignment.timerupdater.period", 10000), master);
      Threads.setDaemonThreadRunning(timerUpdater.getThread(), master.getServerName() + ".timerUpdater");
    ...
    * @throws InterruptedException
    */
   void processDeadServersAndRegionsInTransition(final Map<ServerName, List<Pair<HRegionInfo, Result>>> deadServers)
    ...
    * @throws IOException
    */
   boolean processRegionInTransitionAndBlockUntilAssigned(final HRegionInfo hri) throws InterruptedException,
   ...
   boolean processRegionInTransition(final String encodedRegionName, final HRegionInfo regionInfo,
         final Map<ServerName, List<Pair<HRegionInfo, Result>>> deadServers) throws KeeperException, IOException {
      Stat stat = new Stat();
      ...
      if (hri == null) {
         if ((hri = getHRegionInfo(data)) == null)
            return false;
      }
   ...
   void processRegionsInTransition(final RegionTransitionData data, final HRegionInfo regionInfo,
      ...
      synchronized (regionsInTransition) {
         ...
         case M_ZK_REGION_CLOSING:
            ...
            if (isOnDeadServer(regionInfo, deadServers)
               ...
               regionsInTransition.put(encodedRegionName,
                     new RegionState(regionInfo, RegionState.State.CLOSING, data.getStamp(), data.getOrigin()));
            }
         ...
         case M_ZK_REGION_OFFLINE:
            ...
            if (isOnDeadServer(regionInfo, deadServers)
               ...
               regionsInTransition.put(encodedRegionName, new RegionState(regionInfo, RegionState.State.PENDING_OPEN,
         ...
         case RS_ZK_REGION_OPENING:
            ...
            if (regionInfo.isMetaTable()) {
               regionsInTransition.put(encodedRegionName,
                     new RegionState(regionInfo, RegionState.State.OPENING, data.getStamp(), data.getOrigin()));
         ...
         case RS_ZK_REGION_OPENED:
            ...
            regionsInTransition.put(encodedRegionName,
                  new RegionState(regionInfo, RegionState.State.OPEN, data.getStamp(), data.getOrigin()));
   ...
   private void forceOffline(final HRegionInfo hri, final RegionTransitionData oldData) throws KeeperException {
      ...
      LOG.debug("RIT " + hri.getEncodedName() + " in state=" + oldData.getEventType()
    ...
    * @param oldData
    */
   private void addToRITandCallClose(final HRegionInfo hri, final RegionState.State state,
   ...
   private void handleRegion(final RegionTransitionData data, int expectedVersion) {
      synchronized (regionsInTransition) {
         ...
         if (!serverManager.isServerOnline(sn) && !this.master.getServerName().equals(sn)
         ...
         LOG.debug("Handling transition=" + data.getEventType() + ", server=" + data.getOrigin() + ", region="
         ...
         case M_ZK_REGION_CLOSING:
            ...
            if (regionState == null || (!regionState.isPendingClose() && !regionState.isClosing())) {
               LOG.warn("Received CLOSING for region " + prettyPrintedRegionName + " from server " + data.getOrigin()
                     + " but region was in " + " the state " + regionState + " and not "
         ...
         case RS_ZK_REGION_CLOSED:
            ...
            if (hri != null) {
               ...
               new ClosedRegionHandler(master, this, regionState.getRegion()).process();
               failoverProcessedRegions.put(encodedName, hri);
               break;
            }
            // Should see CLOSED after CLOSING but possible after PENDING_CLOSE
            if (regionState == null || (!regionState.isPendingClose() && !regionState.isClosing())) {
               LOG.warn("Received CLOSED for region " + prettyPrintedRegionName + " from server " + data.getOrigin()
                     + " but region was in " + " the state " + regionState + " and not "
                     + "in expected PENDING_CLOSE or CLOSING states");
               return;
            }
         ...
         case RS_ZK_REGION_FAILED_OPEN:
            ...
            if (regionState == null || (!regionState.isPendingOpen() && !regionState.isOpening())) {
               LOG.warn("Received FAILED_OPEN for region " + prettyPrintedRegionName + " from server "
                     + data.getOrigin() + " but region was in " + " the state " + regionState
         ...
         case RS_ZK_REGION_OPENING:
            ...
            if (regionState == null || (!regionState.isPendingOpen() && !regionState.isOpening())) {
               LOG.warn("Received OPENING for region " + prettyPrintedRegionName + " from server " + data.getOrigin()
                     + " but region was in " + " the state " + regionState + " and not "
         ...
         case RS_ZK_REGION_OPENED:
            ...
            if (hri != null) {
               ...
               regionsInTransition.put(encodedName, regionState);
               new OpenedRegionHandler(master, this, regionState.getRegion(), data.getOrigin(), expectedVersion)
                     .process();
               failoverProcessedRegions.put(encodedName, hri);
               break;
            }
            ...
            if (regionState == null || (!regionState.isPendingOpen() && !regionState.isOpening())) {
               LOG.warn("Received OPENED for region " + prettyPrintedRegionName + " from server " + data.getOrigin()
                     + " but region was in " + " the state " + regionState + " and not "
                     + "in expected PENDING_OPEN or OPENING states");
               return;
            }
            ...
            this.executorService.submit(new OpenedRegionHandler(master, this, regionState.getRegion(),
   ...
   private HRegionInfo checkIfInFailover(RegionState regionState, String encodedName, RegionTransitionData data) {
      if (regionState == null
            && this.failover
            && (failoverProcessedRegions.containsKey(encodedName) == false || failoverProcessedRegions.get(encodedName) == null)) {
         HRegionInfo hri = this.failoverProcessedRegions.get(encodedName);
         if (hri == null)
            hri = getHRegionInfo(data);
         return hri;
      }
      return null;
   }
   ...
   private HRegionInfo getHRegionInfo(RegionTransitionData data) {
      ...
      try {
         ...
         master.abort("Aborting because error occoured while reading " + data.getRegionName() + " from .META.", e);
         return null;
      }
   }
   ...
   private void handleHBCK(RegionTransitionData data) {
      String encodedName = HRegionInfo.encodeRegionName(data.getRegionName());
      LOG.info("Handling HBCK triggered transition=" + data.getEventType() + ", server=" + data.getOrigin()
   ...
   public void nodeDeleted(final String path) {
      if (path.startsWith(this.watcher.assignmentZNode)) {
         ...
         if (rs != null) {
            ...
            if (rs.isSplit()) {
               LOG.debug("Ephemeral node deleted, regionserver crashed?, offlining the region" + rs.getRegion()
                     + " clearing from RIT;");
               regionOffline(rs.getRegion());
            } else if (rs.isSplitting()) {
               LOG.debug("Ephemeral node deleted.  Found in SPLITTING state. " + "Removing from RIT " + rs.getRegion());
   ...
   private void makeRegionOnline(RegionState rs, HRegionInfo regionInfo) {
      ...
      LOG.info("The master has opened the region " + regionInfo.getRegionNameAsString() + " that was online on "
            + rs.serverName);
   ...
   void regionOnline(HRegionInfo regionInfo, ServerName sn) {
      ...
      synchronized (this.regions) {
         ...
         if (isServerOnline(sn)) {
            ...
            LOG.info("The server is not in online servers, ServerName=" + sn.getServerName() + ", region="
   ...
   public void offlineDisabledRegion(HRegionInfo regionInfo) {
      ...
      LOG.debug("Table being disabled so deleting ZK node and removing from "
            + "regions in transition, skipping assignment of region " + regionInfo.getRegionNameAsString());
      try {
   ...
   static class CreateUnassignedAsyncCallback implements AsyncCallback.StringCallback {
      ...
      CreateUnassignedAsyncCallback(final ZooKeeperWatcher zkw, final ServerName destination,
      ...
      public void processResult(int rc, String path, Object ctx, String name) {
         ...
         this.zkw.getRecoverableZooKeeper().getZooKeeper()
               .exists(path, this.zkw, new ExistsUnassignedAsyncCallback(this.counter, destination), ctx);
      }
   }
    ...
    * @param hijack
    */
   private void assign(final HRegionInfo region, final RegionState state, final boolean setOfflineInZK,
         final boolean forceNewPlan, boolean hijack) {
      ...
      for (int i = 0; i < this.maximumAssignmentAttempts; i++) {
         ...
         if (setOfflineInZK) {
            ...
            versionOfOfflineNode = setOfflineInZooKeeper(state, hijack, regionAlreadyInTransitionException);
         ...
         try {
            LOG.debug("Assigning region " + state.getRegion().getRegionNameAsString() + " to "
            ...
            RegionOpeningState regionOpenState = serverManager.sendRegionOpen(plan.getDestination(), state.getRegion(),
                  versionOfOfflineNode);
            if (regionOpenState == RegionOpeningState.ALREADY_OPENED) {
               // Remove region from in-memory transition and unassigned node from ZK
               // While trying to enable the table the regions of the table were
               // already enabled.
               LOG.debug("ALREADY_OPENED region " + state.getRegion().getRegionNameAsString() + " to "
                     + plan.getDestination().toString());
               ...
               try {
                  ...
                  master.abort("Error deleting OFFLINED node in ZK for transition ZK node (" + encodedRegionName + ")",
                        e);
               }
               synchronized (this.regionsInTransition) {
            ...
            if (t instanceof java.net.SocketTimeoutException
               ...
               LOG.warn(
                     "Call openRegion() to " + plan.getDestination() + " has timed out when trying to assign "
                           + region.getRegionNameAsString() + ", but the region might already be opened on "
            ...
            LOG.warn(
                  "Failed assignment of "
                        ...
                        + (regionAlreadyInTransitionException ? "to the same region server"
                              + " because of RegionAlreadyInTransitionException;" : "elsewhere instead; ") + "retry="
                        + i, t);
    ...
    */
   int setOfflineInZooKeeper(final RegionState state, boolean hijack, boolean regionAlreadyInTransitionException) {
      ...
      if (hijack
            && (state.getState().equals(RegionState.State.PENDING_OPEN) || state.getState().equals(
      ...
      try {
         ...
         versionOfOfflineNode = ZKAssign.createOrForceNodeOffline(master.getZooKeeper(), state.getRegion(),
               this.master.getServerName(), hijack, allowZNodeCreation);
         if (versionOfOfflineNode == -1) {
   ...
   boolean asyncSetOfflineInZooKeeper(final RegionState state, final AsyncCallback.StringCallback cb, final Object ctx) {
      ...
      try {
         ZKAssign
               .asyncCreateNodeOffline(master.getZooKeeper(), state.getRegion(), this.master.getServerName(), cb, ctx);
      } catch (KeeperException e) {
   ...
   RegionPlan getRegionPlan(final RegionState state, final ServerName serverToExclude, final boolean forceNewPlan) {
      ...
      synchronized (this.regionPlans) {
         ...
         if (existingPlan != null && existingPlan.getDestination() != null) {
            LOG.debug("Found an existing plan for " + state.getRegion().getRegionNameAsString()
         ...
         if (forceNewPlan || existingPlan == null || existingPlan.getDestination() == null
            ...
            newPlan = true;
            randomPlan = new RegionPlan(state.getRegion(), null, balancer.randomAssignment(servers));
      ...
      if (newPlan) {
         LOG.debug("No previous transition plan was found (or we are ignoring " + "an existing plan) for "
               ...
               + serverManager.countOfRegionServers() + " (online=" + serverManager.getOnlineServers().size()
   ...
   public void unassign(HRegionInfo region, boolean force) {
      ...
      synchronized (this.regions) {
         ...
         if (!regions.containsKey(region)) {
            LOG.debug("Attempted to unassign region " + region.getRegionNameAsString() + " but it is not "
         ...
         if (state == null) {
            ...
            try {
               ...
               if (versionOfClosingNode == -1) {
                  LOG.debug("Attempting to unassign region " + region.getRegionNameAsString() + " but ZK closing node "
                        + "can't be created.");
                  return;
               }
            } catch (KeeperException e) {
            ...
            LOG.debug("Attempting to unassign region " + region.getRegionNameAsString() + " which is already "
                  + state.getState() + " but forcing to send a CLOSE RPC again ");
            state.update(state.getState());
         } else {
            LOG.debug("Attempting to unassign region " + region.getRegionNameAsString() + " but it is "
      ...
      try {
         ...
         if (serverManager.sendRegionClose(server, state.getRegion(), versionOfClosingNode)) {
         ...
         LOG.info("Server " + server + " returned " + nsre + " for " + region.getRegionNameAsString());
         ...
         if (t instanceof RemoteException) {
            ...
            if (t instanceof NotServingRegionException) {
               if (checkIfRegionBelongsToDisabling(region) || checkIfRegionBelongsToDisabled(region)) {
                  // Remove from the regionsinTransition map
                  LOG.info("While trying to recover the table " + region.getTableNameAsString()
   ...
   public void deleteClosingOrClosedNode(HRegionInfo region) {
      try {
         if (!ZKAssign.deleteNode(master.getZooKeeper(), region.getEncodedName(),
            ...
            boolean deleteNode = ZKAssign.deleteNode(master.getZooKeeper(), region.getEncodedName(),
                  EventHandler.EventType.RS_ZK_REGION_CLOSED);
            // TODO : We don't abort if the delete node returns false. Is there any
            // such corner case?
            if (!deleteNode) {
               LOG.error("The deletion of the CLOSED node for the region " + region.getEncodedName() + " returned "
                     + deleteNode);
            }
         }
         ...
         master.abort("Unexpected ZK exception deleting node CLOSING/CLOSED for the region " + region.getEncodedName(),
               ke);
         return;
      }
   }
    ...
    * @throws IOException
    */
   public void assignUserRegions(List<HRegionInfo> regions, List<ServerName> servers) throws IOException,
   ...
   public void assignAllUserRegions() throws IOException, InterruptedException {
      ...
      Map<HRegionInfo, ServerName> allRegions = MetaReader.fullScan(catalogTracker, disablingDisabledAndEnablingTables,
            true);
   ...
   static class StartupBulkAssigner extends BulkAssigner {
      ...
      StartupBulkAssigner(final Server server, final Map<ServerName, List<HRegionInfo>> bulkPlan,
      ...
      protected long getTimeoutOnRIT() {
         ...
         long perRegionOpenTimeGuesstimate = this.server.getConfiguration().getLong(
               "hbase.bulk.assignment.perregion.open.time", 1000);
   ...
   static class GeneralBulkAssigner extends StartupBulkAssigner {
      GeneralBulkAssigner(final Server server, final Map<ServerName, List<HRegionInfo>> bulkPlan,
            final AssignmentManager am) {
   ...
   static class SingleServerBulkAssigner implements Runnable {
      ...
      SingleServerBulkAssigner(final ServerName regionserver, final List<HRegionInfo> regions,
            final AssignmentManager am) {
      ...
      for (Result result : results) {
         ...
         if (regionLocation == null) {
            ...
            if (enabling) {
               ...
               LOG.warn("Region " + regionInfo.getEncodedName() + " has null regionLocation." + " But its table "
   ...
   private void addToEnablingTableRegions(HRegionInfo regionInfo) {
      ...
      if (!hris.contains(regionInfo)) {
         if (LOG.isDebugEnabled()) {
            LOG.debug("Adding region" + regionInfo.getRegionNameAsString() + " to enabling table " + tableName + ".");
         }
         hris.add(regionInfo);
      }
   }

   private void enableTableIfNotDisabledOrDisablingOrEnabling(boolean disabled, boolean disablingOrEnabling,
         String tableName) {
      if (!disabled && !disablingOrEnabling && !getZKTable().isEnabledTable(tableName)) {
         setEnabledTable(tableName);
      }
   }
    ...
    * @throws IOException
    */
   private boolean recoverTableInDisablingState(Set<String> disablingTables) throws KeeperException,
    ...
    * @throws IOException
    */
   private void recoverTableInEnablingState(Set<String> enablingTables, boolean isWatcherCreated)
    ...
    * @throws KeeperException
    */
   private void processDeadServersAndRecoverLostRegions(Map<ServerName, List<Pair<HRegionInfo, Result>>> deadServers,
         ...
         for (Map.Entry<ServerName, List<Pair<HRegionInfo, Result>>> deadServer : deadServers.entrySet()) {
            ...
            for (Pair<HRegionInfo, Result> region : regions) {
               ...
               try {
                  ...
                  boolean assign = ServerShutdownHandler.processDeadRegion(regionInfo, result, this,
                        this.catalogTracker);
                  if (assign) {
   ...
   public class TimeoutMonitor extends Chore {
      ...
      * cleanup and exit cleanly.
       * @param timeout
       */
      public TimeoutMonitor(final int period, final Stoppable stopper, ServerManager serverManager, final int timeout) {
      ...
      private void actOnTimeOut(RegionState regionState) {
         ...
         case CLOSED:
            LOG.info("Region " + regionInfo.getEncodedName() + " has been CLOSED for too long, waiting on queued "
                  + "ClosedRegionHandler to run or server shutdown");
            // Update our timestamp.
            regionState.updateTimestampToNow();
            break;
         case OFFLINE:
            LOG.info("Region has been OFFLINE for too long, " + "reassigning " + regionInfo.getRegionNameAsString()
                  + " to a random server");
            invokeAssign(regionInfo);
            break;
         case PENDING_OPEN:
            LOG.info("Region has been PENDING_OPEN for too " + "long, reassigning region="
         ...
         case PENDING_CLOSE:
            LOG.info("Region has been PENDING_CLOSE for too " + "long, running forced unassign again on region="
                  + regionInfo.getRegionNameAsString());
            invokeUnassign(regionInfo);
            break;
         case CLOSING:
            LOG.info("Region has been CLOSING for too " + "long, this should eventually complete or the server will "
   ...
   private void processOpeningState(HRegionInfo regionInfo) {
      ...
      try {
         ...
         if (dataInZNode.getEventType() == EventType.RS_ZK_REGION_OPENED) {
            ...
            LOG.warn("While timing out a region in state OPENING, " + "found ZK node in unexpected state: "
   ...
   public boolean isCarryingRegion(ServerName serverName, HRegionInfo hri) {
      ...
      if (addressFromZK != null) {
         ...
         LOG.debug("based on ZK, current region=" + hri.getRegionNameAsString() + " is on server=" + addressFromZK
      ...
      LOG.debug("based on AM, current region=" + hri.getRegionNameAsString() + " is on server="
   ...
   public static class RegionState implements org.apache.hadoop.io.Writable {
      ...
      public String toDescriptiveString() {
         ...
         return region.getRegionNameAsString() + " state=" + state + ", ts=" + new Date(lstamp) + " ("
   ...
   protected void setEnabledTable(String tableName) {
      try {
         ...
         String errorMsg = "Unable to ensure that the table " + tableName + " will be"
