...
public class HFileBlock extends SchemaConfigured implements Cacheable {
   ...
   static final int HEADER_SIZE_NO_CHECKSUM = MAGIC_LENGTH + 2 * Bytes.SIZEOF_INT + Bytes.SIZEOF_LONG;
    ...
    */
   static final int HEADER_SIZE = HEADER_SIZE_NO_CHECKSUM + Bytes.SIZEOF_BYTE + 2 * Bytes.SIZEOF_INT;
    ...
    */
   public static final int ENCODED_HEADER_SIZE = HEADER_SIZE + DataBlockEncoding.ID_SIZE;
   ...
   public static final int BYTE_BUFFER_HEAP_SIZE = (int) ClassSize.estimateBase(ByteBuffer.wrap(new byte[0], 0, 0)
         .getClass(), false);
   ...
   private static final CacheableDeserializer<Cacheable> blockDeserializer = new CacheableDeserializer<Cacheable>() {
      public HFileBlock deserialize(ByteBuffer buf) throws IOException {
         ByteBuffer newByteBuffer = ByteBuffer.allocate(buf.limit() - HFileBlock.EXTRA_SERIALIZATION_SPACE);
   ...
   HFileBlock(BlockType blockType, int onDiskSizeWithoutHeader, int uncompressedSizeWithoutHeader,
         long prevBlockOffset, ByteBuffer buf, boolean fillHeader, long offset, boolean includesMemstoreTS,
         int minorVersion, int bytesPerChecksum, byte checksumType, int onDiskDataSizeWithHeader) {
   ...
   public ByteBuffer getBufferReadOnly() {
      return ByteBuffer.wrap(buf.array(), buf.arrayOffset(), buf.limit() - totalChecksumBytes()).slice();
   }
   ...
   void readInto(Writable w) throws IOException {
      ...
      if (Writables.getWritable(buf.array(), buf.arrayOffset() + headerSize(), buf.limit() - headerSize(), w) == null) {
   ...
   void sanityCheck() throws IOException {
...

      {
         ...
         if (blockTypeFromBuf != blockType) {
            throw new IOException("Block type stored in the buffer: " + blockTypeFromBuf + ", block type field: "
                  + blockType);
         }
      }
      ...
      if (buf.capacity() != size && buf.capacity() != size + hdrSize) {
         throw new AssertionError("Invalid buffer capacity: " + buf.capacity() + ", expected " + size + " or "
               + (size + hdrSize));
      }
   }
   ...
   private void validateOnDiskSizeWithoutHeader(int expectedOnDiskSizeWithoutHeader) throws IOException {
      if (onDiskSizeWithoutHeader != expectedOnDiskSizeWithoutHeader) {
         String blockInfoMsg = "Block offset: " + offset + ", data starts with: "
               + Bytes.toStringBinary(buf.array(), buf.arrayOffset(), buf.arrayOffset() + Math.min(32, buf.limit()));
         throw new IOException("On-disk size without header provided is " + expectedOnDiskSizeWithoutHeader
               + ", but block " + "header contains " + onDiskSizeWithoutHeader + ". " + blockInfoMsg);
      }
   }
   ...
   public void assumeUncompressed() throws IOException {
      if (onDiskSizeWithoutHeader != uncompressedSizeWithoutHeader + totalChecksumBytes()) {
         throw new IOException("Using no compression but " + "onDiskSizeWithoutHeader=" + onDiskSizeWithoutHeader
               + ", " + "uncompressedSizeWithoutHeader=" + uncompressedSizeWithoutHeader + ", numChecksumbytes="
               + totalChecksumBytes());
      }
   }
   ...
   public void expectType(BlockType expectedType) throws IOException {
      if (blockType != expectedType) {
         throw new IOException("Invalid block type: expected=" + expectedType + ", actual=" + blockType);
      }
   }
   ...
   public DataInputStream getByteStream() {
      return new DataInputStream(new ByteArrayInputStream(buf.array(), buf.arrayOffset() + headerSize(), buf.limit()
            - headerSize()));
   }
    ...
    */
   public static boolean readWithExtra(InputStream in, byte buf[], int bufOffset, int necessaryLen, int extraLen)
         throws IOException {
      ...
      while (bytesRemaining > 0) {
         ...
         if (ret < 0) {
            throw new IOException("Premature EOF from inputStream (read " + "returned " + ret + ", was trying to read "
                  + necessaryLen + " necessary bytes and " + extraLen + " extra bytes, " + "successfully read "
   ...
   public static class Writer {
       ...
       */
      public Writer(Compression.Algorithm compressionAlgorithm, HFileDataBlockEncoder dataBlockEncoder,
         ...
         if (bytesPerChecksum < HEADER_SIZE) {
            throw new RuntimeException("Unsupported value of bytesPerChecksum. " + " Minimum is " + HEADER_SIZE
      ...
      private void doCompressionAndChecksumming() throws IOException {
         ...
         if (compressAlgo != NONE) {
            ...
            compressionStream.write(uncompressedBytesWithHeader, HEADER_SIZE, uncompressedBytesWithHeader.length
                  - HEADER_SIZE);
            ...
            ChecksumUtil.generateChecksums(onDiskBytesWithHeader, 0, onDiskDataSizeWithHeader, onDiskBytesWithHeader,
                  onDiskDataSizeWithHeader, checksumType, bytesPerChecksum);

            // Checksums are already part of onDiskBytesWithHeader
            onDiskChecksum = HConstants.EMPTY_BYTE_ARRAY;

            //set the header for the uncompressed bytes (for cache-on-write)
            putHeader(uncompressedBytesWithHeader, 0, onDiskBytesWithHeader.length + onDiskChecksum.length,
            ...
            ChecksumUtil.generateChecksums(uncompressedBytesWithHeader, 0, uncompressedBytesWithHeader.length,
      ...
      private void encodeDataBlockForDisk() throws IOException {
         ...
         ByteBuffer rawKeyValues = ByteBuffer.wrap(uncompressedBytesWithHeader, HEADER_SIZE,
               uncompressedBytesWithHeader.length - HEADER_SIZE).slice();
         Pair<ByteBuffer, BlockType> encodingResult = dataBlockEncoder.beforeWriteToDisk(rawKeyValues,
      ...
      public HFileBlock getBlockForCaching() {
         return new HFileBlock(blockType, getOnDiskSizeWithoutHeader(), getUncompressedSizeWithoutHeader(), prevOffset,
               getUncompressedBufferWithHeader(), DONT_FILL_HEADER, startOffset, includesMemstoreTS,
               MINOR_VERSION_WITH_CHECKSUM, 0, ChecksumType.NULL.getCode(), // no checksums in cached data
   ...
   private abstract static class AbstractFSReader implements FSReader {
      ...
      public AbstractFSReader(FSDataInputStream istream, FSDataInputStream istreamNoFsChecksum, Algorithm compressAlgo,
            long fileSize, int minorVersion, HFileSystem hfs, Path path) throws IOException {
         ...
         if (!pread && streamLock.tryLock()) {
            ...
            try {
               ...
               if (realOffset != fileOffset) {
                  throw new IOException("Tried to seek to " + fileOffset + " to " + "read " + size + " bytes, but pos="
                        + realOffset + " after seek");
               }
            ...
            if (ret < size) {
               throw new IOException("Positional read of " + size + " bytes " + "failed at offset " + fileOffset
                     + " (returned " + ret + ")");
            }
       ...
       * @throws IOException
       */
      protected void decompress(byte[] dest, int destOffset, InputStream bufferedBoundedStream, int uncompressedSize)
            throws IOException {
      ...
      protected InputStream createBufferedBoundedStream(long offset, int size, boolean pread) {
         return new BufferedInputStream(new BoundedRangeFileInputStream(istream, offset, size, pread), Math.min(
   ...
   static class FSReaderV1 extends AbstractFSReader {
      ...
      public FSReaderV1(FSDataInputStream istream, Algorithm compressAlgo, long fileSize) throws IOException {
       ...
       */
      @Override
      public HFileBlock readBlockData(long offset, long onDiskSizeWithMagic, int uncompressedSizeWithMagic,
         ...
         if (uncompressedSizeWithMagic < MAGIC_LENGTH) {
            throw new IOException("Uncompressed size for a version 1 block is " + uncompressedSizeWithMagic
                  + " but must be at least " + MAGIC_LENGTH);
         }
         ...
         if (compressAlgo == Compression.Algorithm.NONE) {
            ...
            if (onDiskSize != uncompressedSizeWithMagic) {
               throw new IOException("onDiskSize=" + onDiskSize + " and uncompressedSize=" + uncompressedSizeWithMagic
            ...
            decompress(buf.array(), buf.arrayOffset() + HEADER_DELTA, bufferedBoundedStream, uncompressedSizeWithMagic);
         ...
         HFileBlock b = new HFileBlock(newBlockType, onDiskSizeWithoutHeader, uncompressedSizeWithMagic - MAGIC_LENGTH,
               -1L, buf, FILL_HEADER, offset, MemStore.NO_PERSISTENT_TS, 0, 0, ChecksumType.NULL.getCode(),
   ...
   static class FSReaderV2 extends AbstractFSReader {
      ...
      public FSReaderV2(FSDataInputStream istream, FSDataInputStream istreamNoFsChecksum, Algorithm compressAlgo,
            long fileSize, int minorVersion, HFileSystem hfs, Path path) throws IOException {
       ...
       */
      @Override
      public HFileBlock readBlockData(long offset, long onDiskSizeWithHeaderL, int uncompressedSize, boolean pread)
            throws IOException {
         ...
         HFileBlock blk = readBlockDataInternal(is, offset, onDiskSizeWithHeaderL, uncompressedSize, pread,
               doVerificationThruHBaseChecksum);
         if (blk == null) {
            HFile.LOG.warn("HBase checksum verification failed for file " + path + " at offset " + offset
                  + " filesize " + fileSize + ". Retrying read with HDFS checksums turned on...");

            if (!doVerificationThruHBaseChecksum) {
               String msg = "HBase checksum verification failed for file " + path + " at offset " + offset
            ...
            blk = readBlockDataInternal(is, offset, onDiskSizeWithHeaderL, uncompressedSize, pread,
                  doVerificationThruHBaseChecksum);
            if (blk != null) {
               HFile.LOG.warn("HDFS checksum verification suceeded for file " + path + " at offset " + offset
                     + " filesize " + fileSize);
            }
         }
         if (blk == null && !doVerificationThruHBaseChecksum) {
            String msg = "readBlockData failed, possibly due to " + "checksum verification failed for file " + path
       ...
       */
      private HFileBlock readBlockDataInternal(FSDataInputStream is, long offset, long onDiskSizeWithHeaderL,
            int uncompressedSize, boolean pread, boolean verifyChecksum) throws IOException {
         if (offset < 0) {
            throw new IOException("Invalid offset=" + offset + " trying to read " + "block (onDiskSize="
                  + onDiskSizeWithHeaderL + ", uncompressedSize=" + uncompressedSize + ")");
         }
         ...
         if ((onDiskSizeWithHeaderL < hdrSize && onDiskSizeWithHeaderL != -1)
            ...
            throw new IOException("Invalid onDisksize=" + onDiskSizeWithHeaderL + ": expected to be at least "
                  + hdrSize + " and at most " + Integer.MAX_VALUE + ", or -1 (offset=" + offset + ", uncompressedSize="
                  + uncompressedSize + ")");
         }
         ...
         if (onDiskSizeWithHeader > 0) {
            ...
            if (compressAlgo == Compression.Algorithm.NONE) {
               ...
               int nextBlockOnDiskSizeWithHeader = readAtOffset(is, headerAndData.array(), headerAndData.arrayOffset()
                     + preReadHeaderSize, onDiskSizeWithHeader - preReadHeaderSize, true, offset + preReadHeaderSize,
                     pread);
               ...
               int nextBlockOnDiskSize = readAtOffset(is, onDiskBlock, preReadHeaderSize, onDiskSizeWithHeader
               ...
               try {
                  ...
                  throw new IOException("Failed to read compressed block at " + offset + ", onDiskSizeWithoutHeader="
                        + onDiskSizeWithHeader + ", preReadHeaderSize=" + preReadHeaderSize + ", header.length="
                        + header.length + ", header bytes: " + Bytes.toStringBinary(header, 0, hdrSize), ex);
               }
               b.validateOnDiskSizeWithoutHeader(onDiskSizeWithoutHeader);
               ...
               DataInputStream dis = new DataInputStream(new ByteArrayInputStream(onDiskBlock, hdrSize,
                     onDiskSizeWithoutHeader));

               // This will allocate a new buffer but keep header bytes.
               b.allocateBuffer(b.nextBlockOnDiskSizeWithHeader > 0);

               decompress(b.buf.array(), b.buf.arrayOffset() + hdrSize, dis, b.uncompressedSizeWithoutHeader);

               // Copy next block's header bytes into the new block if we have them.
               if (nextBlockOnDiskSize > 0) {
                  System.arraycopy(onDiskBlock, onDiskSizeWithHeader, b.buf.array(), b.buf.arrayOffset() + hdrSize
                        + b.uncompressedSizeWithoutHeader + b.totalChecksumBytes(), hdrSize);
            ...
            if (compressAlgo == Compression.Algorithm.NONE) {
               ...
               // nothing.
               b.assumeUncompressed();
               b.nextBlockOnDiskSizeWithHeader = readAtOffset(is, b.buf.array(), b.buf.arrayOffset() + hdrSize,
                     b.uncompressedSizeWithoutHeader + b.totalChecksumBytes(), true, offset + hdrSize, pread);
               ...
               b.nextBlockOnDiskSizeWithHeader = readAtOffset(is, compressedBytes, hdrSize, b.onDiskSizeWithoutHeader,
                     true, offset + hdrSize, pread);
               if (verifyChecksum && !validateBlockChecksum(b, compressedBytes, hdrSize)) {
                  return null; // checksum mismatch
               }
               DataInputStream dis = new DataInputStream(new ByteArrayInputStream(compressedBytes, hdrSize,
               ...
               if (b.nextBlockOnDiskSizeWithHeader > 0) {
                  ...
                  int nextHeaderOffset = b.buf.arrayOffset() + hdrSize + b.uncompressedSizeWithoutHeader
                        + b.totalChecksumBytes();
                  System.arraycopy(compressedBytes, compressedBytes.length - hdrSize, b.buf.array(), nextHeaderOffset,
                        hdrSize);
   ...
   static String toStringHeader(ByteBuffer buf) throws IOException {
      ...
      return " Header dump: magic: " + magic + " blockType " + bt + " compressedBlockSizeNoHeader "
            + compressedBlockSizeNoHeader + " uncompressedBlockSizeNoHeader " + uncompressedBlockSizeNoHeader
            + " prevBlockOffset " + prevBlockOffset + " checksumType " + ChecksumType.codeToType(cksumtype)
