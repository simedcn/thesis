...
public abstract class AbstractHFileReader extends SchemaConfigured implements HFile.Reader {
   ...
   protected AbstractHFileReader(Path path, FixedFileTrailer trailer, final FSDataInputStream fsdis,
         final long fileSize, final boolean closeIStream, final CacheConfig cacheConf) {
      this(path, trailer, fsdis, fsdis, fileSize, closeIStream, cacheConf, null);
   }

   protected AbstractHFileReader(Path path, FixedFileTrailer trailer, final FSDataInputStream fsdis,
         final FSDataInputStream fsdisNoFsChecksum, final long fileSize, final boolean closeIStream,
   ...
   public String toString() {
      return "reader="
            + path.toString()
            + (!isFileInfoLoaded() ? "" : ", compression=" + compressAlgo.getName() + ", cacheConf=" + cacheConf
                  + ", firstKey=" + toStringFirstKey() + ", lastKey=" + toStringLastKey()) + ", avgKeyLen=" + avgKeyLen
            + ", avgValueLen=" + avgValueLen + ", entries=" + trailer.getEntryCount() + ", length=" + fileSize;
   }
   ...
   protected static abstract class Scanner implements HFileScanner {
      ...
      public Scanner(final HFile.Reader reader, final boolean cacheBlocks, final boolean pread,
