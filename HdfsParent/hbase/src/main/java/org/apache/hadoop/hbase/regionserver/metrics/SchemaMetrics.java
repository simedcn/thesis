...
public class SchemaMetrics {
   ...
   public static enum BlockMetricType {
      ...
      READ_TIME("Read", true, true), READ_COUNT("BlockReadCnt", true, false), CACHE_HIT("BlockReadCacheHitCnt", true,
            false), CACHE_MISS("BlockReadCacheMissCnt", true, false),

      CACHE_SIZE("blockCacheSize", false, false), CACHED("blockCacheNumCached", false, false), EVICTED(
            "blockCacheNumEvicted", false, false);
      ...
      BlockMetricType(String metricStr, boolean compactionAware, boolean timeVarying) {
   ...
   public static enum StoreMetricType {
      STORE_FILE_COUNT("storeFileCount"), STORE_FILE_INDEX_SIZE("storeFileIndexSizeMB"), STORE_FILE_SIZE_MB(
            "storeFileSizeMB"), STATIC_BLOOM_SIZE_KB("staticBloomSizeKB"), MEMSTORE_SIZE_MB("memstoreSizeMB"), STATIC_INDEX_SIZE_KB(
            "staticIndexSizeKB"), FLUSH_SIZE("flushSize");
   ...
   private static final int NUM_BLOCK_METRICS = NUM_BLOCK_CATEGORIES * // blockCategory
    ...
    */
   private static final ConcurrentHashMap<String, SchemaMetrics> tableAndFamilyToMetrics = new ConcurrentHashMap<String, SchemaMetrics>();
   ...
   private static final int getCacheHitMetricIndex(BlockCategory blockCategory, boolean isCompaction) {
      return blockCategory.ordinal() * BOOL_VALUES.length + (isCompaction ? 1 : 0);
   }

   private static final int getBlockMetricIndex(BlockCategory blockCategory, boolean isCompaction,
         BlockMetricType metricType) {
      int i = 0;
   ...
   public String getBlockMetricName(BlockCategory blockCategory, boolean isCompaction, BlockMetricType metricType) {
      if (isCompaction && !metricType.compactionAware) {
         throw new IllegalArgumentException("isCompaction cannot be true for " + metricType);
      }
      return blockMetricNames[getBlockMetricIndex(blockCategory, isCompaction, metricType)];
   }
    ...
    */
   private void incrNumericMetric(BlockCategory blockCategory, boolean isCompaction, BlockMetricType metricType) {
   ...
   private void incrNumericMetric(BlockCategory blockCategory, boolean isCompaction, BlockMetricType metricType,
      ...
      RegionMetricsStorage.incrNumericMetric(getBlockMetricName(blockCategory, isCompaction, metricType), amount);

      if (blockCategory != BlockCategory.ALL_CATEGORIES) {
         incrNumericMetric(BlockCategory.ALL_CATEGORIES, isCompaction, metricType, amount);
      }
   }

   private void addToReadTime(BlockCategory blockCategory, boolean isCompaction, long timeMs) {
      RegionMetricsStorage.incrTimeVaryingMetric(
            getBlockMetricName(blockCategory, isCompaction, BlockMetricType.READ_TIME), timeMs);
   ...
   public void updatePersistentStoreMetric(StoreMetricType storeMetricType, long value) {
      RegionMetricsStorage.incrNumericPersistentMetric(storeMetricNames[storeMetricType.ordinal()], value);
   }
    ...
    * cache hit.
    */
   public void updateOnCacheHit(BlockCategory blockCategory, boolean isCompaction) {
    ...
    * cache hit.
    */
   public void updateOnCacheHit(BlockCategory blockCategory, boolean isCompaction, long count) {
    ...
    */
   public void updateOnCacheMiss(BlockCategory blockCategory, boolean isCompaction, long timeMs) {
      blockCategory.expectSpecific();
   ...
   public void addToCacheSize(BlockCategory category, long cacheSizeDelta) {
      ...
      RegionMetricsStorage.incrNumericPersistentMetric(getBlockMetricName(category, false, BlockMetricType.CACHE_SIZE),
            cacheSizeDelta);
   ...
   public void updateOnCachePutOrEvict(BlockCategory blockCategory, long cacheSizeDelta, boolean isEviction) {
      addToCacheSize(blockCategory, cacheSizeDelta);
      incrNumericMetric(blockCategory, false, isEviction ? BlockMetricType.EVICTED : BlockMetricType.CACHED);
      if (this != ALL_SCHEMA_METRICS) {
   ...
   public static void configureGlobally(Configuration conf) {
      final boolean useTableNameNew = conf.getBoolean(SHOW_TABLE_NAME_CONF_KEY, false);
   ...
   private static String getEffectiveTableName(String tableName) {
      if (!tableName.equals(TOTAL_KEY)) {
         ...
         if (useTableNameGlobally == null) {
            throw new IllegalStateException("The value of the " + SHOW_TABLE_NAME_CONF_KEY
                  + " conf option has not been specified " + "in SchemaMetrics");
         }
    ...
    */
   public static String generateSchemaMetricsPrefix(String tableName, final String cfName) {
      tableName = getEffectiveTableName(tableName);
      String schemaMetricPrefix = tableName.equals(TOTAL_KEY) ? "" : TABLE_PREFIX + tableName + ".";
      schemaMetricPrefix += cfName.equals(TOTAL_KEY) ? "" : CF_PREFIX + cfName + ".";
      return schemaMetricPrefix;
   }

   public static String generateSchemaMetricsPrefix(byte[] tableName, byte[] cfName) {
      return generateSchemaMetricsPrefix(Bytes.toString(tableName), Bytes.toString(cfName));
   }
   ...
   private static void setUseTableName(final boolean useTableNameNew) {
      if (useTableNameGlobally == null) {
         ...
         LOG.error("Inconsistent configuration. Previous configuration " + "for using table name in metrics: "
               + useTableNameGlobally + ", " + "new configuration: " + useTableNameNew);
   ...
   private static final Pattern BLOCK_CATEGORY_RE = Pattern.compile("\\b" + regexEscape(BLOCK_TYPE_PREFIX) + "[^.]+\\."
         +
         // Also remove the special-case block type marker for meta blocks
         "|" + META_BLOCK_CATEGORY_STR + "(?=" + BlockMetricType.BLOCK_METRIC_TYPE_RE + ")");
   ...
   void printMetricNames() {
      for (BlockCategory blockCategory : BlockCategory.values()) {
         for (boolean isCompaction : BOOL_VALUES) {
            for (BlockMetricType metricType : BlockMetricType.values()) {
               int i = getBlockMetricIndex(blockCategory, isCompaction, metricType);
               LOG.debug("blockCategory=" + blockCategory + ", " + "metricType=" + metricType + ", isCompaction="
                     + isCompaction + ", metricName=" + blockMetricNames[i]);
            }
         }
      }
   }
      ...
      for (SchemaMetrics cfm : tableAndFamilyToMetrics.values()) {
         ...
         for (String metricName : cfm.getAllMetricNames()) {
            ...
            if (isTimeVaryingKey(metricName)) {
               Pair<Long, Integer> totalAndCount = RegionMetricsStorage
                     .getTimeVaryingMetric(stripTimeVaryingSuffix(metricName));
   ...
   public static void validateMetricChanges(Map<String, Long> oldMetrics) {
      ...
      for (String key : allKeys) {
         ...
         if (actual != expected) {
            if (errors.length() > 0)
               errors.append("\n");
            errors.append("The all-block-category metric " + key + " changed by " + actual + " but the aggregation of "
