...
public class HBaseClient {
   ...
   static class FailedServers {
      private final LinkedList<Pair<Long, String>> failedServers = new LinkedList<Pair<Long, String>>();

      private final int recheckServersTimeout;

      FailedServers(Configuration conf) {
         this.recheckServersTimeout = conf.getInt(FAILED_SERVER_EXPIRY_KEY, FAILED_SERVER_EXPIRY_DEFAULT);
      }
      ...
      protected synchronized void addCall(Call call) {
         ...
         if (this.shouldCloseConnection.get()) {
            if (this.closeException == null) {
               call.setException(new IOException("Call " + call.id + " not added as the connection " + remoteId
                     + " is closing"));
            } else {
   ...
   public Writable call(Writable param, InetSocketAddress addr, User ticket, int rpcTimeout) throws IOException,
    ...
    * threw an exception. */
   public Writable call(Writable param, InetSocketAddress addr, Class<? extends VersionedProtocol> protocol,
         User ticket, int rpcTimeout) throws InterruptedException, IOException {
   ...
   public Writable[] call(Writable[] params, InetSocketAddress[] addresses,
         Class<? extends VersionedProtocol> protocol, User ticket) throws IOException, InterruptedException {
