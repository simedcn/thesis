...
public class HFileReaderV1 extends AbstractHFileReader {
      ...
      if (evictOnClose && cacheConf.isBlockCacheEnabled()) {
         ...
         for (int i = 0; i < dataBlockIndexReader.getRootBlockCount(); i++) {
            if (cacheConf.getBlockCache().evictBlock(
                  new BlockCacheKey(name, dataBlockIndexReader.getRootBlockOffset(i), DataBlockEncoding.NONE,
                        BlockType.DATA))) {
               numEvicted++;
            }
         }
   ...
   protected abstract static class AbstractScannerV1 extends AbstractHFileReader.Scanner {
      ...
      public int reseekTo(byte[] key, int offset, int length) throws IOException {
         if (blockBuffer != null && currKeyLen != 0) {
            ...
            int compared = reader.getComparator()
                  .compare(key, offset, length, bb.array(), bb.arrayOffset(), bb.limit());
            if (compared < 1) {
   ...
   protected static class ScannerV1 extends AbstractScannerV1 {
      ...
      public ScannerV1(HFileReaderV1 reader, boolean cacheBlocks, final boolean pread, final boolean isCompaction) {
         super(reader, cacheBlocks, pread, isCompaction);
         this.reader = reader;
      }
      ...
      public ByteBuffer getKey() {
         Preconditions
               .checkState(blockBuffer != null && currKeyLen > 0, "you need to seekTo() before calling getKey()");
      ...
      public String getValueString() {
         return Bytes.toString(blockBuffer.array(), blockBuffer.arrayOffset() + blockBuffer.position() + currKeyLen,
               currValueLen);
      }
