...
public class ThriftServer {
   ...
   private static void printUsageAndExit(Options options, int exitCode) throws ExitCodeException {
      ...
      formatter.printHelp("Thrift", null, options,
            "To start the Thrift server run 'bin/hbase-daemon.sh start thrift'\n"
                  ...
                  + "thrift' or send a kill signal to the thrift server pid", true);
   ...
   private void processOptions(final String[] args) throws Exception {
      ...
      options.addOption("b", BIND_OPTION, true, "Address to bind "
            + "the Thrift server to. Not supported by the Nonblocking and " + "HsHa server [default: "
            + DEFAULT_BIND_ADDR + "]");
      ...
      options.addOption("m", MIN_WORKERS_OPTION, true, "The minimum number of worker threads for "
            + ImplType.THREAD_POOL.simpleClassName());

      options.addOption("w", MAX_WORKERS_OPTION, true, "The maximum number of worker threads for "
            + ImplType.THREAD_POOL.simpleClassName());

      options.addOption("q", MAX_QUEUE_SIZE_OPTION, true, "The maximum number of queued requests in "
            + ImplType.THREAD_POOL.simpleClassName());
