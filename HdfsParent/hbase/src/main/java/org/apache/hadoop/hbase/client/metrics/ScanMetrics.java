...
public class ScanMetrics implements Writable {
    ...
    * number of RPC calls
    */
   public final MetricsTimeVaryingLong countOfRPCcalls = new MetricsTimeVaryingLong("RPC_CALLS", registry);

   /**
    * number of remote RPC calls
    */
   public final MetricsTimeVaryingLong countOfRemoteRPCcalls = new MetricsTimeVaryingLong("REMOTE_RPC_CALLS", registry);

   /**
    * sum of milliseconds between sequential next calls
    */
   public final MetricsTimeVaryingLong sumOfMillisSecBetweenNexts = new MetricsTimeVaryingLong("MILLIS_BETWEEN_NEXTS",
         registry);

   /**
    * number of NotServingRegionException caught
    */
   public final MetricsTimeVaryingLong countOfNSRE = new MetricsTimeVaryingLong("NOT_SERVING_REGION_EXCEPTION",
         registry);

   /**
    * number of bytes in Result objects from region servers
    */
   public final MetricsTimeVaryingLong countOfBytesInResults = new MetricsTimeVaryingLong("BYTES_IN_RESULTS", registry);

   /**
    * number of bytes in Result objects from remote region servers
    */
   public final MetricsTimeVaryingLong countOfBytesInRemoteResults = new MetricsTimeVaryingLong(
         "BYTES_IN_REMOTE_RESULTS", registry);

   /**
    * number of regions
    */
   public final MetricsTimeVaryingLong countOfRegions = new MetricsTimeVaryingLong("REGIONS_SCANNED", registry);

   /**
    * number of RPC retries
    */
   public final MetricsTimeVaryingLong countOfRPCRetries = new MetricsTimeVaryingLong("RPC_RETRIES", registry);

   /**
    * number of remote RPC retries
    */
   public final MetricsTimeVaryingLong countOfRemoteRPCRetries = new MetricsTimeVaryingLong("REMOTE_RPC_RETRIES",
         registry);
   ...
   public void write(DataOutput out) throws IOException {
      ...
      for (MetricsBase mb : mbs) {
         if (mb instanceof MetricsTimeVaryingLong) {
            ...
            throw new IOException("unsupported metrics type. metrics name: " + mb.getName() + ", metrics description: "
                  + mb.getDescription());
         }
      }
   ...
   public void readFields(DataInput in) throws IOException {
      ...
      for (int i = 0; i < metricsCount; i++) {
         ...
         if (mb instanceof MetricsTimeVaryingLong) {
            ...
            LOG.warn("unsupported metrics type. metrics name: " + mb.getName() + ", metrics description: "
                  + mb.getDescription());
         }
      }
   }
