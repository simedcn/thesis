...
public abstract class TableInputFormatBase extends InputFormat<ImmutableBytesWritable, Result> {
   ...
   public List<InputSplit> getSplits(JobContext context) throws IOException {
      ...
      if (keys == null || keys.getFirst() == null || keys.getFirst().length == 0) {
         ...
         InputSplit split = new TableSplit(table.getTableName(), HConstants.EMPTY_BYTE_ARRAY,
               HConstants.EMPTY_BYTE_ARRAY, regLoc.getHostnamePort().split(Addressing.HOSTNAME_PORT_SEPARATOR)[0]);
         splits.add(split);
         return splits;
      }
