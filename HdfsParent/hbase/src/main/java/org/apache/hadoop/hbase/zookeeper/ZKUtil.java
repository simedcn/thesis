...
public class ZKUtil {
   ...
   private static void login(Configuration conf, String keytabFileKey, String userNameKey, String hostname,
      ...
      JaasConfiguration jaasConf = new JaasConfiguration(loginContextName, keytabFilename, principalName);
   ...
   public static String[] transformClusterKey(String key) throws IOException {
      ...
      if (parts.length != 3) {
         throw new IOException("Cluster key invalid, the format should be:" + HConstants.ZOOKEEPER_QUORUM
   ...
   private static ArrayList<ACL> createACL(ZooKeeperWatcher zkw, String node) {
      if (isSecureZooKeeper(zkw.getConfiguration())) {
         ...
         if ((node.equals(zkw.baseZNode) == true) || (node.equals(zkw.rootServerZNode) == true)
               || (node.equals(zkw.masterAddressZNode) == true) || (node.equals(zkw.clusterIdZNode) == true)
               || (node.equals(zkw.rsZNode) == true) || (node.equals(zkw.backupMasterAddressesZNode) == true)
               || (node.startsWith(zkw.masterTableZNode) == true) || (node.startsWith(zkw.masterTableZNode92) == true)) {
   ...
   public static void createAndFailSilent(ZooKeeperWatcher zkw, String znode) throws KeeperException {
      createAndFailSilent(zkw, (CreateAndFailSilent) ZKUtilOp.createAndFailSilent(znode, new byte[0]));
   }
   ...
   public static void createWithParents(ZooKeeperWatcher zkw, String znode) throws KeeperException {
      try {
         ...
         waitForZKConnectionIfAuthenticating(zkw);
         zkw.getRecoverableZooKeeper().create(znode, new byte[0], createACL(zkw, znode), CreateMode.PERSISTENT);
      } catch (KeeperException.NodeExistsException nee) {
         return;
      } catch (KeeperException.NoNodeException nne) {
   ...
   private static void logRetrievedMsg(final ZooKeeperWatcher zkw, final String znode, final byte[] data,
      ...
      if (!LOG.isDebugEnabled())
         return;
      LOG.debug(zkw.prefix("Retrieved "
            + ((data == null) ? 0 : data.length)
            + " byte(s) of data from znode "
            + znode
            + (watcherSet ? " and set watcher; " : "; data=")
            + (data == null ? "null" : data.length == 0 ? "empty"
                  : (znode.startsWith(zkw.assignmentZNode) ? RegionTransitionData.fromBytes(data).toString()
   ...
   public static void waitForBaseZNode(Configuration conf) throws IOException {
      ...
      ZooKeeper zk = new ZooKeeper(ZKConfig.getZKQuorumServersString(conf), conf.getInt(HConstants.ZK_SESSION_TIMEOUT,
