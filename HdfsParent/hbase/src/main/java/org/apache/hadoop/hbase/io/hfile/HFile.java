...
public class HFile {
   ...
   private static final BlockingQueue<Long> fsReadLatenciesNanos = new ArrayBlockingQueue<Long>(LATENCY_BUFFER_SIZE);

   private static final BlockingQueue<Long> fsWriteLatenciesNanos = new ArrayBlockingQueue<Long>(LATENCY_BUFFER_SIZE);

   private static final BlockingQueue<Long> fsPreadLatenciesNanos = new ArrayBlockingQueue<Long>(LATENCY_BUFFER_SIZE);
   ...
   public static abstract class WriterFactory {
      ...
      public Writer create() throws IOException {
         ...
         return createWriter(fs, path, ostream, blockSize, compression, encoder, comparator, checksumType,
               bytesPerChecksum);
      }

      protected abstract Writer createWriter(FileSystem fs, Path path, FSDataOutputStream ostream, int blockSize,
            Compression.Algorithm compress, HFileDataBlockEncoder dataBlockEncoder, KeyComparator comparator,
            ChecksumType checksumType, int bytesPerChecksum) throws IOException;
   }
    ...
    */
   private static Reader pickReaderVersion(Path path, FSDataInputStream fsdis, FSDataInputStream fsdisNoFsChecksum,
         long size, boolean closeIStream, CacheConfig cacheConf, DataBlockEncoding preferredEncodingInCache,
      ...
      case 2:
         return new HFileReaderV2(path, trailer, fsdis, fsdisNoFsChecksum, size, closeIStream, cacheConf,
   ...
   public static Reader createReaderWithEncoding(FileSystem fs, Path path, CacheConfig cacheConf,
      ...
      return pickReaderVersion(path, fsdis, fsdisNoFsChecksum, fs.getFileStatus(path).getLen(), closeIStream,
    ...
    */
   static Reader createReaderFromStream(Path path, FSDataInputStream fsdis, long size, CacheConfig cacheConf)
         throws IOException {
      final boolean closeIStream = false;
      return pickReaderVersion(path, fsdis, fsdis, size, closeIStream, cacheConf, DataBlockEncoding.NONE, null);
   }
   ...
   public static void checkFormatVersion(int version) throws IllegalArgumentException {
      if (version < MIN_FORMAT_VERSION || version > MAX_FORMAT_VERSION) {
         throw new IllegalArgumentException("Invalid HFile version: " + version + " (expected to be " + "between "
