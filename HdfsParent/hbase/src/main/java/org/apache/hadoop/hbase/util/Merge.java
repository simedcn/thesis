...
public class Merge extends Configured implements Tool {
   ...
   private static class MetaScannerListener implements MetaUtils.ScannerListener {
      ...
      public boolean processRow(HRegionInfo info) {
         ...
         if (region2 != null && meta2 == null && HRegion.rowIsInRange(info, region2)) {
            meta2 = info;
         }
   ...
   private void mergeTwoRegions() throws IOException {
      LOG.info("Merging regions " + Bytes.toStringBinary(this.region1) + " and " + Bytes.toStringBinary(this.region2)
            + " in table " + Bytes.toString(this.tableName));
      ...
      LOG.info("Found meta for region1 " + Bytes.toStringBinary(meta1.getRegionName()) + ", meta for region2 "
      ...
      HTableDescriptor htd = FSTableDescriptors.getTableDescriptor(FileSystem.get(getConf()), this.rootdir,
            this.tableName);
    ...
    */
   private HRegion merge(final HTableDescriptor htd, HRegionInfo info1, HRegion meta1, HRegionInfo info2, HRegion meta2)
         throws IOException {
      if (info1 == null) {
   ...
   private boolean notInTable(final byte[] tn, final byte[] rn) {
      if (WritableComparator.compareBytes(tn, 0, tn.length, rn, 0, tn.length) != 0) {
         LOG.error("Region " + Bytes.toStringBinary(rn) + " does not belong to table " + Bytes.toString(tn));
         return true;
      }
      return false;
   }
   ...
   private void usage() {
      System.err.println("For hadoop 0.20,  Usage: bin/hbase org.apache.hadoop.hbase.util.Merge "
            + "[-Dfs.default.name=hdfs://nn:port] <table-name> <region-1> <region-2>\n");
      System.err.println("For hadoop 0.21+, Usage: bin/hbase org.apache.hadoop.hbase.util.Merge "
