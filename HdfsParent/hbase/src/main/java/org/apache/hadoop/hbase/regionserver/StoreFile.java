...
public class StoreFile extends SchemaConfigured {
    ...
    */
   public StoreFile(final FileSystem fs, final Path p, final Configuration conf, final CacheConfig cacheConf,
         final BloomType cfBloomType, final HFileDataBlockEncoder dataBlockEncoder) throws IOException {
      this.fs = fs;
      this.path = p;
   ...
   public static boolean isReference(final Path p) {
      return !p.getName().startsWith("_") && isReference(p, REF_NAME_PARSER.matcher(p.getName()));
   }
    ...
    */
   static private HDFSBlocksDistribution computeRefFileHDFSBlockDistribution(FileSystem fs, Reference reference,
         Path referencePath) throws IOException {
    ...
    * @return HDFS blocks distribution
    */
   static public HDFSBlocksDistribution computeHDFSBlockDistribution(FileSystem fs, Path p) throws IOException {
      if (isReference(p)) {
   ...
   private void computeHDFSBlockDistribution() throws IOException {
      if (isReference()) {
         this.hdfsBlocksDistribution = computeRefFileHDFSBlockDistribution(this.fs, this.reference, this.referencePath);
      } else {
   ...
   private Reader open() throws IOException {
      ...
      if (isReference()) {
         this.reader = new HalfStoreFileReader(this.fs, this.referencePath, this.cacheConf, this.reference,
   ...
   public String toString() {
      return this.path.toString() + (isReference() ? "-" + this.referencePath + "-" + reference.toString() : "");
   }
   ...
   public static class WriterBuilder {
      ...
      public Writer build() throws IOException {
         ...
         return new Writer(fs, filePath, blockSize, compressAlgo, dataBlockEncoder, conf, cacheConf, comparator,
               bloomType, maxKeyCount, checksumType, bytesPerChecksum);
      }
   }
    ...
    * @throws IOException
    */
   static Path getRandomFilename(final FileSystem fs, final Path dir, final String suffix) throws IOException {
    ...
    * @throws IOException
    */
   static Path split(final FileSystem fs, final Path splitDir, final StoreFile f, final byte[] splitRow,
   ...
   public static class Writer {
      ...
      private Writer(FileSystem fs, Path path, int blocksize, Compression.Algorithm compress,
            ...
            final KVComparator comparator, BloomType bloomType, long maxKeys, final ChecksumType checksumType,
         ...
         writer = HFile.getWriterFactory(conf, cacheConf).withPath(fs, path).withBlockSize(blocksize)
         ...
         generalBloomFilterWriter = BloomFilterFactory.createGeneralBloomAtWrite(conf, cacheConf, bloomType,
         ...
         if (this.bloomType != BloomType.ROWCOL) {
            this.deleteFamilyBloomFilterWriter = BloomFilterFactory.createDeleteBloomAtWrite(conf, cacheConf,
                  (int) Math.min(maxKeys, Integer.MAX_VALUE), writer);
         } else {
         ...
         if (this.generalBloomFilterWriter != null) {
            ...
            if (newKey) {
               ...
               if (lastBloomKey != null
                     && generalBloomFilterWriter.getComparator().compare(bloomKey, bloomKeyOffset, bloomKeyLen,
      ...
      private boolean closeGeneralBloomFilter() throws IOException {
         ...
         if (hasGeneralBloom) {
            ...
            if (lastBloomKey != null) {
               writer.appendFileInfo(LAST_BLOOM_KEY,
                     Arrays.copyOfRange(lastBloomKey, lastBloomKeyOffset, lastBloomKeyOffset + lastBloomKeyLen));
            }
         }
      ...
      public void close() throws IOException {
         ...
         StoreFile.LOG.info((hasGeneralBloom ? "" : "NO ") + "General Bloom and " + (hasDeleteFamilyBloom ? "" : "NO ")
   ...
   public static class Reader extends SchemaConfigured {
      ...
      public Reader(FileSystem fs, Path path, CacheConfig cacheConf, DataBlockEncoding preferredEncodingInCache)
            throws IOException {
         super(path);
       ...
       */
      public boolean passesGeneralBloomFilter(byte[] row, int rowOffset, int rowLen, byte[] col, int colOffset,
            int colLen) {
            ...
            if (shouldCheckBloom) {
               ...
               if (bloomFilterType == BloomType.ROWCOL) {
                  ...
                  if (keyIsAfterLast && bloomFilter.getComparator().compare(rowBloomKey, lastBloomKey) > 0) {
                     ...
                     exists = bloomFilter.contains(key, 0, key.length, bloom)
         ...
         try {
            if (blockType == BlockType.GENERAL_BLOOM_META) {
               ...
               if (bloomMeta != null) {
                  ...
                  if (bloomFilterType == BloomType.NONE) {
                     ...
                     LOG.info("Loaded " + bloomFilterType.toString() + " ("
                           + generalBloomFilter.getClass().getSimpleName() + ") metadata for " + reader.getName());
                  }
               }
               ...
               if (bloomMeta != null) {
                  ...
                  LOG.info("Loaded Delete Family Bloom (" + deleteFamilyBloomFilter.getClass().getSimpleName()
   ...
   abstract static class Comparators {
       ...
       */
      static final Comparator<StoreFile> FLUSH_TIME = Ordering.compound(ImmutableList.of(
            Ordering.natural().onResultOf(new GetBulkTime()), Ordering.natural().onResultOf(new GetSeqId()), Ordering
