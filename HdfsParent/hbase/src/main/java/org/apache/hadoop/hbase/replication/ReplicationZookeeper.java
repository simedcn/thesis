...
public class ReplicationZookeeper {
    ...
    * @throws IOException
    */
   public ReplicationZookeeper(final Abortable abortable, final Configuration conf, final ZooKeeperWatcher zk)
    ...
    * @throws KeeperException 
    */
   public ReplicationZookeeper(final Server server, final AtomicBoolean replicating) throws IOException,
         KeeperException {
   ...
   public Map<String, String> listPeers() {
      ...
      try {
         ...
         for (String id : ids) {
            peers.put(id, Bytes.toString(ZKUtil.getData(this.zookeeper, ZKUtil.joinZNode(this.peersZNode, id))));
         }
      } catch (KeeperException e) {
         this.abortable.abort("Cannot get the list of peers ", e);
      }
   ...
   public void setReplicating(boolean newState) throws KeeperException {
      ZKUtil.createWithParents(this.zookeeper, ZKUtil.joinZNode(this.replicationZNode, this.replicationStateNodeName));
      ZKUtil.setData(this.zookeeper, ZKUtil.joinZNode(this.replicationZNode, this.replicationStateNodeName),
