...
public class MetaScanner {
   ...
   public static void metaScan(Configuration configuration, MetaScannerVisitor visitor, byte[] userTableName,
      ...
      metaScan(configuration, visitor, userTableName, row, rowLimit, HConstants.META_TABLE_NAME);
   }
    ...
    * @throws IOException e
    */
   public static void metaScan(Configuration configuration, final MetaScannerVisitor visitor, final byte[] tableName,
         final byte[] row, final int rowLimit, final byte[] metaTableName) throws IOException {
      try {
         HConnectionManager.execute(new HConnectable<Void>(configuration) {
            @Override
            public Void connect(HConnection connection) throws IOException {
               metaScan(conf, connection, visitor, tableName, row, rowLimit, metaTableName);
               return null;
            }
         });
      } finally {
   ...
   private static void metaScan(Configuration configuration, HConnection connection, MetaScannerVisitor visitor,
         byte[] tableName, byte[] row, int rowLimit, final byte[] metaTableName) throws IOException {
      ...
      if (row != null) {
         ...
         try {
            ...
            if (startRowResult == null) {
               throw new TableNotFoundException("Cannot find row in .META. for table: " + Bytes.toString(tableName)
                     + ", row=" + Bytes.toStringBinary(searchRow));
            }
            byte[] value = startRowResult.getValue(HConstants.CATALOG_FAMILY, HConstants.REGIONINFO_QUALIFIER);
            if (value == null || value.length == 0) {
               throw new IOException("HRegionInfo was null or empty in Meta for " + Bytes.toString(tableName)
                     + ", row=" + Bytes.toStringBinary(searchRow));
            }
      ...
      int rows = Math.min(rowLimit,
            configuration.getInt(HConstants.HBASE_META_SCANNER_CACHING, HConstants.DEFAULT_HBASE_META_SCANNER_CACHING));
      do {
         final Scan scan = new Scan(startRow).addFamily(HConstants.CATALOG_FAMILY);
         if (LOG.isDebugEnabled()) {
            LOG.debug("Scanning " + Bytes.toString(metaTableName) + " starting at row="
                  + Bytes.toStringBinary(startRow) + " for max=" + rowUpperLimit + " rows using "
                  + connection.toString());
         }
    ...
    * @throws IOException
    */
   public static NavigableMap<HRegionInfo, ServerName> allTableRegions(Configuration conf, final byte[] tablename,
   ...
   public static abstract class BlockingMetaScannerVisitor extends MetaScannerVisitorBase {
      ...
      @Override
      public boolean processRow(Result rowResult) throws IOException {
         HRegionInfo info = Writables.getHRegionInfoOrNull(rowResult.getValue(HConstants.CATALOG_FAMILY,
               HConstants.REGIONINFO_QUALIFIER));
         if (info == null) {
            return true;
         }
         ...
         if (info.isSplitParent()) {
            ...
            if (resultA != null) {
               ...
               throw new RegionOfflineException("Split daughter region " + splitA.getRegionNameAsString()
            ...
            if (resultB != null) {
               ...
               throw new RegionOfflineException("Split daughter region " + splitB.getRegionNameAsString()
                     + " cannot be found in META.");
            }
         }
      ...
      private Result getRegionResultBlocking(HTable metaTable, long timeout, byte[] regionName) throws IOException {
         ...
         while (System.currentTimeMillis() - start < timeout) {
            ...
            HRegionInfo info = Writables.getHRegionInfoOrNull(result.getValue(HConstants.CATALOG_FAMILY,
                  HConstants.REGIONINFO_QUALIFIER));
            if (info != null) {
               return result;
            }
