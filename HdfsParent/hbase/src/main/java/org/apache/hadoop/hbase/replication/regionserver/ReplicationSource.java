...
public class ReplicationSource extends Thread implements ReplicationSourceInterface {
   ...
   public void run() {
      ...
      if (this.queueRecovered) {
         try {
            this.repLogReader.setPosition(this.zkHelper.getHLogRepPosition(this.peerClusterZnode, this.queue.peek()
                  .getName()));
      ...
      while (entry != null) {
         ...
         if (!logKey.getClusterId().equals(peerClusterId)) {
            ...
            if (!(Bytes.equals(logKey.getTablename(), HConstants.ROOT_TABLE_NAME) || Bytes.equals(
                  logKey.getTablename(), HConstants.META_TABLE_NAME)) && edit.size() != 0 && replicating.get()) {
