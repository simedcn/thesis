...
public class MetaMigrationRemovingHTD {
    ...
    * @throws IOException
    */
   public static Set<HTableDescriptor> updateMetaWithNewRegionInfo(final MasterServices masterServices)
         throws IOException {
    ...
    * @throws IOException
    */
   static Set<HTableDescriptor> updateRootWithNewRegionInfo(final MasterServices masterServices) throws IOException {
      MigratingVisitor v = new MigratingVisitor(masterServices);
   ...
   static void checkSplit(final Result r, final Put p, final byte[] which) throws IOException {
    ...
    * @throws IOException
    */
   static HRegionInfo090x get090HRI(final Result r, final byte[] qualifier) throws IOException {
      byte[] hriBytes = r.getValue(HConstants.CATALOG_FAMILY, qualifier);
      if (hriBytes == null || hriBytes.length <= 0)
         return null;
      if (isMigrated(hriBytes))
   ...
   static boolean isMigrated(final byte[] hriBytes) {
      ...
      if (hriBytes[0] == HRegionInfo.VERSION)
         return true;
      if (hriBytes[0] == HRegionInfo.VERSION_PRE_092)
         return false;
    ...
    * @throws IOException
    */
   public static void migrateRootAndMeta(final MasterServices masterServices) throws IOException {
    ...
    * @throws IOException
    */
   public static void updateRootWithMetaMigrationStatus(final CatalogTracker catalogTracker) throws IOException {
   ...
   static Put setMetaVersion(final Put p) {
      p.add(HConstants.CATALOG_FAMILY, HConstants.META_VERSION_QUALIFIER, Bytes.toBytes(HConstants.META_VERSION));
      return p;
   }
   ...
   public static boolean isMetaHRIUpdated(final MasterServices services) throws IOException {
    ...
    * @throws IOException
    */
   public static boolean updateMetaWithNewHRI(final MasterServices services) throws IOException {
   ...
   public static HRegionInfo090x getHRegionInfo090x(final byte[] bytes) {
      ...
      try {
         ...
         LOG.warn("Failed deserialize as a 090 HRegionInfo); bytes=" + Bytes.toStringBinary(bytes), ioe);
      }
      return hri;
   }
}
