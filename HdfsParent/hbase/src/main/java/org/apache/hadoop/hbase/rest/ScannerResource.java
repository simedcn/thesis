...
public class ScannerResource extends ResourceBase {
   ...
   static final Map<String, ScannerInstanceResource> scanners = Collections
         .synchronizedMap(new HashMap<String, ScannerInstanceResource>());
   ...
   Response update(final ScannerModel model, final boolean replace, final UriInfo uriInfo) {
      ...
      RowSpec spec = new RowSpec(model.getStartRow(), endRow, model.getColumns(), model.getStartTime(),
   ...
   @Path("{scanner: .+}")
   public ScannerInstanceResource getScannerInstanceResource(final @PathParam("scanner") String id) throws IOException {
