...
public class Exec extends Invocation implements Row {
   ...
   public Exec(Configuration configuration, Class<? extends CoprocessorProtocol> protocol, Method method,
         Object[] parameters) {
      this(configuration, HConstants.EMPTY_BYTE_ARRAY, protocol, method, parameters);
   }

   public Exec(Configuration configuration, byte[] row, Class<? extends CoprocessorProtocol> protocol, Method method,
         Object[] parameters) {
   ...
   public void write(DataOutput out) throws IOException {
      ...
      for (int i = 0; i < parameterClasses.length; i++) {
         HbaseObjectWritable.writeObject(out, parameters[i], parameters[i] != null ? parameters[i].getClass()
