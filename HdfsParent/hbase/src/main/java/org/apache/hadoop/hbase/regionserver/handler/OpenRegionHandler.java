...
public class OpenRegionHandler extends EventHandler {
   ...
   public OpenRegionHandler(final Server server, final RegionServerServices rsServices, HRegionInfo regionInfo,
   ...
   protected OpenRegionHandler(final Server server, final RegionServerServices rsServices,
         final HRegionInfo regionInfo, final HTableDescriptor htd, EventType eventType, final int versionOfOfflineNode) {
   ...
   boolean updateMeta(final HRegion r) {
      ...
      t.start();
      int assignmentTimeout = this.server.getConfiguration().getInt("hbase.master.assignment.timeoutmonitor.period",
            10000);
   ...
   static class PostOpenDeployTasksThread extends Thread {
      ...
      PostOpenDeployTasksThread(final HRegion region, final Server server, final RegionServerServices services,
   ...
   private boolean transitionToOpened(final HRegion r) throws IOException {
      ...
      try {
         if (ZKAssign.transitionNodeOpened(this.server.getZooKeeper(), hri, this.server.getServerName(), this.version) == -1) {
            LOG.warn("Completed the OPEN of region " + name + " but when transitioning from "
            ...
            LOG.debug("region transitioned to opened in zookeeper: " + r.getRegionInfo() + ", server: "
   ...
   HRegion openRegion() {
      ...
      try {
         ...
         region = HRegion.openHRegion(this.regionInfo, this.htd, this.rsServices.getWAL(),
         ...
         LOG.error("Failed open of region=" + this.regionInfo.getRegionNameAsString()
   ...
   boolean tickleOpening(final String context) {
      ...
      try {
         this.version = ZKAssign.retransitionNodeOpening(server.getZooKeeper(), this.regionInfo,
