...
public class BlockTokenSecretManager extends SecretManager<BlockTokenIdentifier> {
   ...
   private synchronized void generateKeys() {
       ...
       */
      serialNo++;
      currentKey = new BlockKey(serialNo, System.currentTimeMillis() + 2 * keyUpdateInterval + tokenLifetime,
            generateSecret());
      serialNo++;
      nextKey = new BlockKey(serialNo, System.currentTimeMillis() + 3 * keyUpdateInterval + tokenLifetime,
            generateSecret());
   ...
   public synchronized ExportedBlockKeys exportKeys() {
      ...
      return new ExportedBlockKeys(true, keyUpdateInterval, tokenLifetime, currentKey, allKeys.values().toArray(
            new BlockKey[0]));
   }
   ...
   public synchronized void updateKeys() throws IOException {
      ...
      allKeys.put(currentKey.getKeyId(), new BlockKey(currentKey.getKeyId(), System.currentTimeMillis()
            + keyUpdateInterval + tokenLifetime, currentKey.getKey()));
      // update the estimated expiry date of new currentKey
      currentKey = new BlockKey(nextKey.getKeyId(), System.currentTimeMillis() + 2 * keyUpdateInterval + tokenLifetime,
            nextKey.getKey());
      allKeys.put(currentKey.getKeyId(), currentKey);
    ...
    */
   public void checkAccess(BlockTokenIdentifier id, String userId, Block block, AccessMode mode) throws InvalidToken {
      if (LOG.isDebugEnabled()) {
   ...
   public void checkAccess(Token<BlockTokenIdentifier> token, String userId, Block block, AccessMode mode)
      ...
      try {
         ...
         throw new InvalidToken("Unable to de-serialize block token identifier for user=" + userId + ", block=" + block
               + ", access mode=" + mode);
      }
