...
public class DFSClient implements FSConstants, java.io.Closeable {
	...
	static ClientDatanodeProtocol createClientDatanodeProtocolProxy(
			DatanodeID datanodeid, Configuration conf, Block block,
			Token<BlockTokenIdentifier> token, int socketTimeout)
			throws IOException {
		InetSocketAddress addr = NetUtils.makeSocketAddr(datanodeid.getHost(),
		...
		return (ClientDatanodeProtocol) RPC.getProxy(
				...
				addr, ticket, conf, NetUtils.getDefaultSocketFactory(conf),
				socketTimeout);
	}
	...
	static ClientDatanodeProtocol createClientDatanodeProtocolProxy(
		...
		return (ClientDatanodeProtocol) RPC.getProxy(
				ClientDatanodeProtocol.class, ClientDatanodeProtocol.versionID,
				addr, conf, NetUtils.getDefaultSocketFactory(conf),
				socketTimeout);
	}
	...
	private static BlockReader getLocalBlockReader(Configuration conf,
		...
		try {
			return BlockReaderLocal.newBlockReader(conf, src, blk, accessToken,
					chosenNode, socketTimeout, offsetIntoBlock,
	...
	private static boolean tokenRefetchNeeded(IOException ex,
		...
		if (ex instanceof InvalidBlockTokenException
			...
			LOG.info("Access token was invalid when connecting to "
					+ targetAddr + " : " + ex);
			return true;
		}
		return false;
	...
	public static class Renewer extends TokenRenewer {
		...
		public long renew(Token<?> token, Configuration conf)
			...
			ClientProtocol nn = createRPCNamenode(addr, conf,
					UserGroupInformation.getCurrentUser());
			try {
	...
	public BlockLocation[] getBlockLocations(String src, long start, long length)
			throws IOException {
		LocatedBlocks blocks = callGetBlockLocations(namenode, src, start,
				length);
	...
	private DFSOutputStream append(String src, int buffersize,
		...
		final DFSOutputStream result = new DFSOutputStream(src, buffersize,
				progress, lastBlock, stat, conf.getInt("io.bytes.per.checksum",
						512));
		leasechecker.put(src, result);
		return result;
	}
	...
	public static MD5MD5CRC32FileChecksum getFileChecksum(String src,
			ClientProtocol namenode, SocketFactory socketFactory,
   ...
   */
	...
	void setQuota(String src, long namespaceQuota, long diskspaceQuota)
		...
		// sanity check
		if ((namespaceQuota <= 0
				&& namespaceQuota != FSConstants.QUOTA_DONT_SET && namespaceQuota != FSConstants.QUOTA_RESET)
				|| (diskspaceQuota <= 0
						&& diskspaceQuota != FSConstants.QUOTA_DONT_SET && diskspaceQuota != FSConstants.QUOTA_RESET)) {
	...
	public static class BlockReader extends FSInputChecker {
		...
		public static BlockReader newBlockReader(Socket sock, String file,
				...
				long genStamp, long startOffset, long len, int bufferSize)
				throws IOException {
		...
		public static BlockReader newBlockReader(Socket sock, String file,
				long blockId, Token<BlockTokenIdentifier> accessToken,
				long genStamp, long startOffset, long len, int bufferSize,
				boolean verifyChecksum, String clientName) throws IOException {
				...
				if (status == DataTransferProtocol.OP_STATUS_ERROR_ACCESS_TOKEN) {
					throw new InvalidBlockTokenException(
							"Got access token error for OP_READ_BLOCK, self="
									...
									+ ", for file " + file + ", for block "
									+ blockId + "_" + genStamp);
				} else {
					throw new IOException("Got error for OP_READ_BLOCK, self="
							+ sock.getLocalSocketAddress() + ", remote="
							+ sock.getRemoteSocketAddress() + ", for file "
							+ file + ", for block " + blockId + "_" + genStamp);
				}
			}
		...
		private void checksumOk(Socket sock) {
			try {
				OutputStream out = NetUtils.getOutputStream(sock,
						HdfsConstants.WRITE_TIMEOUT);
				byte buf[] = {
						(DataTransferProtocol.OP_STATUS_CHECKSUM_OK >>> 8) & 0xff,
	...
	public class DFSInputStream extends FSInputStream {
		 ...
		 * during the retry logic, the failure count exceeds a threshold, the
		 * errors will be thrown back to the operation.
		 * 
		 * Specifically this counts the number of times the client has gone back
		 * to the namenode to get a new list of block locations, and is capped
		 * at maxBlockAcquireFailures
		 */
		...
		synchronized void openInfo() throws IOException {
			...
			if (locatedBlocks != null && !locatedBlocks.isUnderConstruction()
				...
				while (oldIter.hasNext() && newIter.hasNext()) {
					if (!oldIter.next().getBlock()
							.equals(newIter.next().getBlock())) {
		...
		/**
		 * For files under construction, update the last block size based on the
		 * length of the block from the datanode.
		 */
		private void updateBlockInfo(LocatedBlocks newInfo) {
			...
			boolean lastBlockInFile = (last.getStartOffset()
					+ last.getBlockSize() == newInfo.getFileLength());
			...
			try {
				...
				LOG.debug("DFSClient setting last block " + last
						+ " to length " + newBlockSize + " filesize is now "
		...
		private synchronized void fetchBlockAt(long offset) throws IOException {
			...
			newBlocks = callGetBlockLocations(namenode, src, offset,
					prefetchSize);
			if (newBlocks == null) {
				throw new IOException("Could not find target position "
						+ offset);
			}
			locatedBlocks.insertRange(targetBlockIdx,
					newBlocks.getLocatedBlocks());
		...
		private synchronized List<LocatedBlock> getBlockRange(long offset,
			...
			while (remaining > 0) {
				...
				if (blk == null || curOff < blk.getStartOffset()) {
					LocatedBlocks newBlocks;
					newBlocks = callGetBlockLocations(namenode, src, curOff,
							remaining);
					locatedBlocks.insertRange(blockIdx,
							newBlocks.getLocatedBlocks());
					continue;
				}
		...
		private synchronized DatanodeInfo blockSeekTo(long target)
			...
			while (true) {
				...
				if (shouldTryShortCircuitRead(targetAddr)) {
					try {
						blockReader = getLocalBlockReader(conf, src, blk,
								accessToken, chosenNode,
								DFSClient.this.socketTimeout, offsetIntoBlock);
						return chosenNode;
						...
						if (refetchToken > 0
							...
							LOG.info("Failed to read block "
									+ targetBlock.getBlock()
									+ " on local machine"
									+ StringUtils.stringifyException(ex));
							LOG.info("Try reading via the datanode on "
									+ targetAddr);
						}
					}
				}
				...
				try {
					...
					blockReader = BlockReader.newBlockReader(s, src,
							blk.getBlockId(), accessToken,
							blk.getGenerationStamp(), offsetIntoBlock,
							blk.getNumBytes() - offsetIntoBlock, buffersize,
		...
		private DNAddrPair chooseDataNode(LocatedBlock block)
			...
			while (true) {
				...
				try {
					...
					LOG.info("Could not obtain block "
							+ block.getBlock()
							+ " from any node: "
							+ ie
		...
		private void fetchBlockByteRange(LocatedBlock block, long start,
			...
			while (true) {
				...
				try {
					Token<BlockTokenIdentifier> accessToken = block
							.getBlockToken();
					// first try reading the block locally.
					if (shouldTryShortCircuitRead(targetAddr)) {
						try {
							reader = getLocalBlockReader(conf, src,
									block.getBlock(), accessToken, chosenNode,
									DFSClient.this.socketTimeout, start);
						} catch (AccessControlException ex) {
							LOG.warn("Short circuit access failed ", ex);
							// Disable short circuit reads
							shortCircuitLocalReads = false;
						...
						reader = BlockReader.newBlockReader(dn, src, block
								.getBlock().getBlockId(), accessToken, block
								.getBlock().getGenerationStamp(), start, len,
			...
			if (pos <= targetPos && targetPos <= blockEnd) {
				...
				if (diff <= TCP_WINDOW_SIZE) {
					try {
						...
						LOG.debug("Exception while seek to " + targetPos
								+ " from " + currentBlock + " of " + src
								+ " from " + currentNode + ": "
								+ StringUtils.stringifyException(e));
					}
				}
	...
	class DFSOutputStream extends FSOutputSummer implements Syncable {
				...
				while (!closed && clientRunning) {
					...
					synchronized (dataQueue) {
						...
						while ((!closed && !hasError && clientRunning
								&& dataQueue.size() == 0 && (blockStream == null || (blockStream != null && now
								- lastPacket < timeoutValue / 2)))
								|| doSleep) {
							long timeout = timeoutValue / 2
									- (now - lastPacket);
			...
			public void run() {
				...
				while (!closed && clientRunning && !lastPacketInBlock) {
					...
					try {
						...
						if (LOG.isDebugEnabled()) {
							LOG.debug("DFSClient for block " + block + " "
									+ ack);
						}
		...
		private boolean processDatanodeError(boolean hasError, boolean isAppend) {
			...
			if (errorIndex >= 0) {
				LOG.warn("Error Recovery for block "
						...
						+ errorIndex
						+ "] "
						+ (nodes == null ? "nodes == null" : nodes[errorIndex]
								.getName()));
			}
			...
			while (!success && clientRunning) {
				...
				if (errorIndex < 0) {
					...
					LOG.warn("Error Recovery for block " + block
							+ " in pipeline " + pipelineMsg + ": bad datanode "
							+ nodes[errorIndex].getName());
					newnodes = new DatanodeInfo[nodes.length - 1];
					System.arraycopy(nodes, 0, newnodes, 0, errorIndex);
					System.arraycopy(nodes, errorIndex + 1, newnodes,
							errorIndex, newnodes.length - errorIndex);
				}
				...
				try {
					...
					primary = createClientDatanodeProtocolProxy(primaryNode,
							conf, block, accessToken, socketTimeout);
		...
		private DFSOutputStream(String src, long blockSize,
		...
		DFSOutputStream(String src, int buffersize, Progressable progress,
			...
			if (lastBlock != null) {
				...
				if (freeInLastBlock > blockSize) {
					throw new IOException("The last block for file " + src
							+ " is full.");
				}
				...
				if (usedInCksum > 0 && freeInCksum > 0) {
					...
					computePacketChunkSize(
							Math.min(writePacketSize, freeInLastBlock),
							bytesPerChecksum);
				}
		...
		private void computePacketChunkSize(int psize, int csize) {
			...
			chunksPerPacket = Math.max((psize - n + chunkSize - 1) / chunkSize,
					1);
		...
		private DatanodeInfo[] nextBlockOutputStream(String client)
			...
			do {
				...
				DatanodeInfo[] excluded = excludedNodes
			...
			try {
				...
				if (pipelineStatus != DataTransferProtocol.OP_STATUS_SUCCESS) {
					if (pipelineStatus == DataTransferProtocol.OP_STATUS_ERROR_ACCESS_TOKEN) {
						...
						throw new IOException(
								"Bad connect ack with firstBadLink as "
										+ firstBadLink);
					}
				}
				...
				LOG.info("Exception in createBlockOutputStream "
						+ nodes[0].getName() + " " + ie);
				...
				if (currentPacket.numChunks == currentPacket.maxChunks
					...
					if (LOG.isDebugEnabled()) {
						LOG.debug("DFSClient writeChunk packet full seqno="
								+ currentPacket.seqno + ", src=" + src
								+ ", bytesCurBlock=" + bytesCurBlock
								+ ", blockSize=" + blockSize + ", appendChunk="
								+ appendChunk);
					}
					//
		...
		public void sync() throws IOException {
			...
			try {
				...
				synchronized (this) {
					 ...
					 * flushed. After the flush, reset the bytesCurBlock back to
					 * its previous value, any partial checksum chunk will be
					 * sent now and in next packet.
					 */
					...
					try {
						...
						DFSClient.LOG
								.warn("Unable to persist blocks in hflush for "
										+ src, ioe);
						// If we got an error here, it might be because some
						// other thread called
						...
						// If we aren't closed but failed to sync, we should
						// expose that to the
						// caller.
						throw ioe;
					}
				...
				synchronized (this) {
					if (!closed) {
						lastException = new IOException("IOException flush:"
								+ e);
						closed = true;
						closeThreads();
					}
		...
		/**
		 * Returns the number of replicas of current block. This can be
		 * different from the designated replication factor of the file because
		 * the NameNode does not replicate the block to which a client is
		 * currently writing to. The client continues to write to a block even
		 * if a few datanodes in the write pipeline have failed. If the current
		 * block is full and the next block is not yet allocated, then this API
		 * will return 0 because there are no replicas in the pipeline.
		 */
		...
		/**
		 * Harsh abort method that should only be used from tests - this is in
		 * order to prevent pipeline recovery when eg a DN shuts down.
		 */
