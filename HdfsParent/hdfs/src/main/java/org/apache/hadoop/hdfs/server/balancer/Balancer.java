...
public class Balancer implements Tool {
                  ...
                  if (chooseProxySource()) {
                     ...
                     if (LOG.isDebugEnabled()) {
                        LOG.debug("Decided to move block " + block.getBlockId() + " with a length of "
                              + StringUtils.byteDesc(block.getNumBytes()) + " bytes from " + source.getName() + " to "
                              + target.getName() + " using proxy source " + proxySource.getName());
                     }
                     return true;
                  }
               }
   ...
   private boolean chooseTarget(Source source, Iterator<BalancerDatanode> targetCandidates, boolean onRackTarget) {
      ...
      if (foundTarget) {
         ...
         LOG.info("Decided to move " + StringUtils.byteDesc(size) + " bytes from " + source.datanode.getName() + " to "
      ...
      try {
         ...
         while (true) {
            ...
            formatter.format("%-24s %10d  %19s  %18s  %17s\n", DateFormat.getDateTimeInstance().format(new Date()),
                  iterations, StringUtils.byteDesc(bytesMoved.get()), StringUtils.byteDesc(bytesLeftToMove),
