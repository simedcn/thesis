...
public class FSEditLog {
   ...
   synchronized void removeEditsForStorageDir(StorageDirectory sd) {
      ...
      for (int idx = 0; idx < editStreams.size(); idx++) {
         ...
         if (parentDir.getAbsolutePath().equals(sd.getRoot().getAbsolutePath())) {
            editStreams.remove(idx);
            idx--;
         }
      }
   ...
   static int loadFSEdits(EditLogInputStream edits) throws IOException {
      ...
      String path = null;
      int numOpAdd = 0, numOpClose = 0, numOpDelete = 0, numOpRename = 0, numOpSetRepl = 0, numOpMkDir = 0, numOpSetPerm = 0, numOpSetOwner = 0, numOpSetGenStamp = 0, numOpTimes = 0, numOpGetDelegationToken = 0, numOpRenewDelegationToken = 0, numOpCancelDelegationToken = 0, numOpUpdateMasterKey = 0, numOpOther = 0;
         ...
         while (true) {
            ...
            try {
               ...
               if (opcode == OP_INVALID) {
                  FSNamesystem.LOG.info("Invalid opcode, reached end of edit log " + "Number of transactions found "
                        + numEdits);
            ...
            case OP_CLOSE: {
               ...
               if (FSNamesystem.LOG.isDebugEnabled()) {
                  FSNamesystem.LOG.debug(opcode + ": " + path + " numblocks : " + blocks.length + " clientHolder "
               ...
               INodeFile node = (INodeFile) fsDir.unprotectedAddFile(path, permissions, blocks, replication, mtime,
                     atime, blockSize);
               if (opcode == OP_ADD) {
                  numOpAdd++;
                  //
      ...
      if (FSImage.LOG.isDebugEnabled()) {
         FSImage.LOG.debug("numOpAdd = " + numOpAdd + " numOpClose = " + numOpClose + " numOpDelete = " + numOpDelete
               + " numOpRename = " + numOpRename + " numOpSetRepl = " + numOpSetRepl + " numOpMkDir = " + numOpMkDir
               + " numOpSetPerm = " + numOpSetPerm + " numOpSetOwner = " + numOpSetOwner + " numOpSetGenStamp = "
               + numOpSetGenStamp + " numOpTimes = " + numOpTimes + " numOpGetDelegationToken = "
               + numOpGetDelegationToken + " numOpRenewDelegationToken = " + numOpRenewDelegationToken
               + " numOpCancelDelegationToken = " + numOpCancelDelegationToken + " numOpUpdateMasterKey = "
   ...
   synchronized void rollEditLog() throws IOException {
      ...
      if (existsNew()) {
         ...
         while (it.hasNext()) {
            ...
            b.append("\n  ").append(editsNew);
            if (!editsNew.exists()) {
               throw new IOException("Inconsistent existence of edits.new " + editsNew);
            }
         }
         FSNamesystem.LOG.warn("Cannot roll edit log," + " edits.new files already exists in all healthy directories:"
               + b);
         return;
      }
      ...
      while (it.hasNext()) {
         ...
         try {
            ...
            FSImage.LOG.error("error retrying to reopen storage directory '" + sd.getRoot().getAbsolutePath() + "'",
                  ioe);
            toRemove.add(sd);
            it.remove();
         }
