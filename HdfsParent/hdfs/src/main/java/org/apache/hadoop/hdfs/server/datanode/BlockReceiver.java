...
class BlockReceiver implements java.io.Closeable, FSConstants {
   ...
   private int readNextPacket() throws IOException {
      ...
      if (buf == null) {
         ...
         int chunksPerPacket = (datanode.writePacketSize - DataNode.PKT_HEADER_LEN - SIZE_OF_INTEGER + chunkSize - 1)
               / chunkSize;
         buf = ByteBuffer
               .allocate(DataNode.PKT_HEADER_LEN + SIZE_OF_INTEGER + Math.max(chunksPerPacket, 1) * chunkSize);
         buf.limit(0);
      }
               ...
               if (partialCrc != null) {
                  if (len > bytesPerChecksum) {
                     throw new IOException("Got wrong length during writeBlock(" + block + ") from " + inAddr + " "
                           + "A packet can have only one partial chunk." + " len = " + len + " bytesPerChecksum "
                           + bytesPerChecksum);
                  }
   ...
   private void computePartialChunkCrc(long blkoff, long ckoff, int bytesPerChecksum) throws IOException {
      ...
      LOG.info("computePartialChunkCrc sizePartialChunk " + sizePartialChunk + " block " + block + " offset in block "
            + blkoff + " offset in metafile " + ckoff);
               ...
               try {
                  ...
                  synchronized (this) {
                     ...
                     while (running && datanode.shouldRun && ackQueue.size() == 0) {
                        if (LOG.isDebugEnabled()) {
                           LOG.debug("PacketResponder " + numTargets + " seqno = " + seqno + " for block " + block
                  ...
                  if (numTargets > 0 && !localMirrorError) {
                     ...
                     if (LOG.isDebugEnabled()) {
                        LOG.debug("PacketResponder " + numTargets + " for block " + block + " got " + ack);
                     }
                     seqno = ack.getSeqno();
                     // verify seqno
                     if (seqno != expected) {
               ...
               if (lastPacketInBlock && !receiver.finalized) {
                  ...
                  if (ClientTraceLog.isInfoEnabled() && receiver.clientName.length() > 0) {
                     ...
                     ClientTraceLog.info(String.format(DN_CLIENTTRACE_FORMAT, receiver.inAddr, receiver.myAddr,
                           block.getNumBytes(), "HDFS_WRITE", receiver.clientName, offset,
