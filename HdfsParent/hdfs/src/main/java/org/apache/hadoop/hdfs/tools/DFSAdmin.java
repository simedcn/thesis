...
public class DFSAdmin extends FsShell {
   ...
   public void report() throws IOException {
      if (fs instanceof DistributedFileSystem) {
         ...
         System.out
               .println("Present Capacity: " + presentCapacity + " (" + StringUtils.byteDesc(presentCapacity) + ")");
   ...
   public int refreshServiceAcl() throws IOException {
      ...
      // Create the client
      RefreshAuthorizationPolicyProtocol refreshProtocol = (RefreshAuthorizationPolicyProtocol) RPC.getProxy(
            RefreshAuthorizationPolicyProtocol.class, RefreshAuthorizationPolicyProtocol.versionID,
   ...
   public int refreshUserToGroupsMappings() throws IOException {
      ...
      // Create the client
      RefreshUserMappingsProtocol refreshProtocol = (RefreshUserMappingsProtocol) RPC.getProxy(
            RefreshUserMappingsProtocol.class, RefreshUserMappingsProtocol.versionID, NameNode.getAddress(conf),
            getUGI(), conf, NetUtils.getSocketFactory(conf, RefreshUserMappingsProtocol.class));
