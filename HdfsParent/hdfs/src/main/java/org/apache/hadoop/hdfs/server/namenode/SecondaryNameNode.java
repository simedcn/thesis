...
public class SecondaryNameNode implements Runnable {
   ...
   private void initialize(final Configuration conf) throws IOException {
      ...
      UserGroupInformation httpUGI = UserGroupInformation.loginUserFromKeytabAndReturnUGI(
            SecurityUtil.getServerPrincipal(conf.get(DFSConfigKeys.DFS_SECONDARY_NAMENODE_KRB_HTTPS_USER_NAME_KEY),
                  infoBindAddress), conf.get(DFSConfigKeys.DFS_SECONDARY_NAMENODE_KEYTAB_FILE_KEY));
      try {
   ...
   private void putFSImage(CheckpointSignature sig) throws IOException {
      String fileid = "putimage=1&port=" + imagePort + "&machine=" + infoBindAddress + "&token=" + sig.toString();
