...
abstract class DfsServlet extends HttpServlet {
   ...
   protected ClientProtocol createNameNodeProxy() throws IOException {
      ...
      Configuration conf = new Configuration((Configuration) context.getAttribute(JspHelper.CURRENT_CONF));
   ...
   protected URI createRedirectUri(String servletpath, UserGroupInformation ugi, DatanodeID host,
      ...
      return new URI(scheme, null, hostname, port, servletpath, "filename=" + filename + "&ugi="
            + ugi.getShortUserName() + dt, null);
   }
