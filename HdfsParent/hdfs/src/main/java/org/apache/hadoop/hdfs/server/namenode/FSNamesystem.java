...
public class FSNamesystem implements FSConstants, FSNamesystemMBean, NameNodeMXBean, MetricsSource {
   ...
   private void initialize(NameNode nn, Configuration conf) throws IOException {
      ...
      this.dnthread = new Daemon(new DecommissionManager(this).new Monitor(conf.getInt(
            "dfs.namenode.decommission.interval", 30), conf.getInt("dfs.namenode.decommission.nodes.per.interval", 5)));
      dnthread.start();
   ...
   private void setConfigurationParameters(Configuration conf) throws IOException {
      ...
      this.defaultPermission = PermissionStatus.createImmutable(fsOwner.getShortUserName(), supergroup,
            new FsPermission(filePermission));
      ...
      if (isAccessTokenEnabled) {
         this.accessKeyUpdateInterval = conf.getLong(DFSConfigKeys.DFS_BLOCK_ACCESS_KEY_UPDATE_INTERVAL_KEY, 600) * 60 * 1000L; // 10 hrs
         this.accessTokenLifetime = conf.getLong(DFSConfigKeys.DFS_BLOCK_ACCESS_TOKEN_LIFETIME_KEY, 600) * 60 * 1000L; // 10 hrs
      }
   ...
   synchronized void metaSave(String filename) throws IOException {
      ...
      String str = totalInodes + " files and directories, " + totalBlocks + " blocks = " + (totalInodes + totalBlocks)
            + " total";
      out.println(str);
   ...
   public LocatedBlocks getBlockLocations(String src, long offset, long length, boolean doAccessTime,
      ...
      final LocatedBlocks ret = getBlockLocationsInternal(src, offset, length, Integer.MAX_VALUE, doAccessTime,
            needBlockToken);
      if (auditLog.isInfoEnabled() && isExternalInvocation()) {
   ...
   private synchronized LocatedBlocks getBlockLocationsInternal(String src, long offset, long length,
      ...
      do {
         ...
         if (inode.isUnderConstruction() && curBlk == blocks.length - 1 && blocksMap.numNodes(blocks[curBlk]) == 0) {
   ...
   private synchronized void startFileInternal(String src, PermissionStatus permissions, String holder,
         String clientMachine, boolean overwrite, boolean append, boolean createParent, short replication,
         long blockSize) throws IOException {
      if (NameNode.stateChangeLog.isDebugEnabled()) {
         NameNode.stateChangeLog.debug("DIR* NameSystem.startFile: src=" + src + ", holder=" + holder
               + ", clientMachine=" + clientMachine + ", createParent=" + createParent + ", replication=" + replication
      ...
      try {
         ...
         if (append) {
            ...
            INodeFileUnderConstruction cons = new INodeFileUnderConstruction(node.getLocalNameBytes(),
                  node.getReplication(), node.getModificationTime(), node.getPreferredBlockSize(), node.getBlocks(),
                  node.getPermissionStatus(), holder, clientMachine, clientNode);
      ...
      if (fileInode != null && fileInode.isUnderConstruction()) {
         ...
         if (!force && lease != null) {
            ...
            if (leaseFile != null && leaseFile.equals(lease)) {
               throw new AlreadyBeingCreatedException("failed to create file " + src + " for " + holder + " on client "
                     + clientMachine + " because current leaseholder is trying to recreate file.");
            }
         }
         //
         ...
         if (lease == null) {
            throw new AlreadyBeingCreatedException("failed to create file " + src + " for " + holder + " on client "
                  + clientMachine + " because pendingCreates is non-null but no leases found.");
         }
         if (force) {
            ...
            if (lease.expiredSoftLimit()) {
               LOG.info("startFile: recover lease " + lease + ", src=" + src + " from client " + pendingFile.clientName);
               internalReleaseLease(lease, src);
            }
            throw new AlreadyBeingCreatedException("failed to create file " + src + " for " + holder + " on client "
                  + clientMachine + ", because this file is already being created by " + pendingFile.getClientName()
   ...
   public LocatedBlock getAdditionalBlock(String src, String clientName, List<Node> excludedNodes) throws IOException {
      ...
      if (targets.length < this.minReplication) {
         throw new IOException("File " + src + " could only be replicated to " + targets.length + " nodes, instead of "
               + minReplication);
      }
   ...
   private synchronized CompleteFileStatus completeFileInternal(String src, String holder) throws IOException {
      ...
      if (fileBlocks == null) {
         NameNode.stateChangeLog.warn("DIR* NameSystem.completeFile: " + "failed to complete " + src
               + " because dir.getFileBlocks() is null," + " pending from " + pendingFile.getClientMachine());
   ...
   private synchronized void dumpRecentInvalidateSets(PrintWriter out) {
      ...
      out.println("Metasave: Blocks " + pendingDeletionBlocksCount + " waiting deletion from " + size + " datanodes.");
      if (size == 0) {
         return;
      }
   ...
   public DirectoryListing getListing(String src, byte[] startAfter) throws IOException {
      ...
      if (auditLog.isInfoEnabled() && isExternalInvocation()) {
         logAuditEvent(UserGroupInformation.getCurrentUser(), Server.getRemoteIp(), "listStatus", src, null, null);
      }
   ...
   public synchronized void registerDatanode(DatanodeRegistration nodeReg) throws IOException {
      ...
      if (nodeS != null) {
         if (nodeN == nodeS) {
            ...
            */
            NameNode.stateChangeLog.info("BLOCK* NameSystem.registerDatanode: " + "node " + nodeS.getName()
                  + " is replaced by " + nodeReg.getName() + " with the same storageID " + nodeReg.getStorageID());
         }
         // update cluster map
   ...
   boolean computeReplicationWorkForBlock(Block block, int priority) {
      ...
      DatanodeDescriptor targets[] = replicator.chooseTarget(requiredReplication - numEffectiveReplicas, srcNode,
               ...
               synchronized (heartbeats) {
                  synchronized (datanodeMap) {
                     ...
                     if (nodeInfo != null && isDatanodeDead(nodeInfo)) {
                        NameNode.stateChangeLog.info("BLOCK* NameSystem.heartbeatCheck: " + "lost heartbeat from "
                              + nodeInfo.getName());
                        removeDatanode(nodeInfo);
                     }
                  }
   ...
   private Block rejectAddStoredBlock(Block block, DatanodeDescriptor node, String msg) {
      NameNode.stateChangeLog.info("BLOCK* NameSystem.addStoredBlock: " + "addStoredBlock request received for "
            + block + " on " + node.getName() + " size " + block.getNumBytes() + " but was rejected: " + msg);
   ...
   public synchronized void processBlocksBeingWrittenReport(DatanodeID nodeID, BlockListAsLongs blocksBeingWritten)
      ...
      for (int i = 0; i < blocksBeingWritten.getNumberOfBlocks(); i++) {
         block.set(blocksBeingWritten.getBlockId(i), blocksBeingWritten.getBlockLen(i),
         ...
         boolean isLastBlock = inode.getLastBlock() != null && inode.getLastBlock().getBlockId() == block.getBlockId();
         ...
         if (!isLastBlock) {
            rejectAddStoredBlock(new Block(block), dataNode,
                  "Reported as block being written but not the last block of " + "an under-construction file.");
            continue;
         }
   ...
   public synchronized void processReport(DatanodeID nodeID, BlockListAsLongs newReport) throws IOException {
      ...
      if (isInStartupSafeMode() && !node.firstBlockReport()) {
         NameNode.stateChangeLog.info("BLOCK* NameSystem.processReport: " + "discarded non-initial block report from "
               + nodeID.getName() + " because namenode still in startup phase");
         return;
      }
      ...
      NameNode.stateChangeLog.info("*BLOCK* NameSystem.processReport: from " + nodeID.getName() + ", blocks: "
            + newReport.getNumberOfBlocks() + ", processing time: " + (endTime - startTime) + " msecs");
   ...
   synchronized Block addStoredBlock(Block block, DatanodeDescriptor node, DatanodeDescriptor delNodeHint) {
      ...
      if (storedBlock == null) {
         ...
         if (storedBlock == null) {
            return rejectAddStoredBlock(block, node, "Block not in blockMap with any generation stamp");
         }
         ...
         if (reportedOldGS && !(underConstruction && isLastBlock)) {
            return rejectAddStoredBlock(block, node,
                  "Reported block has old generation stamp but is not the last block of "
                        + "an under-construction file. (current generation is " + storedBlock.getGenerationStamp()
                        + ")");
         }
         ...
         if (underConstruction && isLastBlock && (reportedOldGS || reportedNewGS)) {
            NameNode.stateChangeLog.info("BLOCK* NameSystem.addStoredBlock: " + "Targets updated: block " + block
                  + " on " + node.getName() + " is added as a target for block " + storedBlock + " with size "
                  + block.getNumBytes());
      ...
      if (fileINode.isUnderConstruction()) {
         ...
         if (last == null) {
            ...
            // an NPE below.
            LOG.error("Null blocks for reported block=" + block + " stored=" + storedBlock + " inode=" + fileINode);
            return block;
         }
      ...
      if (added) {
         ...
         //
         if (!isInSafeMode()) {
            NameNode.stateChangeLog.info("BLOCK* NameSystem.addStoredBlock: " + "blockMap updated: " + node.getName()
                  + " is added to " + block + " size " + block.getNumBytes());
         }
      } else {
   ...
   long[] getStats() throws IOException {
      checkSuperuserPrivilege();
      synchronized (heartbeats) {
         return new long[] { this.capacityTotal, this.capacityUsed, this.capacityRemaining,
               this.underReplicatedBlocksCount, this.corruptReplicaBlocksCount, getMissingBlocksCount() };
      }
   }
   ...
   private void startDecommission(DatanodeDescriptor node) throws IOException {
      ...
      if (!node.isDecommissionInProgress() && !node.isDecommissioned()) {
         ...
         node.getDecommissioningStatus().setStartTime(now());
         //
   ...
   private void logBlockReplicationInfo(Block block, DatanodeDescriptor srcNode, NumberReplicas num) {
      ...
      FSNamesystem.LOG.info("Block: " + block + ", Expected Replicas: " + curExpectedReplicas + ", live replicas: "
            ...
            + fileINode.isUnderConstruction() + ", Datanodes having this block: " + nodeList + ", Current Datanode: "
   ...
   private boolean isReplicationInProgress(DatanodeDescriptor srcNode) {
      ...
      srcNode.getDecommissioningStatus().set(underReplicatedBlocks, decommissionOnlyReplicas,
            underReplicatedInOpenFiles);
   ...
   class SafeModeInfo {
      ...
      synchronized void leave(boolean checkForUpgrades) {
         ...
         NameNode.stateChangeLog.info("STATE* Safe mode termination "
               ...
               + (now() - startTimeMisReplicatedScan) + " msec");
   ...
   private FSPermissionChecker checkPathAccess(String path, FsAction access) throws AccessControlException {
      return checkPermission(path, false, null, null, access, null);
   }

   private FSPermissionChecker checkParentAccess(String path, FsAction access) throws AccessControlException {
      return checkPermission(path, false, null, access, null, null);
   }

   private FSPermissionChecker checkAncestorAccess(String path, FsAction access) throws AccessControlException {
    ...
    */
   private FSPermissionChecker checkPermission(String path, boolean doCheckOwner, FsAction ancestorAccess,
   ...
   synchronized long nextGenerationStampForBlock(Block block, boolean fromNN) throws IOException {
      ...
      if (!fromNN
            && HdfsConstants.NN_RECOVERY_LEASEHOLDER.equals(leaseManager.getLeaseByPath(
                  FSDirectory.getFullPathName(fileINode)).getHolder())) {
   ...
   private DelegationTokenSecretManager createDelegationTokenSecretManager(Configuration conf) {
      return new DelegationTokenSecretManager(conf.getLong("dfs.namenode.delegation.key.update-interval",
            24 * 60 * 60 * 1000), conf.getLong("dfs.namenode.delegation.token.max-lifetime", 7 * 24 * 60 * 60 * 1000),
            conf.getLong("dfs.namenode.delegation.token.renew-interval", 24 * 60 * 60 * 1000),
   ...
   private boolean isAllowedDelegationTokenOp() throws IOException {
      ...
      if (UserGroupInformation.isSecurityEnabled() && (authMethod != AuthenticationMethod.KERBEROS)
            && (authMethod != AuthenticationMethod.KERBEROS_SSL) && (authMethod != AuthenticationMethod.CERTIFICATE)) {
         return false;
      }
      return true;
   }
   ...
   public String getLiveNodes() {
      ...
      final ArrayList<DatanodeDescriptor> aliveNodeList = this.getDatanodeListForReport(DatanodeReportType.LIVE);
   ...
   public String getDeadNodes() {
      final Map<String, Object> info = new HashMap<String, Object>();
      final ArrayList<DatanodeDescriptor> deadNodeList = this.getDatanodeListForReport(DatanodeReportType.DEAD);
   ...
   public String getDecomNodes() {
      final Map<String, Object> info = new HashMap<String, Object>();
      final ArrayList<DatanodeDescriptor> decomNodeList = this.getDecommissioningNodes();
      for (DatanodeDescriptor node : decomNodeList) {
         final Map<String, Object> innerinfo = new HashMap<String, Object>();
         innerinfo.put("underReplicatedBlocks", node.getDecommissioningStatus().getUnderReplicatedBlocks());
         innerinfo.put("decommissionOnlyReplicas", node.getDecommissioningStatus().getDecommissionOnlyReplicas());
         innerinfo.put("underReplicateInOpenFiles", node.getDecommissioningStatus().getUnderReplicatedInOpenFiles());
         info.put(node.getHostName(), innerinfo);
      }
      return JSON.toString(info);
   }
   ...
   public String getNameDirStatuses() {
      ...
      for (Iterator<StorageDirectory> it = getFSImage().dirIterator(); it.hasNext();) {
   ...
   public void getMetrics(MetricsBuilder builder, boolean all) {
      builder.addRecord("FSNamesystem").setContext("dfs").addGauge("FilesTotal", "", getFilesTotal())
            .addGauge("BlocksTotal", "", getBlocksTotal())
            .addGauge("CapacityTotalGB", "", roundBytesToGBytes(getCapacityTotal()))
            .addGauge("CapacityUsedGB", "", roundBytesToGBytes(getCapacityUsed()))
            .addGauge("CapacityRemainingGB", "", roundBytesToGBytes(getCapacityRemaining()))
            .addGauge("TotalLoad", "", getTotalLoad()).addGauge("CorruptBlocks", "", getCorruptReplicaBlocks())
            ...
            .addGauge("ScheduledReplicationBlocks", "", getScheduledReplicationBlocks())
            .addGauge("MissingBlocks", "", getMissingBlocksCount()).addGauge("BlockCapacity", "", getBlockCapacity());
   }
   ...
   void removeDecomNodeFromDeadList(ArrayList<DatanodeDescriptor> dead) {
      ...
      for (Iterator<DatanodeDescriptor> it = dead.iterator(); it.hasNext();) {
         DatanodeDescriptor node = it.next();
         if ((!inHostsList(node, null)) && (!inExcludedHostsList(node, null)) && node.isDecommissioned()) {
