...
public class DataNode extends Configured implements InterDatanodeProtocol, ClientDatanodeProtocol, FSConstants,
   ...
   void startDataNode(Configuration conf, AbstractList<File> dataDirs, SecureResources resources) throws IOException {
      ...
      this.infoServer = (secureResources == null) ? new HttpServer("datanode", infoHost, tmpInfoPort, tmpInfoPort == 0,
            conf, SecurityUtil.getAdminAcls(conf, DFSConfigKeys.DFS_ADMIN)) : new HttpServer("datanode", infoHost,
            tmpInfoPort, tmpInfoPort == 0, conf, SecurityUtil.getAdminAcls(conf, DFSConfigKeys.DFS_ADMIN),
      ...
      if (WebHdfsFileSystem.isEnabled(conf, LOG)) {
         infoServer.addJerseyResourcePackage(DatanodeWebHdfsMethods.class.getPackage().getName() + ";"
               + Param.class.getPackage().getName(), WebHdfsFileSystem.PATH_PREFIX + "/*");
      }
   ...
   private void register() throws IOException {
      ...
      if (!isBlockTokenInitialized) {
         ...
         if (isBlockTokenEnabled) {
            ...
            LOG.info("Block token params received from NN: keyUpdateInterval=" + blockKeyUpdateInterval / (60 * 1000)
      ...
      while (shouldRun) {
         try {
            ...
            if (startTime - lastHeartbeat > heartBeatInterval) {
               ...
               DatanodeCommand[] cmds = namenode.sendHeartbeat(dnRegistration, data.getCapacity(), data.getDfsUsed(),
            ...
            if (startTime - lastBlockReport > blockReportInterval) {
               if (data.isAsyncBlockReportReady()) {
                  ...
                  LOG.info("BlockReport of " + bReport.length + " blocks took " + brCreateCost
                        + " msec to generate and " + brSendCost + " msecs for RPC and NN processing");
                  ...
                  if (lastBlockReport > 0) { // this isn't the first report
                     ...
                     String msg = "Block report is due, and been waiting for it for " + (waitingFor / 1000)
                           + " seconds...";
   ...
   private void delayBeforeBlockReceived() {
      if (artificialBlockReceivedDelay > 0 && !receivedBlockList.isEmpty()) {
         try {
            ...
            LOG.debug("DataNode " + dnRegistration + " sleeping for " + "artificial delay: " + sleepFor + " ms");
   ...
   private void transferBlock(Block block, DatanodeInfo xferTargets[]) throws IOException {
      ...
      if (block.getNumBytes() > onDiskLength) {
         ...
         namenode.reportBadBlocks(new LocatedBlock[] { new LocatedBlock(block, new DatanodeInfo[] { new DatanodeInfo(
               dnRegistration) }) });
    ...
    * @throws IOException
    */
   public static DataNode makeInstance(String[] dataDirs, Configuration conf, SecureResources resources)
         throws IOException {
   ...
   public String toString() {
      return "DataNode{" + "data=" + data + ", localName='" + dnRegistration.getName() + "'" + ", storageID='"
            + dnRegistration.getStorageID() + "'" + ", xmitsInProgress=" + xmitsInProgress.get() + "}";
   }
   ...
   public BlockMetaDataInfo getBlockMetaDataInfo(Block block) throws IOException {
      ...
      if (LOG.isDebugEnabled()) {
         LOG.debug("getBlockMetaDataInfo successful block=" + stored + " length " + stored.getNumBytes() + " genstamp "
   ...
   private void checkBlockToken(Block block, Token<BlockTokenIdentifier> token, AccessMode accessMode)
         throws IOException {
   ...
   private void checkBlockToken(Block block, BlockTokenSecretManager.AccessMode accessMode) throws IOException {
      if (isBlockTokenEnabled && UserGroupInformation.isSecurityEnabled()) {
         ...
         if (tokenIds.size() != 1) {
            throw new IOException("Can't continue with " + "authorization since " + tokenIds.size()
         ...
         for (DatanodeID id : datanodeids) {
            try {
               InterDatanodeProtocol datanode = dnRegistration.equals(id) ? this : DataNode
                     .createInterDataNodeProtocolProxy(id, getConf(), socketTimeout);
               ...
               if (info.getBlock().getGenerationStamp() < block.getGenerationStamp()) {
                  LOG.info("Only old generation stamp " + info.getBlock().getGenerationStamp() + " found on datanode "
   ...
   private LocatedBlock syncBlock(Block block, List<BlockRecord> syncList, DatanodeInfo[] targets, boolean closeFile)
         throws IOException {
      if (LOG.isDebugEnabled()) {
         LOG.debug("block=" + block + ", (length=" + block.getNumBytes() + "), syncList=" + syncList + ", closeFile="
               + closeFile);
      }
      ...
      if (!successList.isEmpty()) {
         ...
         namenode.commitBlockSynchronization(block, newblock.getGenerationStamp(), newblock.getNumBytes(), closeFile,
               false, nlist);
