...
public class NameNode implements ClientProtocol, DatanodeProtocol, NamenodeProtocol, FSConstants,
   ...
   */
         ...
         if (httpsUser == null) {
            LOG.warn(DFSConfigKeys.DFS_NAMENODE_KRB_HTTPS_USER_NAME_KEY
                  ...
                  + SecurityUtil.getServerPrincipal(conf.get(DFSConfigKeys.DFS_NAMENODE_USER_NAME_KEY),
                        serverAddress.getHostName()) + ": Kerberized SSL may be not function correctly.");
         } else {
      ...
      try {
         this.httpServer = ugi.doAs(new PrivilegedExceptionAction<HttpServer>() {
            ...
            public HttpServer run() throws IOException, InterruptedException {
               ...
               httpServer = new HttpServer("hdfs", infoHost, infoPort, infoPort == 0, conf, SecurityUtil.getAdminAcls(
               ...
               if (certSSL || useKrb) {
                  ...
                  InetSocketAddress secInfoSocAddr = NetUtils.createSocketAddr(infoHost + ":"
                        + conf.get("dfs.https.port", infoHost + ":" + 0));
                  ...
                  InetSocketAddress datanodeSslPort = NetUtils.createSocketAddr(conf.get("dfs.datanode.https.address",
                        infoHost + ":" + 50475));
         ...
         if (UserGroupInformation.isSecurityEnabled()
            ...
            LOG.info("Logging back in as "
                  + SecurityUtil.getServerPrincipal(conf.get(DFSConfigKeys.DFS_NAMENODE_USER_NAME_KEY),
                        serverAddress.getHostName()) + " following http server start.");
            SecurityUtil.login(conf, DFSConfigKeys.DFS_NAMENODE_KEYTAB_FILE_KEY,
   ...
   public boolean mkdirs(String src, FsPermission masked) throws IOException {
      ...
      return namesystem.mkdirs(src, new PermissionStatus(UserGroupInformation.getCurrentUser().getShortUserName(),
            null, masked));
   }
