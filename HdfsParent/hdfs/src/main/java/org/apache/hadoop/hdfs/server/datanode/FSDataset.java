...
public class FSDataset implements FSConstants, FSDatasetInterface {
   ...
   static class FSVolumeSet {
      ...
      synchronized void getBlocksBeingWrittenInfo(TreeSet<Block> blockSet) {
         ...
         DataNode.LOG.info("Finished generating blocks being written report for " + volumes.length + " volumes in "
               + scanTime + " seconds");
      }
      ...
      synchronized List<FSVolume> checkDirs() {
         ...
         Log.info("Completed FSVolumeSet.checkDirs. Removed=" + removed_size + "volumes. List of current volumes: "
               + toString());
   ...
   public FSDataset(DataStorage storage, Configuration conf) throws IOException {
      ...
      if (volsFailed < 0 || volsFailed > volFailuresTolerated) {
         throw new DiskErrorException("Invalid value for volsFailed : " + volsFailed + " , Volumes tolerated : "
      ...
      if (validVolsRequired < 1 || validVolsRequired > storage.getNumStorageDirs()) {
         throw new DiskErrorException("Invalid value for validVolsRequired : " + validVolsRequired
   ...
   public void updateBlock(Block oldblock, Block newblock) throws IOException {
      ...
      boolean isValidUpdate = (newblock.getGenerationStamp() > oldblock.getGenerationStamp())
            || (newblock.getGenerationStamp() == oldblock.getGenerationStamp() && newblock.getNumBytes() == oldblock
                  .getNumBytes());
   ...
   private synchronized List<Thread> tryUpdateBlock(Block oldblock, Block newblock) throws IOException {
      ...
      if (oldgs > newblock.getGenerationStamp()) {
         throw new IOException("Cannot update block (id=" + newblock.getBlockId() + ") generation stamp from " + oldgs
    ...
    * replication request.
    */
   public BlockWriteStreams writeToBlock(Block b, boolean isRecovery, boolean replicationRequest) throws IOException {
      //
   ...
   public void setChannelPosition(Block b, BlockWriteStreams streams, long dataOffset, long ckOffset)
      ...
      if (file.getChannel().size() < dataOffset) {
         String msg = "Trying to change block file offset of block " + b + " file "
               + volumeMap.get(b).getVolume().getTmpFile(b) + " to " + dataOffset + " but actual size of file is "
   ...
   HashMap<Block, File> roughBlockScan() {
      ...
      HashMap<Block, File> seenOnDisk = new HashMap<Block, File>(expectedNumBlocks, 1.1f);
      volumes.scanBlockFilesInconsistent(seenOnDisk);
      return seenOnDisk;
   }

   static void reconcileRoughBlockScan(Map<Block, File> seenOnDisk, Map<Block, DatanodeBlockInfo> volumeMap,
      ...
      for (Map.Entry<Block, DatanodeBlockInfo> entry : volumeMap.entrySet()) {
         ...
         if (!seenOnDisk.containsKey(b) && info.getFile().exists()) {
            ...
            Block toAdd = new Block(b.getBlockId(), info.getFile().length(), b.getGenerationStamp());
      ...
      if (numDeletedAfterScan + numAddedAfterScan + numOngoingIgnored > 0) {
         DataNode.LOG.info("Reconciled asynchronous block scan with filesystem. " + numDeletedAfterScan
               + " blocks concurrently deleted during scan, " + numAddedAfterScan
               + " blocks concurrently added during scan, " + numOngoingIgnored + " ongoing creations ignored");
      }
   }
   ...
   public synchronized void validateBlockMetadata(Block b) throws IOException {
      ...
      if (f.length() > maxDataSize || f.length() <= minDataSize) {
         throw new IOException("Block " + b + " is of size " + f.length() + " but has " + (numChunksInMeta + 1)
   ...
   public BlockRecoveryInfo startBlockRecovery(long blockId) throws IOException {
      ...
      synchronized (this) {
         ...
         if (DataNode.LOG.isDebugEnabled()) {
            DataNode.LOG.debug("getBlockMetaDataInfo successful block=" + stored + " length " + stored.getNumBytes()
      ...
      public void run() {
         while (shouldRun) {
            try {
               ...
               DataNode.LOG.info("Finished asynchronous block report scan in " + (System.currentTimeMillis() - st)
                     + "ms");
