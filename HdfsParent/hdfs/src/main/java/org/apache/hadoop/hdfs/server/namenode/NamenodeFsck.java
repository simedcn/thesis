...
public class NamenodeFsck {
   ...
   NamenodeFsck(Configuration conf, NameNode namenode, NetworkTopology networktopology, Map<String, String[]> pmap,
         PrintWriter out, int totalDatanodes, short minReplication, InetAddress remoteAddress) {
      this.conf = conf;
   ...
   private void check(String parent, HdfsFileStatus file, Result res) throws IOException {
      ...
      for (LocatedBlock lBlk : blocks.getLocatedBlocks()) {
         ...
         if (locs.length < targetFileReplication && locs.length > 0) {
            ...
            out.println(" Under replicated " + block + ". Target Replicas is " + targetFileReplication + " but found "
