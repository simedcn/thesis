...
class DataXceiver implements Runnable, FSConstants {
   ...
   private void readBlock(DataInputStream in) throws IOException {
      ...
      final String clientTraceFmt = clientName.length() > 0 && ClientTraceLog.isInfoEnabled() ? String.format(
            ...
            datanode.dnRegistration.getStorageID(), block, "%d") : datanode.dnRegistration + " Served block " + block
            + " to " + s.getInetAddress();
      try {
         try {
   ...
   private void copyBlock(DataInputStream in) throws IOException {
      ...
      if (datanode.isBlockTokenEnabled) {
         try {
            ...
            LOG.warn("Invalid access token in request from " + remoteAddress + " for OP_COPY_BLOCK for block " + block);
            sendResponse(s, (short) DataTransferProtocol.OP_STATUS_ERROR_ACCESS_TOKEN, datanode.socketWriteTimeout);
            return;
         }
      }
   ...
   private void replaceBlock(DataInputStream in) throws IOException {
      ...
      try {
         ...
         // open a block receiver and check if the block does not exist
         blockReceiver = new BlockReceiver(block, proxyReply, proxySock.getRemoteSocketAddress().toString(), proxySock
               .getLocalSocketAddress().toString(), false, "", null, datanode);
