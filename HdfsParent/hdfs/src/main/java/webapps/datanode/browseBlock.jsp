<%@ page contentType="text/html; charset=UTF-8" import="javax.servlet.*"
	...
	import="java.net.*" import="org.apache.hadoop.hdfs.*"
	import="org.apache.hadoop.hdfs.server.namenode.*"
	import="org.apache.hadoop.hdfs.server.datanode.*"
	import="org.apache.hadoop.hdfs.server.common.*"
	import="org.apache.hadoop.hdfs.protocol.*"
	import="org.apache.hadoop.io.*" import="org.apache.hadoop.conf.*"
...
<%!static JspHelper jspHelper = new JspHelper();
	...
	public void generateFileDetails(JspWriter out, HttpServletRequest req,
		...
		String filename = HtmlQuoting.unquoteHtmlChars(req
		...
		String downloadUrl = "http://" + req.getServerName() + ":"
				...
				+ JspHelper.DELEGATION_PARAMETER_NAME + "=" + tokenString;
		...
		out.print("<input type=\"hidden\" name=\"blockId\" value=\""
				+ currBlockId + "\">");
		...
		for (LocatedBlock cur : blocks) {
			...
			for (int j = 0; j < locs.length; j++) {
				...
				fqdn = InetAddress.getByName(locs[j].getHost())
						.getCanonicalHostName();
				String blockUrl = "http://" + fqdn + ":"
						+ locs[j].getInfoPort() + "/browseBlock.jsp?blockId="
						+ Long.toString(blockId) + "&blockSize=" + blockSize
						+ "&filename=" + URLEncoder.encode(filename, "UTF-8")
						+ "&datanodePort=" + datanodePort + "&genstamp="
	...
	public void generateFileChunks(JspWriter out, HttpServletRequest req,
		...
		String tokenString = req
				.getParameter(JspHelper.DELEGATION_PARAMETER_NAME);
		UserGroupInformation ugi = JspHelper.getUGI(req, conf);
		final DFSClient dfs = JspHelper.getDFSClient(ugi,
				jspHelper.nameNodeAddr, conf);

		Token<BlockTokenIdentifier> accessToken = BlockTokenSecretManager.DUMMY_TOKEN;
		if (conf.getBoolean(DFSConfigKeys.DFS_BLOCK_ACCESS_TOKEN_ENABLE_KEY,
				false)) {
			List<LocatedBlock> blks = dfs.namenode.getBlockLocations(filename,
					0, Long.MAX_VALUE).getLocatedBlocks();
		...
		datanodePort = Integer.parseInt(datanodePortStr);
		out.print("<h3>File: ");
		JspHelper.printPathWithLinks(HtmlQuoting.quoteHtmlChars(filename), out,
				namenodeInfoPort, tokenString);
		out.print("</h3><hr>");
		...
		if (startOffset + chunkSizeToView >= blockSize) {
			...
			List<LocatedBlock> blocks = dfs.namenode.getBlockLocations(
					filename, 0, Long.MAX_VALUE).getLocatedBlocks();
			for (int i = 0; i < blocks.size(); i++) {
				if (blocks.get(i).getBlock().getBlockId() == blockId) {
					if (i != blocks.size() - 1) {
						LocatedBlock nextBlock = blocks.get(i + 1);
						nextBlockIdStr = Long.toString(nextBlock.getBlock()
								.getBlockId());
						...
						nextDatanodePort = Integer.parseInt(datanodeAddr
								.substring(datanodeAddr.indexOf(':') + 1,
										datanodeAddr.length()));
		...
		if (nextBlockIdStr != null) {
			nextUrl = "http://" + nextHost + ":" + nextPort
					...
					+ nextStartOffset + "&genstamp=" + nextGenStamp
					+ "&filename=" + URLEncoder.encode(filename, "UTF-8")
					+ "&chunkSizeToView=" + chunkSizeToView + "&datanodePort="
					+ nextDatanodePort + "&namenodeInfoPort="
					+ namenodeInfoPort
					+ JspHelper.getDelegationTokenUrlParam(tokenString);
			out.print("<a href=\"" + nextUrl
					+ "\">View Next chunk</a>&nbsp;&nbsp;");
		}
			...
			for (int i = 0; i < blocks.size(); i++) {
				if (blocks.get(i).getBlock().getBlockId() == blockId) {
					if (i != 0) {
						...
						prevBlockIdStr = Long.toString(prevBlock.getBlock()
								.getBlockId());
						...
						String datanodeAddr = d.getName();
						prevDatanodePort = Integer.parseInt(datanodeAddr
								.substring(datanodeAddr.indexOf(':') + 1,
										datanodeAddr.length()));
		...
		if (prevBlockIdStr != null) {
			prevUrl = "http://" + prevHost + ":" + prevPort
					...
					+ URLEncoder.encode(filename, "UTF-8")
					+ "&chunkSizeToView=" + chunkSizeToView + "&genstamp="
					+ prevGenStamp + "&datanodePort=" + prevDatanodePort
					+ "&namenodeInfoPort=" + namenodeInfoPort
					+ JspHelper.getDelegationTokenUrlParam(tokenString);
			out.print("<a href=\"" + prevUrl
					+ "\">View Prev chunk</a>&nbsp;&nbsp;");
		}
		out.print("<hr>");
		...
		try {
			jspHelper.streamBlockInAscii(
					...
					blockId, accessToken, genStamp, blockSize, startOffset,
					chunkSizeToView, out, conf);
		} catch (Exception e) {
			out.print(e);
		}
		out.print("</textarea>");
...
<head>
<%
	JspHelper.createTitle(out, request,
			request.getParameter("filename"));
%>
</head>
<body onload="document.goto.dir.focus()">
	<%
		Configuration conf = (Configuration) getServletContext()
