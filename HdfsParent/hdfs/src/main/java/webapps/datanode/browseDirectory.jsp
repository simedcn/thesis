<%@ page contentType="text/html; charset=UTF-8" import="javax.servlet.*"
	...
	import="java.net.*" import="org.apache.hadoop.conf.Configuration"
	import="org.apache.hadoop.fs.*" import="org.apache.hadoop.hdfs.*"
...
<%!static JspHelper jspHelper = new JspHelper();
	...
	public void generateDirectoryStructure(JspWriter out,
			HttpServletRequest req, HttpServletResponse resp, Configuration conf)
		...
		if (!dfs.exists(target)) {
			out.print("<h3>File or directory : " + target
					+ " does not exist</h3>");
			...
			if (!dfs.isDirectory(target)) { // a file
				List<LocatedBlock> blocks = dfs.namenode.getBlockLocations(dir,
						0, 1).getLocatedBlocks();
			...
			String[] headings = { "Name", "Type", "Size", "Replication",
					"Block Size", "Modification Time", "Permission", "Owner",
					"Group" };
			out.print("<h3>Contents of directory ");
			JspHelper.printPathWithLinks(HtmlQuoting.quoteHtmlChars(dir), out,
			...
			if ((parent = f.getParent()) != null)
				out.print("<a href=\"" + req.getRequestURL() + "?dir="
						+ URLEncoder.encode(parent, "UTF-8")
						+ "&namenodeInfoPort=" + namenodeInfoPort
						+ JspHelper.getDelegationTokenUrlParam(tokenString)
						+ "\">Go to parent directory</a><br>");

			DirectoryListing thisListing = dfs.listPaths(target,
				...
				do {
					...
					for (int i = 0; i < files.length; i++) {
						...
						if (!files[i].isDir()) {
							...
							cols[4] = StringUtils.byteDesc(files[i]
									.getBlockSize());
						} else {
							cols[1] = "dir";
							cols[2] = "";
						...
						String datanodeUrl = req.getRequestURL()
								+ "?dir="
								+ URLEncoder.encode(
										files[i].getFullName(target), "UTF-8")
								+ "&namenodeInfoPort="
								+ namenodeInfoPort
								+ JspHelper
					...
					thisListing = dfs.listPaths(target,
							thisListing.getLastName());
