...
<%!JspHelper jspHelper = new JspHelper();
	...
	void generateDecommissioningNodeData(JspWriter out, DatanodeDescriptor d,
		...
		long decommRequestTime = d.getDecommissioningStatus().getStartTime();
		...
		out.print("<td class=\"lastcontact\"> "
				...
				+ d.getDecommissioningStatus().getUnderReplicatedBlocks()
				+ "<td class=\"blockswithonlydecommissioningreplicas\">"
				+ d.getDecommissioningStatus().getDecommissionOnlyReplicas()
				+ "<td class=\"underrepblocksinfilesunderconstruction\">"
				+ d.getDecommissioningStatus().getUnderReplicatedInOpenFiles()
				+ "<td class=\"timesincedecommissionrequest\">"
				+ hoursSinceDecommStarted + " hrs " + remainderMinutes
				+ " mins" + "\n");
	}
			...
			if (whatNodes.equals("LIVE")) {
				...
				if (live.size() > 0) {
					...
					out.print("<tr class=\"headerRow\"> <th "
							...
							+ "> Last <br>Contact <th "
							+ NodeHeaderStr("adminstate")
							+ "> Admin State <th " + NodeHeaderStr("capacity")
							+ "> Configured <br>Capacity (" + diskByteStr
							+ ") <th " + NodeHeaderStr("used") + "> Used <br>("
							+ diskByteStr + ") <th "
							+ NodeHeaderStr("nondfsused")
				...
				ArrayList<DatanodeDescriptor> decommissioning = nn
						.getNamesystem().getDecommissioningNodes();
				out.print("<br> <a name=\"DecommissioningNodes\" id=\"title\"> "
						+ " Decommissioning Datanodes : "
						+ decommissioning.size() + "</a><br><br>\n");
