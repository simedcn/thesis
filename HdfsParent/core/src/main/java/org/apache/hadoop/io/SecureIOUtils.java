...
public class SecureIOUtils {
   ...
   public static FileOutputStream createForWrite(File f, int permissions) throws IOException {
      if (skipSecurity) {
         ...
         try {
            FileDescriptor fd = NativeIO.open(f.getAbsolutePath(), NativeIO.O_WRONLY | NativeIO.O_CREAT
                  | NativeIO.O_EXCL, permissions);
