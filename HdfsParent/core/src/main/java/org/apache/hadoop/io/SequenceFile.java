...
public class SequenceFile {
   ...
   public static Writer createWriter(FileSystem fs, Configuration conf, Path name, Class keyClass, Class valClass,
      ...
      case NONE:
         return new Writer(conf, fs.createNonRecursive(name, true, bufferSize, replication, blockSize, null), keyClass,
               valClass, metadata).ownStream();
      case RECORD:
         return new RecordCompressWriter(conf, fs.createNonRecursive(name, true, bufferSize, replication, blockSize,
               null), keyClass, valClass, codec, metadata).ownStream();
      case BLOCK:
         return new BlockCompressWriter(conf, fs.createNonRecursive(name, true, bufferSize, replication, blockSize,
               null), keyClass, valClass, codec, metadata).ownStream();
      default:
         return null;
      }
   ...
   public static class Writer implements java.io.Closeable {
      ...
      Writer(Configuration conf, FSDataOutputStream out, Class keyClass, Class valClass, Metadata metadata)
            throws IOException {
   ...
   static class RecordCompressWriter extends Writer {
      ...
      /** Write to an arbitrary stream using a specified buffer size. */
      RecordCompressWriter(Configuration conf, FSDataOutputStream out, Class keyClass, Class valClass,
            CompressionCodec codec, Metadata metadata) throws IOException {
         this.ownOutputStream = false;
   ...
   static class BlockCompressWriter extends Writer {
      ...
      /** Write to an arbitrary stream using a specified buffer size. */
      BlockCompressWriter(Configuration conf, FSDataOutputStream out, Class keyClass, Class valClass,
            CompressionCodec codec, Metadata metadata) throws IOException {
         this.ownOutputStream = false;
         super.init(null, conf, out, keyClass, valClass, true, codec, metadata);
         init(conf.getInt("io.seqfile.compress.blocksize", 1000000));
   ...
   public static class Sorter {
      ...
      public Writer cloneFileAttributes(Path inputFile, Path outputFile, Progressable prog) throws IOException {
         ...
         Writer writer = createWriter(outputFile.getFileSystem(conf), conf, outputFile, keyClass, valClass, compress,
               blockCompress, codec, prog, new Metadata());
         return writer;
      }
      ...
      private class MergeQueue extends PriorityQueue implements RawKeyValueIterator {
         ...
         protected boolean lessThan(Object a, Object b) {
            ...
            return comparator.compare(msa.getKey().getData(), 0, msa.getKey().getLength(), msb.getKey().getData(), 0,
