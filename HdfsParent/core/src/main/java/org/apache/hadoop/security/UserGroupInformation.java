...
public class UserGroupInformation {
   ...
   private static synchronized void initialize(Configuration conf) {
      ...
      if (value == null || "simple".equals(value)) {
         ...
         throw new IllegalArgumentException("Invalid attribute value for " + HADOOP_SECURITY_AUTHENTICATION + " of "
               + value);
      }
      ...
      try {
         if (System.getProperty("java.vendor").contains("IBM")) {
            if (windows) {
               ...
               return (Class<? extends Principal>) (System.getProperty("os.arch").contains("64") ? cl
                     .loadClass("com.ibm.security.auth.UsernamePrincipal") : cl
                     .loadClass("com.ibm.security.auth.LinuxPrincipal"));
            }
         } else {
            return (Class<? extends Principal>) (windows ? cl.loadClass("com.sun.security.auth.NTUserPrincipal") : cl
                  .loadClass("com.sun.security.auth.UnixPrincipal"));
         }
   ...
   private static class HadoopConfiguration extends javax.security.auth.login.Configuration {
      ...
      private static final AppConfigurationEntry HADOOP_LOGIN = new AppConfigurationEntry(
            HadoopLoginModule.class.getName(), LoginModuleControlFlag.REQUIRED, new HashMap<String, String>());
   ...
   public synchronized static UserGroupInformation getLoginUser() throws IOException {
      if (loginUser == null) {
         try {
            ...
            loginUser.setAuthenticationMethod(isSecurityEnabled() ? AuthenticationMethod.KERBEROS
               ...
               public void run() {
                  ...
                  while (shouldRunRenewerThread) {
                     try {
                        ...
                        LOG.warn("Exception encountered while running the" + " renewal command." + ie
