   ...
   public SaslRpcClient(AuthMethod method, Token<? extends TokenIdentifier> token, String serverPrincipal)
      ...
      case DIGEST:
         ...
         saslClient = Sasl.createSaslClient(new String[] { AuthMethod.DIGEST.getMechanismName() }, null, null,
               SaslRpcServer.SASL_DEFAULT_REALM, SaslRpcServer.SASL_PROPS, new SaslClientCallbackHandler(token));
         break;
      case KERBEROS:
         if (LOG.isDebugEnabled()) {
         ...
         if (names.length != 3) {
            throw new IOException("Kerberos principal name does NOT have the expected hostname part: "
                  + serverPrincipal);
         }
         saslClient = Sasl.createSaslClient(new String[] { AuthMethod.KERBEROS.getMechanismName() }, null, names[0],
