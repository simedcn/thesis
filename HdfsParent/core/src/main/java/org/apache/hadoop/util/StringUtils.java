   ...
   public static String unEscapeString(String str, char escapeChar, char[] charsToEscape) {
      ...
      for (int i = 0; i < str.length(); i++) {
         ...
         if (hasPreEscape) {
            if (curChar != escapeChar && !hasChar(charsToEscape, curChar)) {
               // no special char
               throw new IllegalArgumentException("Illegal escaped string " + str + " unescaped " + escapeChar + " at "
                     + (i - 1));
            }
            ...
            if (hasChar(charsToEscape, curChar)) {
               throw new IllegalArgumentException("Illegal escaped string " + str + " unescaped " + curChar + " at "
                     + i);
