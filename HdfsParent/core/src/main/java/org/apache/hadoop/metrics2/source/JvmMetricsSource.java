...
public class JvmMetricsSource implements MetricsSource {
   ...
   public void getMetrics(MetricsBuilder builder, boolean all) {
      MetricsRecordBuilder rb = builder.addRecord(RECORD_NAME).setContext(CONTEXT)
            .tag(PROCESSNAME_KEY, PROCESSNAME_DESC, processName).tag(SESSIONID_KEY, SESSIONID_DESC, sessionId);
      getMemoryUsage(rb);
      getGcUsage(rb);
      getThreadUsage(rb);
      getEventCounters(rb);
   ...
   private void getMemoryUsage(MetricsRecordBuilder rb) {
      ...
      rb.addGauge(NONHEAP_USED_KEY, NONHEAP_USED_DESC, memNonHeap.getUsed() / M)
            .addGauge(NONHEAP_COMMITTED_KEY, NONHEAP_COMMITTED_DESC, memNonHeap.getCommitted() / M)
            .addGauge(HEAP_USED_KEY, HEAP_USED_DESC, memHeap.getUsed() / M)
            .addGauge(HEAP_COMMITTED_KEY, HEAP_COMMITTED_DESC, memHeap.getCommitted() / M);
   }
   ...
   private void getThreadUsage(MetricsRecordBuilder rb) {
      ...
      rb.addGauge(THREADS_NEW_KEY, THREADS_NEW_DESC, threadsNew)
            ...
            .addGauge(THREADS_TIMEDWAITING_KEY, THREADS_TIMEDWAITING_DESC, threadsTimedWaiting)
            .addGauge(THREADS_TERMINATED_KEY, THREADS_TERMINATED_DESC, threadsTerminated);
   }
