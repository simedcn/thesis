...
public class HttpServer implements FilterContainer {
   ...
   public HttpServer(String name, String bindAddress, int port, boolean findPort, Configuration conf,
      ...
      defineFilter(webAppContext, "krb5Filter", Krb5AndCertsSslSocketConnector.Krb5SslFilter.class.getName(), null,
            null);
         ...
         if (listenerStartedExternally) { // Expect that listener was started
            ...
            while (true) {
               try {
                  ...
                  LOG.info("Port returned by webServer.getConnectors()[0]." + "getLocalPort() before open() is " + port
                  ...
                  if (port < 0) {
                     ...
                     while (port < 0) {
                        ...
                        if (numRetries++ > MAX_RETRIES) {
                           throw new Exception(" listener.getLocalPort is returning " + "less than 0 even after "
                                 + numRetries + " resets");
                        }
                        for (int i = 0; i < 2; i++) {
   ...
   public static boolean hasAdministratorAccess(ServletContext servletContext, HttpServletRequest request,
      ...
      if (adminsAcl != null) {
         if (!adminsAcl.isUserAllowed(remoteUserUGI)) {
            response.sendError(HttpServletResponse.SC_UNAUTHORIZED, "User " + remoteUser
                  + " is unauthorized to access this page. " + "AccessControlList for accessing this page : "
                  + adminsAcl.toString());
            return false;
         }
      }
