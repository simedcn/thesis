...
public class HttpServer implements FilterContainer {
	...
	public HttpServer(String name, String bindAddress, int port,
			boolean findPort, Configuration conf, Connector connector)
			throws IOException {
	...
	public HttpServer(String name, String bindAddress, int port,
		...
		webAppContext.getServletContext().setAttribute(CONF_CONTEXT_ATTRIBUTE,
				conf);
	 ...
	 * (except internal Kerberized filters) are not enabled.
	 * 
	 * @param name
	 *            The name of the servlet (can be passed as null)
	 * @param pathSpec
	...
	public void addFilter(String name, String classname,
		...
		defineFilter(webAppContext, name, classname, parameters,
				USER_FACING_URLS);
	...
	public void addSslListener(InetSocketAddress addr, Configuration sslConf,
		...
		SslSocketConnector sslListener = new Krb5AndCertsSslSocketConnector(
				mode);
		sslListener.setHost(addr.getHostName());
		sslListener.setPort(addr.getPort());
		sslListener.setKeystore(sslConf.get("ssl.server.keystore.location"));
		sslListener
				.setPassword(sslConf.get("ssl.server.keystore.password", ""));
		sslListener.setKeyPassword(sslConf.get(
				"ssl.server.keystore.keypassword", ""));
		sslListener.setKeystoreType(sslConf.get("ssl.server.keystore.type",
				"jks"));
		...
		try {
			if (listenerStartedExternally) { // Expect that listener was started
				...
				if (listener.getLocalPort() == -1) // ... and verify
					throw new Exception(
							"Exepected webserver's listener to be started"
				...
				while (true) {
					try {
						...
						if (port < 0) {
							...
							int numRetries = 1;
							while (port < 0) {
								LOG.warn("listener.getLocalPort returned "
										+ port);
	...
	public static boolean hasAdministratorAccess(ServletContext servletContext,
		...
		UserGroupInformation remoteUserUGI = UserGroupInformation
	...
	public static class QuotingInputFilter implements Filter {
		...
		public static class RequestQuoter extends HttpServletRequestWrapper {
			...
			public Enumeration<String> getParameterNames() {
				return new Enumeration<String>() {
					...
					public String nextElement() {
						return HtmlQuoting.quoteHtmlChars(rawIterator
								.nextElement());
					}
				};
			}
			...
			public String getParameter(String name) {
				return HtmlQuoting.quoteHtmlChars(rawRequest
						.getParameter(HtmlQuoting.unquoteHtmlChars(name)));
			}

			@Override
			public String[] getParameterValues(String name) {
				String unquoteName = HtmlQuoting.unquoteHtmlChars(name);
				String[] unquoteValue = rawRequest
						.getParameterValues(unquoteName);
			...
			public Map<String, String[]> getParameterMap() {
				...
				for (Map.Entry<String, String[]> item : raw.entrySet()) {
					...
					for (int i = 0; i < rawValue.length; ++i) {
						cookedValue[i] = HtmlQuoting
								.quoteHtmlChars(rawValue[i]);
					}
		...
		public void doFilter(ServletRequest request, ServletResponse response,
			...
			HttpServletRequestWrapper quoted = new RequestQuoter(
