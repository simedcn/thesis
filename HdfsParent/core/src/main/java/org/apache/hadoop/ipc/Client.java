...
public class Client {
   ...
   private class Connection extends Thread {
      ...
      private synchronized void handleSaslConnectionFailure(final int currRetries, final int maxRetries,
            final Exception ex, final Random rand, final UserGroupInformation ugi) throws IOException,
               ...
               if (ex instanceof SocketTimeoutException) {
                  if (currRetries < maxRetries) {
                     LOG.warn("Encountered " + ex + " while trying to establish"
                           ...
                           + " to server with principal " + serverPrincipal);
            ...
            while (true) {
               ...
               if (useSasl) {
                  ...
                  try {
                     ...
                     handleSaslConnectionFailure(numRetries++, maxRetries, ex, rand, ticket);
                     continue;
                  }
                  if (continueSasl) {
    ...
    */
   @Deprecated
   public Writable[] call(Writable[] params, InetSocketAddress[] addresses, Class<?> protocol,
         UserGroupInformation ticket) throws IOException, InterruptedException {
   ...
   static class ConnectionId {
      ...
      ConnectionId(InetSocketAddress address, Class<?> protocol, UserGroupInformation ticket, int rpcTimeout,
            String serverPrincipal, int maxIdleTime, int maxRetries, boolean tcpNoDelay, int pingInterval) {
      ...
      private static String getRemotePrincipal(Configuration conf, InetSocketAddress address, Class<?> protocol)
         ...
         if (krbInfo != null) {
            ...
            if (serverKey == null) {
               throw new IOException("Can't obtain server Kerberos config key from protocol="
      ...
      public boolean equals(Object obj) {
         ...
         if (obj instanceof ConnectionId) {
            ...
            return isEqual(this.address, that.address) && this.maxIdleTime == that.maxIdleTime
                  && this.maxRetries == that.maxRetries && this.pingInterval == that.pingInterval
      ...
      public int hashCode() {
         ...
         result = PRIME * result + ((serverPrincipal == null) ? 0 : serverPrincipal.hashCode());
