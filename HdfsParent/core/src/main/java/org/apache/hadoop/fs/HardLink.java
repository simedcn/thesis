...
public class HardLink {
   ...
   static private OSType getOSType() {
      ...
      if (osName.contains("Windows")
            && (osName.contains("XP") || osName.contains("2003") || osName.contains("Vista")
   ...
   */
   ...
   private static IOException createIOException(File f, String message, String error, int exitvalue, Exception cause) {
      ...
      final String s = "Failed to get link count on file " + f + ": message=" + message + "; error=" + error
            + ((osType == OSType.OS_TYPE_WINXP) ? winErrMsg : "") + "; exit value=" + exitvalue;
   ...
   public static class LinkStats {
      ...
      public String report() {
         return "HardLinkStats: " + countDirs + " Directories, including " + countEmptyDirs + " Empty Directories, "
               + countSingleLinks + " single Link operations, " + countMultLinks + " multi-Link operations, linking "
               + countFilesMultLinks + " files, total " + (countSingleLinks + countFilesMultLinks)
               + " linkable files.  Also physically copied " + countPhysicalFileCopies + " other files.";
      }
   }
}
